...
const root = path.resolve('docs/evidence/c4b-pptx-text-patch')
...
if (manifest.environment !== 'Tauri Debug WebView2 via Chrome DevTools Protocol') {
  failures.push('C4B evidence must come from the real Tauri WebView2 desktop')
}
if (manifest.producer !== 'wps-presentation') failures.push('C4B desktop evidence must use the real WPS fixture')
if ((manifest.viewportMatrix || []).join(',') !== '1280x820,960x720') {
  failures.push('C4B evidence must cover normal and compact Library widths')
}
const requiredChecks = new Set([
  'safe-slide-and-notes-targets-visible',
  'slide-text-single-part-preview-verified',
  'speaker-notes-single-part-preview-verified',
...
for (const check of manifest.checks || []) {
  ...
  if (check.id === 'wps-source-bytes-unchanged' && check.sourceUnchanged !== true) {
    failures.push('C4B must prove the source WPS PPTX is byte-identical')
  }
}
...
if ((manifest.evidenceFiles || []).length !== 2) failures.push('expected exactly 2 C4B screenshots')
if (failures.length) {
  console.error(failures.map(failure => `- ${failure}`).join('\n'))
  process.exit(1)
}
console.log('C4B desktop evidence passed: 6 checks, 2 viewport sizes, and 2 screenshots.')
