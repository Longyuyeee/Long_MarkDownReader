param([string]$OutputDirectory = "docs\evidence\post-v115-m4c0-controlled-conversion-workflow-selection")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4c0-controlled-conversion-workflow-selection"))
if ($output -ne $expectedOutput) { throw "M4C-0 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14532
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4C-0 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4c0-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
[IO.File]::WriteAllText((Join-Path $library "Conversion Matrix.csv"), "format,current,target`r`nXLSX,bounded-edit,object-depth`r`nDOCX,bounded-edit,layout-depth`r`nPPTX,reliable-copy,object-depth`r`n", [Text.UTF8Encoding]::new($true))
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\formats\mindmap.opml") -Destination (Join-Path $library "Conversion Outline.opml")
[IO.File]::WriteAllText((Join-Path $library "Graph Center.md"), "# Graph Center`n`n[[Graph Peer]]`n", [Text.UTF8Encoding]::new($false))
[IO.File]::WriteAllText((Join-Path $library "Graph Peer.md"), "# Graph Peer`n`n[[Graph Center]]`n", [Text.UTF8Encoding]::new($false))
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4C0_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4C0_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4C0_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4c0-controlled-conversion-workflow-selection-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4C-0 desktop capture failed" }
...
Write-Output "M4C-0 controlled conversion selection audit completed: $output"
