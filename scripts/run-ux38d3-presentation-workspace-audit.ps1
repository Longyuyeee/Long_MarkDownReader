param([string]$OutputDirectory = "docs\evidence\ux38d3-presentation-workspace")

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\ux38d3-presentation-workspace"))
if ($output -ne $expectedOutput) { throw "UX-38D3 output must remain inside $expectedOutput" }
$appPort = 14200
$cdpPort = 14423
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "UX-38D3 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-ux38d3-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$pptx = Join-Path $library "UX38D3 PowerPoint Presentation.pptx"
$odp = Join-Path $library "UX38D3 OpenDocument Presentation.odp"
Copy-Item -LiteralPath (Join-Path $workspace "fixtures\pptx\producers\microsoft-powerpoint-16.pptx") -Destination $pptx
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\odf-content\longedit-e1c-presentation.odp") -Destination $odp
$before = @{ pptx=(Get-FileHash -LiteralPath $pptx -Algorithm SHA256).Hash; odp=(Get-FileHash -LiteralPath $odp -Algorithm SHA256).Hash }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_UX38D3_AUDIT_OUTPUT = $output
    $env:LONGEDIT_UX38D3_SOURCE_COMMIT = $sourceCommit
    $env:LONGEDIT_UX38D3_PPTX = $pptx
    $env:LONGEDIT_UX38D3_ODP = $odp
    & node (Join-Path $workspace "scripts\capture-ux38d3-presentation-workspace.mjs")
    if ($LASTEXITCODE -ne 0) { throw "UX-38D3 desktop capture failed" }
    $unchanged = $before.pptx -eq (Get-FileHash -LiteralPath $pptx -Algorithm SHA256).Hash -and $before.odp -eq (Get-FileHash -LiteralPath $odp -Algorithm SHA256).Hash
    ...
    if (-not $unchanged) { throw "UX-38D3 changed a source fixture" }
  } finally {
...
Write-Output "UX-38D3 presentation workspace audit completed: $output"
