...
if (contract.stage !== 'P1-B2B4' || contract.status !== 'checkbox-form-copy-complete' || !contract.currentCapabilities?.includes('bounded-checkbox-form-copy')) fail('P1-B2B4 contract is stale')
if (contract.plannedSlices?.find(item => item.id === 'P1-B2B')?.status !== 'checkbox-complete-radio-choice-pending') fail('P1-B2B4 next-slice boundary is stale')
