...
if (!['P1-B2B4', 'P1-B2B5'].includes(contract.stage) || !['checkbox-form-copy-complete', 'radio-form-copy-complete'].includes(contract.status) || !contract.currentCapabilities?.includes('bounded-checkbox-form-copy')) fail('P1-B2B4 contract lineage is stale')
if (!['checkbox-complete-radio-choice-pending', 'radio-complete-choice-pending'].includes(contract.plannedSlices?.find(item => item.id === 'P1-B2B')?.status)) fail('P1-B2B4 next-slice lineage is stale')
