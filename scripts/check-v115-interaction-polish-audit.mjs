...
if (!evidence.dropdownLight?.visible || evidence.dropdownLight?.borderRadius === '0px' || evidence.dropdownLight?.boxShadow === 'none' || Number(evidence.dropdownLight?.zIndex) < 1000 || evidence.dropdownLight?.height > 520 || evidence.dropdownLight?.bottom > evidence.dropdownLight?.viewportHeight + 1) fail('light dropdown surface, stacking or viewport bounds are not accepted')
