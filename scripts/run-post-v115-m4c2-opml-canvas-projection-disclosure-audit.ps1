param([string]$OutputDirectory = "docs\evidence\post-v115-m4c2-opml-canvas-projection-disclosure")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4c2-opml-canvas-projection-disclosure"))
if ($output -ne $expectedOutput) { throw "M4C-2 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14532
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4C-2 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4c2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
$library = Join-Path $auditRoot "library"; $maps = Join-Path $library "maps"; $webviewData = Join-Path $auditRoot "webview"
Remove-Item -LiteralPath $output -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $maps,$webviewData,$output -Force | Out-Null
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\formats\mindmap.opml") -Destination (Join-Path $maps "Conversion Outline.opml")
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4C2_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4C2_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4C2_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4c2-opml-canvas-projection-disclosure-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4C-2 desktop capture failed" }
...
Write-Output "M4C-2 OPML Canvas projection disclosure audit completed: $output"
