param([string]$OutputDirectory = "docs\evidence\post-v115-m1a1-xlsx-validation")

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m1a1-xlsx-validation"))
if ($output -ne $expectedOutput) { throw "M1A1 output must remain inside $expectedOutput" }
$appPort = 14200
$cdpPort = 14412
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1A1 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m1a1-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$xlsx = Join-Path $library "M1A1 Validation.xlsx"
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_M1A1_AUDIT_OUTPUT = $output
    $env:LONGEDIT_M1A1_SOURCE_COMMIT = $sourceCommit
    $env:LONGEDIT_M1A1_XLSX = $xlsx
    & node (Join-Path $workspace "scripts\capture-post-v115-m1a1-xlsx-validation.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M1A1 desktop capture failed" }
  } finally {
  ...
  if (-not $evidence.differenceResolved) { throw "M1A1 expected/actual difference was not resolved" }
} finally {
...
Write-Output "M1A1 XLSX validation audit completed: $output"
