...
if (!['P1-B4C', 'P1-B4D', 'P1-B5A'].includes(advanced.stage) || !['watermark-workspace-complete', 'watermark-complete', 'metadata-safety-audit-complete'].includes(advanced.status) || !advanced.currentCapabilities?.includes('watermark-copy') || !['workspace-complete-desktop-evidence-pending', 'completed'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.currentWriteUserFile !== true) fail('P1-B4C advanced capability contract is stale')
