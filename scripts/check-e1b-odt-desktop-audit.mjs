...
if ((manifest.producerMatrix || []).join(',') !== 'microsoft-word-16,libreoffice-writer') failures.push('trusted producer matrix is incomplete')
...
if ((manifest.scenarios || []).length !== 4 || manifest.scenarios.some(scenario => scenario.status !== 'passed')) failures.push('four desktop scenarios are incomplete')
for (const id of [
  'trusted-producers-open-read-only',
...
console.log('E1B ODT desktop evidence passed: 2 producers, 2 layouts, 2 themes, search, locator, and immutable sources.')
