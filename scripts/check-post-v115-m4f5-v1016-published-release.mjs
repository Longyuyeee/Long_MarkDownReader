...
const expectedStage = postClosureStage
...
const latestReleasePublished = ['1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.publicVersion) && development.publicTag === `v${development.publicVersion}`
const latestTarget = latestReleasePublished ? `1.0.${Number(development.publicVersion.split('.')[2]) + 1}` : null
const latestRuntimeAccepted = latestReleasePublished && [development.publicVersion, latestTarget].includes(development.runtimeBaseVersion)
if ((!latestReleasePublished && development.publicVersion !== (laterReleasePublished ? '1.0.17' : '1.0.16')) || (latestReleasePublished && !latestRuntimeAccepted) || development.publicTag !== (latestReleasePublished ? `v${development.publicVersion}` : laterReleasePublished ? 'v1.0.17' : policy.tag) || (!latestReleasePublished && development.publicTagCommit !== (laterReleasePublished ? m5Published.candidateSourceCommit : policy.candidateSourceCommit)) || development.developmentTargetVersion !== (latestReleasePublished ? latestTarget : laterReleasePublished ? '1.0.18' : '1.0.17') || development.currentStage !== expectedStage || (postClosureStage && !/^M[678]-[0-9]+-/.test(development.currentStage)) || !(['v1.0.16-public-release-published', 'v1.0.17-quality-gate-pending', 'v1.0.17-candidate-packaged', 'v1.0.17-hosted-lifecycle-passed', 'v1.0.17-release-ready', 'v1.0.17-public-release-published', 'v1.0.17-release-and-managed-updater-closed'].includes(development.binaryVersionTransition) || development.binaryVersionTransition.startsWith('v1.0.18-') || development.binaryVersionTransition.startsWith('v1.0.19-') || development.binaryVersionTransition.startsWith('v1.0.20-') || (development.binaryVersionTransition.startsWith('v1.0.21-') || (development.binaryVersionTransition.startsWith('v1.0.22-') || ['v1.0.23-quality-gate-pending', 'v1.0.23-release-ready', 'v1.0.23-public-release-published', 'v1.0.23-release-and-managed-updater-closed'].includes(development.binaryVersionTransition))))) fail('development/public handoff drifted')
