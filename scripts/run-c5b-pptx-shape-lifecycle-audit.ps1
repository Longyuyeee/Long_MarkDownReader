param(
  [string]$OutputDirectory = "docs\evidence\c5b-pptx-shape-lifecycle"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c5b-pptx-shape-lifecycle"))
if ($output -ne $expectedOutput) { throw "C5B audit output must remain inside docs\evidence\c5b-pptx-shape-lifecycle" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C5B desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c5b-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C5B WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C5B_WPS = $fixture
    $env:LONGEDIT_C5B_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c5b-pptx-shape-lifecycle-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C5B desktop audit capture failed" }
    $artifactDirectory = Join-Path $workspace "fixtures\pptx\output-reopen"
    New-Item -ItemType Directory -Path $artifactDirectory -Force | Out-Null
    foreach ($file in @(
      "c5b-rectangle-copy.pptx",
      "c5b-ellipse-copy.pptx",
      "c5b-line-copy.pptx",
      "c5b-delete-copy.pptx"
    )) {
      Copy-Item -LiteralPath (Join-Path $library $file) -Destination (Join-Path $artifactDirectory $file) -Force
    }
  }
  finally {
...
Write-Output "C5B PPTX shape lifecycle desktop audit completed: $output"
