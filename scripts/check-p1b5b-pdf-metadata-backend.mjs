...
if (!['P1-B5B', 'P1-B5C', 'P1-B5D'].includes(metadata.stage) || !['backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending', 'desktop-and-independent-render-verified'].includes(metadata.status) || metadata.currentWriteCapability !== ['P1-B5C', 'P1-B5D'].includes(metadata.stage)) fail('P1-B5B metadata contract identity is stale')
...
if (!['P1-B5B', 'P1-B5C', 'P1-B5D'].includes(advanced.stage) || !['metadata-backend-complete', 'metadata-workspace-complete', 'metadata-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('metadata-copy') !== ['P1-B5C', 'P1-B5D'].includes(advanced.stage)) fail('P1-B5B public capability boundary is stale')
...
if (metadata.stage === 'P1-B5B') for (const token of ['PdfMetadataPanel', 'preview_pdf_metadata_copy', 'save_pdf_metadata_copy']) if (view.includes(token)) fail(`P1-B5B must keep UI closed until P1-B5C: ${token}`)
