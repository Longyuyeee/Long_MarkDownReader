...
function Wait-ForInstalledProcess([string]$ExecutablePath, [int]$Attempts = 2400) {
    $expectedPath = [IO.Path]::GetFullPath($ExecutablePath)
    for ($attempt = 0; $attempt -lt $Attempts; $attempt += 1) {
        $matches = @(Get-Process -Name "tauri-app" -ErrorAction SilentlyContinue | Where-Object {
            try {
                [IO.Path]::GetFullPath($_.Path).Equals($expectedPath, [StringComparison]::OrdinalIgnoreCase)
            }
            catch {
                $false
            }
        ...
        if ($matches.Count -gt 1) { throw "Managed updater relaunched more than one installed application process." }
        Start-Sleep -Milliseconds 250
    }
    throw "Managed updater installed v$CurrentVersion but did not automatically relaunch LongEdit."
}
...
[IO.File]::WriteAllText($configMarker, '{"stage":"V1.0.8-U1","retain":true}', [Text.UTF8Encoding]::new($false))
$config = [ordered]@{
    libraries = @([ordered]@{
...
try {
    ...
    $checks.Add([ordered]@{ id = "official-v1.0.7-fresh-install"; status = "passed"; version = $PreviousVersion; installerSha256 = $previousInstallerSha256 })
    ...
    $installedBinaryReceipt = [ordered]@{
        schemaVersion = 1
        stage = "V1.0.8-U1-INSTALLED-BINARY"
    ...
    $startedProcess = Wait-ForInstalledProcess -ExecutablePath $mainBinary
    Wait-ForPort -Port 9343 -Listening $true -Process $startedProcess
    $checks.Add([ordered]@{
        id = "automatic-relaunch-after-managed-update"
        status = "passed"
        version = $CurrentVersion
        processId = $startedProcess.Id
        executablePath = $mainBinary
    })
    ...
    $result = [ordered]@{
        ...
        automaticRelaunchAfterInstall = $true
    ...
    Write-Host "v1.0.7 -> v1.0.8 managed updater lifecycle and automatic relaunch passed."
}
finally {
