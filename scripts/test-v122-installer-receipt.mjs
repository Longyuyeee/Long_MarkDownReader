...
import { verifyV122InstallerReceipt } from './lib/v122-installer-receipt.mjs'
...
function fixture() {
  ...
  const previous = { release: { tag: 'v1.0.21', taggedCommit: policy.previousTagCommit }, assets: [{ ...identity, name: 'LongEdit_1.0.21_x64-setup.exe' }] }
  const receipt = { stage: 'M8-15', status: 'hosted-installers-built', candidateSourceCommit: policy.candidateCommit, previousSourceCommit: policy.previousTagCommit, appVersion: '1.0.22', previousVersion: '1.0.21', releaseCandidate: false, sourceUserContentIncluded: false, previousInstallerSource: 'official-github-release', previousInstallerSha256: identity.sha256, artifacts: [{ ...identity, target: 'msi', fileName: 'Long编辑_1.0.22_x64_zh-CN.msi' }, { ...identity, target: 'nsis', fileName: 'Long编辑_1.0.22_x64-setup.exe' }] }
  return { bytes, policy, previous, receipt }
}
test('verified bytes do not promote lifecycle or search gates', () => {
  const f = fixture()
  const result = verifyV122InstallerReceipt(f.receipt, f.policy, f.previous, () => f.bytes)
  assert.equal(result.installersVerified, 3)
  assert.equal(result.installedLifecycleAccepted, false)
  assert.equal(result.searchAccepted, false)
})
for (const scenario of ['wrong-candidate', 'rebuilt-previous', 'wrong-previous-hash', 'duplicate-target', 'path-traversal', 'wrong-version', 'missing-privacy', 'changed-bytes', 'wrong-size']) {
  test(`reject ${scenario}`, () => {
    ...
    if (scenario === 'path-traversal') f.receipt.artifacts[0].fileName = '../Long编辑_1.0.22_x64_zh-CN.msi'
    if (scenario === 'wrong-version') f.receipt.appVersion = '1.0.21'
    ...
    assert.throws(() => verifyV122InstallerReceipt(f.receipt, f.policy, f.previous, () => f.bytes))
  })
}
