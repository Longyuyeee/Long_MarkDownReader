...
for (const token of ['data-testid="knowledge-network-pulse"', 'data-testid="knowledge-network-coverage"', 'data-testid="knowledge-network-topic"', ':data-node-id="node.id"', "watch(() => props.open, value => { if (value) loadReport() }, { immediate: true })"]) requireText(graphPanel, token, `G11 graph governance selector missing: ${token}`)
for (const token of ['data-testid="graph-selected-node"', "router.replace({ name: 'Graph', query: { ...route.query, root: node.id } })"]) requireText(graph, token, `G11 centered graph marker missing: ${token}`)
