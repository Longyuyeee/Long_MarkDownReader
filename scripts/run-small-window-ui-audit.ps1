...
$cdpPort = 14526
if (Get-NetTCPConnection -LocalPort $cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "Small-window UI audit requires free CDP port $cdpPort" }
$auditRoot = Join-Path $env:TEMP ("longedit-small-window-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    Start-Process -FilePath $edge -ArgumentList @(
      ...
      "--user-data-dir=$webviewData", "--window-size=900,720", "about:blank"
    ) -WindowStyle Hidden | Out-Null
    Wait-Port $cdpPort $true
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"
    & node (Join-Path $workspace "scripts\capture-small-window-ui-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "Small-window UI audit failed" }
  } finally {
...
Write-Output "Small-window UI audit completed."
