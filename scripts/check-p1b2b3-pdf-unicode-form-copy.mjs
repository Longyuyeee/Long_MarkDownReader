...
for (const token of ['支持中文', '内置字体覆盖', '源 PDF 和已有文件不会覆盖']) if (!panel.includes(token)) fail(`P1-B2B3 UI boundary missing: ${token}`)
if (contract.stage !== 'P1-B2B3' || contract.status !== 'unicode-text-form-copy-complete') fail('P1-B2B3 contract is stale')
