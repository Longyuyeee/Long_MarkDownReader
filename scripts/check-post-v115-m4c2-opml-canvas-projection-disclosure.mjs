...
if (policy.selectedNextStage?.id !== successor.stage || successor.predecessor !== policy.stage || successor.selection.id !== projectNote.stage || projectNote.predecessor !== successor.stage || projectNote.selectedNextStage?.id !== graphCanvas.stage || graphCanvas.predecessor !== projectNote.stage || development.currentStage !== `${graphCanvas.selectedNextStage.id}-${graphCanvas.selectedNextStage.name}`) failures.push('M4C successor handoff is not aligned')
