...
if (cargo.includes('jsonschema')) fail('M7-0 must not install a schema validator before feasibility acceptance')
...
if (development.runtimeBaseVersion !== '1.0.18' || development.publicVersion !== '1.0.18' || development.developmentTargetVersion !== '1.0.19' || development.currentStage !== 'M7-1-local-json-schema-sidecar-provider-mapping-feasibility-audit' || development.binaryVersionTransition !== 'v1.0.18-release-and-managed-updater-closed' || development.releaseCandidate) fail('M7-1 development handoff drift')
for (const [document, tokens] of [[audit, ['最初需求', '本地 JSON Schema sidecar', '未配置 Schema 时不制造错误', 'M7-1']], [roadmap, ['M7-0', 'M7-1', 'JSON/JSONC', '联网']], [alignment, ['当前阶段：**M7-1 本地 JSON Schema sidecar provider/mapping 可行性审计**', 'M7-0 已完成']]]) for (const token of tokens) if (!document.includes(token)) fail(`M7-0 document missing: ${token}`)
