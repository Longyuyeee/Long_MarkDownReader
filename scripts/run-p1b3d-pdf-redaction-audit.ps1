...
$output = Join-Path $workspace "docs\evidence\p1b3d-pdf-redaction"
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\p1b3d-pdf-redaction"))
if ([IO.Path]::GetFullPath($output) -ne $expectedOutput) { throw "P1-B3D evidence path mismatch" }
if (Test-Path -LiteralPath $output) {
  if (-not $Refresh) { throw "P1-B3D accepted evidence already exists; pass -Refresh to replace it" }
...
$appPort = 14200
$cdpPort = 14519
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "P1-B3D requires free ports $appPort and $cdpPort" }
...
if ($LASTEXITCODE -ne 0) { throw "P1-B3D custom-protocol build failed" }

$auditRoot = Join-Path $env:TEMP ("longedit-p1b3d-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$source = Join-Path $library "P1B3D Redaction Evidence.pdf"
& node (Join-Path $workspace "scripts\create-p1b3d-pdf-redaction-fixture.mjs") $source
if ($LASTEXITCODE -ne 0) { throw "P1-B3D fixture generation failed" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_P1B3D_OUTPUT=$output
    $env:LONGEDIT_P1B3D_LIBRARY=$library
    $env:LONGEDIT_P1B3D_SOURCE=$source
    $env:LONGEDIT_P1B3D_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-p1b3d-pdf-redaction-evidence.mjs")
    if ($LASTEXITCODE -ne 0) { throw "P1-B3D desktop capture failed" }
    $targetFile = Get-ChildItem -LiteralPath $library -Filter "*.pdf" | Where-Object { $_.FullName -ne $source } | Select-Object -First 1
    if (-not $targetFile) { throw "P1-B3D target PDF was not created" }
    $target = $targetFile.FullName
    if ($sourceHash -ne (Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash) { throw "P1-B3D source fixture changed" }
    ...
    if (-not (Test-Path -LiteralPath $pdftoppm)) { throw "P1-B3D Poppler renderer is unavailable" }
    & $pdftoppm -f 1 -l 1 -singlefile -png -r 144 $source (Join-Path $output "poppler-source-page-1")
    if ($LASTEXITCODE -ne 0) { throw "P1-B3D source Poppler render failed" }
    & $pdftoppm -f 1 -l 1 -singlefile -png -r 144 $target (Join-Path $output "poppler-target-page-1")
    if ($LASTEXITCODE -ne 0) { throw "P1-B3D target Poppler render failed" }

    $python="C:\Users\Administrator\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe"
    & $python (Join-Path $workspace "scripts\verify-p1b3d-pdf-redaction.py") $source $target (Join-Path $output "poppler-source-page-1.png") (Join-Path $output "poppler-target-page-1.png") (Join-Path $output "independent-verification.json")
    if ($LASTEXITCODE -ne 0) { throw "P1-B3D independent verification failed" }
    & node (Join-Path $workspace "scripts\finalize-p1b3d-pdf-redaction-evidence.mjs") $output
    if ($LASTEXITCODE -ne 0) { throw "P1-B3D evidence manifest failed" }
  } finally {
...
Write-Output "P1-B3D PDF redaction audit completed: $output"
