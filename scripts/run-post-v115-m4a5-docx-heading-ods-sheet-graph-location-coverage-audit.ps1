param([string]$OutputDirectory = "docs\evidence\post-v115-m4a5-docx-heading-ods-sheet-graph-location-coverage")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4a5-docx-heading-ods-sheet-graph-location-coverage"))
if ($output -ne $expectedOutput) { throw "M4A-5 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14538
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4A-5 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4a5-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
Copy-Item -LiteralPath (Join-Path $workspace "fixtures\docx\producers\microsoft-word-16.docx") -Destination (Join-Path $library "M4A5 Word.docx")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\odf-content\longedit-e1c-spreadsheet.ods") -Destination (Join-Path $library "M4A5 Sheet.ods")
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4A5_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4A5_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4A5_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4a5-docx-heading-ods-sheet-graph-location-coverage-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4A-5 desktop capture failed" }
...
Write-Output "M4A-5 DOCX heading/ODS sheet graph location audit completed: $output"
