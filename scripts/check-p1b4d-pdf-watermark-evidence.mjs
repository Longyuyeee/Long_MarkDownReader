...
if (!['P1-B4D', 'P1-B5A'].includes(advanced.stage) || !['watermark-complete', 'metadata-safety-audit-complete'].includes(advanced.status) || !advanced.currentCapabilities?.includes('watermark-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status !== 'completed') fail('P1-B4D advanced capability contract is stale')
