...
if (advanced.stage !== 'P1-B4D' || advanced.status !== 'watermark-complete' || !advanced.currentCapabilities?.includes('watermark-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status !== 'completed') fail('P1-B4D advanced capability contract is stale')
