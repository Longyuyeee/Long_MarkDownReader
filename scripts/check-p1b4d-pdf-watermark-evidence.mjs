...
if (!['P1-B4D', 'P1-B5A', 'P1-B5B', 'P1-B5C', 'P1-B5D'].includes(advanced.stage) || !['watermark-complete', 'metadata-safety-audit-complete', 'metadata-backend-complete', 'metadata-workspace-complete', 'metadata-complete'].includes(advanced.status) || !advanced.currentCapabilities?.includes('watermark-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status !== 'completed') fail('P1-B4D advanced capability contract is stale')
