param(
  [string]$OutputDirectory = "docs\evidence\a3r-json-creation"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\a3r-json-creation"))
if ($output -ne $expectedOutput) { throw "A3R audit output must remain inside docs\evidence\a3r-json-creation" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "A3R desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-a3r-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_A3R_AUDIT_LIBRARY = $library
    $env:LONGEDIT_A3R_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-a3r-json-creation-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "A3R desktop audit capture failed" }
  }
  finally {
...
Write-Output "A3R JSON creation desktop audit completed: $output"
