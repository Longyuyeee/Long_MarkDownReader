...
if (policy.selectedNextStage?.id !== successor.stage || successor.predecessor !== policy.stage || successor.selectedNextStage?.id !== capabilityDecision.stage || capabilityDecision.predecessor !== successor.stage || capabilityDecision.selectedNextStage?.id !== freezeEntry.stage || freezeEntry.predecessor !== capabilityDecision.stage || freezeEntry.selectedNextStage?.id !== transition.stage || transition.predecessor !== freezeEntry.stage || development.currentStage !== `${transition.selectedNextStage.id}-${transition.selectedNextStage.name}`) failures.push('M4 successor handoff is not aligned')
