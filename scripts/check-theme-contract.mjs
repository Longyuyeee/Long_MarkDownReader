...
requireMatch(graph, /if \(initialNode && \(requestedRoot \|\| selectedNode\.value \|\| route\.query\.mode === 'mindmap'\)\)/, 'Knowledge graph must not force initial details in normal network mode')
requireMatch(graph, /data-testid="graph-tools-entry"[\s\S]*?data-testid="graph-tools-menu"/, 'Knowledge graph compact toolbar contract is missing')
