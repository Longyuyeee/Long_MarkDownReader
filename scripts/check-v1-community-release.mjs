...
const expectedReleaseUrl = `https://github.com/Longyuyeee/Long_MarkDownReader/releases/tag/${expectedTag}`;
const versionPattern = /^1\.\d+\.\d+$/;

if (!versionPattern.test(pkg.version) || tauri.version !== pkg.version || !cargo.includes(`version = "${pkg.version}"`)) fail("v1 version identity drift");
...
if (!readme.includes(expectedTag) || !readme.includes("未知发布者") || !readme.includes("SHA-256") || !readme.includes("每 24 小时")) fail("README release disclosure drift");
...
if (policy.gates.githubReleasePublished === true) {
  if (policy.gates.qualityGatePassed !== true || policy.releaseCandidate !== true || policy.currentStatus !== `${expectedTag}-community-release-published`) fail("published V1 state is inconsistent");
  if (policy.release?.tag !== expectedTag || !/^[0-9a-f]{40}$/.test(policy.release?.taggedCommit ?? "") || !Number.isInteger(policy.release?.qualityGateRunId) || policy.release?.url !== expectedReleaseUrl) fail("published V1 release receipt drift");
} else if (policy.gates.qualityGatePassed === true) {
  if (policy.releaseCandidate !== true || policy.currentStatus !== `${expectedTag}-community-release-ready-to-publish`) fail("ready-to-publish V1 state is inconsistent");
...
const versionSlug = pkg.version.replaceAll(".", "_");
for (const path of [`docs/RELEASE_NOTES_v${pkg.version}.md`, `docs/V${versionSlug}_Unsigned_Community_Release_Audit_2026-08-02.md`]) if (!fs.existsSync(path)) fail(`release document missing: ${path}`);

if (failures.length) {
  console.error(failures.map((message) => `- ${message}`).join("\n"));
  process.exit(1);
}
console.log(`V1 community release contract passed: ${pkg.version}, unsigned disclosure, updater integrity, and installed lifecycle evidence are aligned.`);
