...
if (!['P1-B3C', 'P1-B3D', 'P1-B4A'].includes(advanced.stage) || !['permanent-redaction-workspace-complete', 'permanent-redaction-complete', 'watermark-safety-audit-complete'].includes(advanced.status) || !advanced.currentCapabilities?.includes('permanent-redaction-copy') || !['workspace-complete-desktop-evidence-pending', 'completed'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B3')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B3')?.currentWriteUserFile !== true) fail('P1-B3C advanced capability contract is stale')
