...
if (safety.stage !== 'P1-B3C' || safety.status !== 'workspace-complete-desktop-evidence-pending' || safety.currentWriteCapability !== true || safety.implementationSlices?.find(item => item.id === 'P1-B3C')?.status !== 'completed') fail('P1-B3C safety contract is stale')
if (advanced.stage !== 'P1-B3C' || advanced.status !== 'permanent-redaction-workspace-complete' || !advanced.currentCapabilities?.includes('permanent-redaction-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B3')?.status !== 'workspace-complete-desktop-evidence-pending' || advanced.plannedSlices?.find(item => item.id === 'P1-B3')?.currentWriteUserFile !== true) fail('P1-B3C advanced capability contract is stale')
