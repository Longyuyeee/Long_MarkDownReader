param([string]$OutputDirectory = "docs\evidence\post-v115-m4c3-graph-derived-output-disclosure-selection")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4c3-graph-derived-output-disclosure-selection"))
if ($output -ne $expectedOutput) { throw "M4C-3 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14532
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4C-3 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4c3-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4C3_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4C3_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4C3_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4c3-graph-derived-output-disclosure-selection-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4C-3 desktop capture failed" }
...
Write-Output "M4C-3 graph output selection audit completed: $output"
