...
for (const extension of ['.mp4', '.webm', '.ogv', '.m4v']) if (!video.extensions.includes(extension)) fail(`Video extension missing: ${extension}`)
...
requireTokens(read('src/views/MediaViewerView.vue'), 'Media workspace', [
  "invoke<MediaInspection>('inspect_media_file'",
  'await readFile(inspected.path)',
  'URL.createObjectURL',
  'URL.revokeObjectURL',
  'requestFullscreen()',
  'fitImage',
...
console.log('Media workspace contract passed: 8 image and 4 video extensions use guarded, read-only in-library previews with bounded decoding.')
