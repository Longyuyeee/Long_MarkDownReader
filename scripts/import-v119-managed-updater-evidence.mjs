...
if (!artifactRootArg || !runIdArg || !artifactIdArg || !artifactDigestArg || !headCommitArg || !importedAtArg) throw new Error('Usage: node scripts/import-v119-managed-updater-evidence.mjs <artifact-root> <run-id> <artifact-id> <artifact-digest> <head-commit> <imported-at>')
const sourceRoot = path.resolve(artifactRootArg)
const destinationRoot = path.resolve('docs/evidence/v1.0.19-managed-updater')
...
if (!discovery.release?.releaseNotes?.includes('v1.0.19 聚焦 v1.0.18 之后') || !discovery.release?.releaseNotes?.includes('已通过本地 WebView2') || discovery.confirmation?.installerStartedBeforeConfirmation !== false) throw new Error('Official published release messaging or confirmation boundary is missing')
