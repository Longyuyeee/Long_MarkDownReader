...
replaceExact('package.json', /"version": "1\.0\.19"/g, '"version": "1.0.20"', 1)
replaceExact('package-lock.json', /"version": "1\.0\.19"/g, '"version": "1.0.20"', 2)
replaceExact('src-tauri/tauri.conf.json', /"version": "1\.0\.19"/g, '"version": "1.0.20"', 1)
replaceExact('src-tauri/Cargo.toml', /^version = "1\.0\.19"$/gm, 'version = "1.0.20"', 1)
replaceExact('src-tauri/Cargo.lock', /(name = "tauri-app"\r?\nversion = ")1\.0\.19("\r?\n)/g, '$11.0.20$2', 1)

for (const file of activeSharedFiles) {
  if (json(file).appVersion !== '1.0.19') throw new Error(`${file}: expected appVersion 1.0.19`)
  replaceExact(file, /"appVersion": "1\.0\.19"/g, '"appVersion": "1.0.20"', 1)
}
replaceExact('shared/p1-final-capability-closure.json', /"nextStage": "V1\.0\.19-UNSIGNED-PATCH-RELEASE"/g, '"nextStage": "V1.0.20-UNSIGNED-PATCH-RELEASE"', 1)

const development = json('shared/development-version-policy.json')
Object.assign(development, {
  runtimeBaseVersion: '1.0.20',
  releaseCandidate: false,
  currentStage: 'M8-4-v1.0.20-atomic-version-transition-and-candidate-packaging',
  binaryVersionTransition: 'v1.0.20-quality-gate-pending',
  displayLabel: 'v1.0.20 候选质量门与打包中 · 当前公开 v1.0.19 · M8-4',
})
...
Object.assign(community, {
  ...
  currentStatus: 'v1.0.20-community-release-quality-gate-pending',
  gates: {
  ...
  patchValidation: {
    ...
    managedUpdaterUpgradePath: '1.0.19-to-1.0.20-pending',
    scope: 'post-v1.0.19-knowledge-graph-usability-performance-and-visual-closure',
  },
  targetRelease: {
    tag: 'v1.0.20',
    url: 'https://github.com/Longyuyeee/Long_MarkDownReader/releases/tag/v1.0.20',
  ...
  releaseWarnings: [
    ...
    'v1.0.19-to-v1.0.20-managed-update-observation-pending',
    'download-only-from-official-github-release',
    'verify-published-sha256-before-install',
  ],
  nextAction: 'pass-v1.0.20-quality-gate-and-build-real-candidate-installers',
})
write('shared/v1-community-release-policy.json', `${JSON.stringify(community, null, 2)}\n`)

console.log(`M8-4 atomic transition applied: package/Cargo/Tauri, ${activeSharedFiles.length} active shared contracts and candidate policies identify v1.0.20; public v1.0.19 facts remain frozen.`)
