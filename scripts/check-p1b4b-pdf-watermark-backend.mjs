...
if (view.includes('data-testid="p1b4c-pdf-watermark"') || view.includes('previewPdfWatermarkCopy') || view.includes('savePdfWatermarkCopy')) fail('P1-B4B must not expose the P1-B4C workspace prematurely')
if (watermark.stage !== 'P1-B4B' || watermark.status !== 'isolated-vector-backend-complete-workspace-pending' || watermark.currentWriteCapability !== false || watermark.backendWriteCapability !== true || watermark.implementationSlices?.find(item => item.id === 'P1-B4B')?.status !== 'completed' || watermark.implementationSlices?.find(item => item.id === 'P1-B4C')?.status !== 'planned') fail('P1-B4B watermark contract is stale')
if (advanced.stage !== 'P1-B4B' || advanced.status !== 'watermark-backend-complete' || advanced.currentCapabilities?.includes('watermark-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status !== 'isolated-vector-backend-complete-workspace-pending' || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.backendWriteUserFile !== true || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.currentWriteUserFile !== false) fail('P1-B4B advanced capability boundary is stale')
