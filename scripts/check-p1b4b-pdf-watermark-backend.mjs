...
if (!['P1-B4B', 'P1-B4C', 'P1-B4D'].includes(advanced.stage) || !['watermark-backend-complete', 'watermark-workspace-complete', 'watermark-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('watermark-copy') !== ['P1-B4C', 'P1-B4D'].includes(advanced.stage) || !['isolated-vector-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending', 'completed'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.backendWriteUserFile !== true || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.currentWriteUserFile !== ['P1-B4C', 'P1-B4D'].includes(advanced.stage)) fail('P1-B4B advanced capability boundary is stale')
