...
if (!['P1-B4B', 'P1-B4C'].includes(watermark.stage) || !['isolated-vector-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending'].includes(watermark.status) || watermark.currentWriteCapability !== ['P1-B4C'].includes(watermark.stage) || watermark.backendWriteCapability !== true || watermark.implementationSlices?.find(item => item.id === 'P1-B4B')?.status !== 'completed' || !['planned', 'completed'].includes(watermark.implementationSlices?.find(item => item.id === 'P1-B4C')?.status)) fail('P1-B4B watermark contract is stale')
if (!['P1-B4B', 'P1-B4C'].includes(advanced.stage) || !['watermark-backend-complete', 'watermark-workspace-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('watermark-copy') !== ['P1-B4C'].includes(advanced.stage) || !['isolated-vector-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.backendWriteUserFile !== true || advanced.plannedSlices?.find(item => item.id === 'P1-B4')?.currentWriteUserFile !== ['P1-B4C'].includes(advanced.stage)) fail('P1-B4B advanced capability boundary is stale')
