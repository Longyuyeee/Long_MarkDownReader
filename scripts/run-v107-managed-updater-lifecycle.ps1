...
[IO.File]::WriteAllText($configMarker, '{"stage":"V1.0.7-U1","retain":true}', [Text.UTF8Encoding]::new($false))
$config = [ordered]@{
    libraries = @([ordered]@{
...
try {
    ...
    $checks.Add([ordered]@{ id = "official-v1.0.6-fresh-install"; status = "passed"; version = $PreviousVersion; installerSha256 = $previousInstallerSha256 })
    ...
    $installedBinaryReceipt = [ordered]@{
        schemaVersion = 1
        stage = "V1.0.7-U1-INSTALLED-BINARY"
    ...
    Write-Host "v1.0.6 -> v1.0.7 managed updater lifecycle passed."
}
finally {
