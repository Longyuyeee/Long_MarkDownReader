...
if (!artifactRootArg || !runIdArg || !artifactIdArg || !artifactDigestArg || !headCommitArg || !importedAtArg) throw new Error('Usage: node scripts/import-v118-managed-updater-evidence.mjs <artifact-root> <run-id> <artifact-id> <artifact-digest> <head-commit> <imported-at>')
const sourceRoot = path.resolve(artifactRootArg)
const destinationRoot = path.resolve('docs/evidence/v1.0.18-managed-updater')
