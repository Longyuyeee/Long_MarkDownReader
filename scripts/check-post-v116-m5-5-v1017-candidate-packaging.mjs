...
const laterCandidateActive = ['1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(pkg.version) && community.appVersion === pkg.version && /^M[678]-[0-9]+-/.test(development.currentStage)
...
const currentLaterReleasePublished = laterCandidateActive && development.publicVersion === pkg.version
...
const expectedLaterPublicVersion = currentLaterReleasePublished ? pkg.version : `1.0.${Number(pkg.version.split('.')[2]) - 1}`
const expectedLaterTargetVersion = currentLaterReleasePublished ? `1.0.${Number(pkg.version.split('.')[2]) + 1}` : pkg.version
if (!['1.0.17', '1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.runtimeBaseVersion)
  || development.publicVersion !== (laterCandidateActive ? expectedLaterPublicVersion : laterPublicActive ? development.runtimeBaseVersion : releasePublished ? '1.0.17' : '1.0.16')
  || development.publicTag !== (laterCandidateActive ? `v${expectedLaterPublicVersion}` : laterPublicActive ? `v${development.publicVersion}` : releasePublished ? 'v1.0.17' : 'v1.0.16')
  || development.developmentTargetVersion !== (laterCandidateActive ? expectedLaterTargetVersion : laterPublicActive ? `1.0.${Number(development.publicVersion.split('.')[2]) + 1}` : releasePublished ? '1.0.18' : '1.0.17')) fail('development public/candidate split drifted')
...
if (policy.status === 'atomic-transition-complete-package-pending') {
  ...
  if (!/^[0-9a-f]{40}$/.test(policy.candidateSourceCommit ?? '') || !policy.qualityGatePassed || !policy.candidatePackageBuilt
    ...
    || !(releaseClosed ? /^M[678]-[0-9]+-/.test(development.currentStage) : expectedStages.includes(development.currentStage))
