...
if (!['1.0.17', '1.0.18'].includes(development.runtimeBaseVersion)
  || development.publicVersion !== (releasePublished ? '1.0.17' : '1.0.16')
  || development.publicTag !== (releasePublished ? 'v1.0.17' : 'v1.0.16')
  || development.developmentTargetVersion !== (releasePublished ? '1.0.18' : '1.0.17')) fail('development public/candidate split drifted')
...
if (policy.status === 'atomic-transition-complete-package-pending') {
  ...
  const expectedTransitions = releaseClosed ? ['v1.0.17-release-and-managed-updater-closed', 'v1.0.18-quality-gate-pending', 'v1.0.18-candidate-packaged'] : [releasePublished ? 'v1.0.17-public-release-published' : releaseReady ? 'v1.0.17-release-ready' : successorPassed ? 'v1.0.17-hosted-lifecycle-passed' : 'v1.0.17-candidate-packaged']
