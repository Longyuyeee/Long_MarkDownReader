...
if (mode === 'discover-install') {
  ...
  const evidence = {
    schemaVersion: 1,
    stage: 'V1.0.8-U1-DISCOVERY',
    capturedAt: new Date().toISOString(),
    status: 'passed',
    environment: 'GitHub-hosted disposable Windows installed official v1.0.7',
    release: info,
    confirmation: {
      ...surface,
  ...
  await writeEvidence('managed-updater-post-upgrade-evidence.json', {
    schemaVersion: 1,
    stage: 'V1.0.8-U1-POST-UPGRADE',
    capturedAt: new Date().toISOString(),
    status: 'passed',
    environment: 'GitHub-hosted disposable Windows updated through the installed v1.0.7 client',
    release: info,
