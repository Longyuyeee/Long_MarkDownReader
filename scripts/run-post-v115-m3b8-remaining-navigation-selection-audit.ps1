...
try {
  $arguments = @{ Stage = 'M3B8'; Theme = 'dark'; Motion = 'reduced' }
  if ($SkipBuild) { $arguments.SkipBuild = $true }
  & $runner @arguments
  if ($LASTEXITCODE -ne 0) { throw 'M3B-8 real desktop audit failed' }
  Write-Output 'M3B-8 dark/reduced real desktop selection audit completed'
} finally {
