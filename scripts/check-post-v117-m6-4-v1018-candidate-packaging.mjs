...
const laterCandidateActive = ['1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(pkg.version) && community.appVersion === pkg.version && /^M[78]-(?:[4-9]|[1-9]\d)-/.test(development.currentStage)
...
const laterPublicActive = ['1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.publicVersion) && development.publicTag === `v${development.publicVersion}`
const laterDevelopmentTarget = laterPublicActive ? `1.0.${Number(development.publicVersion.split('.')[2]) + 1}` : null
if (!['1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.runtimeBaseVersion) || development.publicVersion !== (laterPublicActive ? development.publicVersion : releasePublished ? '1.0.18' : '1.0.17')
  || development.publicTag !== (laterPublicActive ? `v${development.publicVersion}` : releasePublished ? 'v1.0.18' : 'v1.0.17')
  || development.developmentTargetVersion !== (laterPublicActive ? laterDevelopmentTarget : releasePublished ? '1.0.19' : '1.0.18') || development.releaseCandidate) fail('development candidate/public split drift')
