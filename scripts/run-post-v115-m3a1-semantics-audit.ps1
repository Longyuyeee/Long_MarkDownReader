param([switch]$SkipBuild,[ValidateSet('M3A1','M3A2','M3A3','M3A4','M3A5')][string]$Stage = 'M3A1')
$ErrorActionPreference = 'Stop'
$workspace = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$outputRelative = if ($Stage -eq 'M3A5') { 'docs\evidence\post-v115-m3a5-community' } elseif ($Stage -eq 'M3A4') { 'docs\evidence\post-v115-m3a4-relation-evidence' } elseif ($Stage -eq 'M3A3') { 'docs\evidence\post-v115-m3a3-shortest-path' } elseif ($Stage -eq 'M3A2') { 'docs\evidence\post-v115-m3a2-neighbor-focus' } else { 'docs\evidence\post-v115-m3a1-semantics' }
...
try {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'M3A-1 desktop capture failed' }
  } finally {
...
$checkRelative = if ($Stage -eq 'M3A5') { 'scripts\check-post-v115-m3a5-community.mjs' } elseif ($Stage -eq 'M3A4') { 'scripts\check-post-v115-m3a4-relation-evidence.mjs' } elseif ($Stage -eq 'M3A3') { 'scripts\check-post-v115-m3a3-shortest-path.mjs' } elseif ($Stage -eq 'M3A2') { 'scripts\check-post-v115-m3a2-neighbor-focus.mjs' } else { 'scripts\check-post-v115-m3a1-semantics.mjs' }
