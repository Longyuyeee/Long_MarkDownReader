...
if (!artifactRootArg) throw new Error('Usage: node scripts/verify-post-v116-m5-7-v1017-real-release-assets.mjs <downloaded-artifact-root>')
const root = path.resolve(artifactRootArg)
const policy = JSON.parse(fs.readFileSync('shared/post-v116-m5-7-v1017-final-artifact-manifest-release-readiness-policy.json', 'utf8'))
const manifest = JSON.parse(fs.readFileSync('docs/evidence/v1.0.17-release/artifact-manifest.json', 'utf8'))
...
const output = path.resolve('.release-secrets/release-v1.0.17')
...
if (checksumBytes.length !== policy.checksumFile.sizeBytes || sha256(checksumBytes) !== policy.checksumFile.sha256 || !fs.readFileSync('docs/evidence/v1.0.17-release/SHA256SUMS.txt').equals(checksumBytes)) throw new Error('SHA256SUMS mismatch')
fs.writeFileSync(path.join(output, 'SHA256SUMS.txt'), checksumBytes)
console.log(`M5-7 real release assets verified and prepared at ${output}`)
