...
const policy = json('shared/v109-managed-updater-lifecycle-policy.json')
const previousReceipt = json('docs/evidence/v1.0.8-release/release-receipt.json')
const currentReceipt = json('docs/evidence/v1.0.9-release/release-receipt.json')
const currentManifest = json('docs/evidence/v1.0.9-release/artifact-manifest.json')
const workflow = read('.github/workflows/v109-managed-updater-lifecycle.yml')
...
const audit = read('docs/V1_0_9_Managed_Updater_Lifecycle_Audit_2026-08-11.md')

if (policy.schemaVersion !== 1 || policy.stage !== 'V1.0.9-U1'
  || !['hosted-execution-pending', 'hosted-managed-update-passed'].includes(policy.status)) fail('v1.0.9 updater policy identity drift')
...
if (policy.releases?.previous?.version !== '1.0.8'
...
if (policy.releases?.current?.version !== '1.0.9'
...
requireTokens(workflow, [
  ...
  'shared\\v109-managed-updater-lifecycle-policy.json',
  ...
  'v109-managed-updater-lifecycle-${{ github.run_id }}',
], 'v1.0.9 updater workflow')
...
], 'v1.0.9 updater probe')
for (const token of ['v1.0.8', 'v1.0.9', '用户确认', 'SHA-256', '自动重启', '资料保留', '托管 Windows']) {
  if (!audit.includes(token)) fail(`v1.0.9 updater audit missing: ${token}`)
}
...
if (!completed) {
  ...
  if (Object.values(policy.gates ?? {}).some(value => value !== true)
    ...
    || policy.nextAction !== 'v1.0.9-release-closure-complete') fail('completed updater state drift')
  const root = policy.evidenceRoot
  const importPath = `${root}/import-manifest.json`
  if (!fs.existsSync(importPath)) fail('v1.0.9 updater evidence is missing')
  else {
    const manifest = json(importPath)
    if (manifest.stage !== 'V1.0.9-U1I'
      || manifest.status !== 'accepted'
      || manifest.githubRunId !== policy.githubRun.id
      || manifest.headCommit !== policy.githubRun.headCommit
      || manifest.previousVersion !== '1.0.8'
      || manifest.currentVersion !== '1.0.9'
      ...
      || manifest.files.filter(file => file.path.endsWith('.jpg')).some(file => file.visuallyReviewed !== true)) fail('v1.0.9 updater import drift')
    for (const file of manifest.files ?? []) {
      const evidencePath = `${root}/${file.path}`
      if (!fs.existsSync(evidencePath)) fail(`v1.0.9 updater evidence missing: ${file.path}`)
      else if (fs.statSync(evidencePath).size !== file.bytes || sha256(evidencePath) !== file.sha256) fail(`v1.0.9 updater evidence hash drift: ${file.path}`)
    }
    ...
    if (fs.existsSync(lifecyclePath)) {
      ...
      if (lifecycle.status !== 'passed'
        || lifecycle.previousVersion !== '1.0.8'
        || lifecycle.currentVersion !== '1.0.9'
        ...
        || lifecycle.sourceUserContentIncluded !== false) fail('v1.0.9 updater lifecycle drift')
    }
  }
}
...
console.log(`V1.0.9 managed updater lifecycle contract passed: ${completed ? 'hosted 1.0.8 -> 1.0.9 evidence accepted' : 'safe hosted execution harness is ready'}.`)
