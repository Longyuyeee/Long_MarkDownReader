...
if (policy.selectedNextStage?.id !== 'M4F-1' || development.currentStage !== `${policy.selectedNextStage.id}-${policy.selectedNextStage.name}`) failures.push('M4F-1 handoff is not aligned')
if (policy.releaseCandidate !== false || evidence.releaseCandidate !== false || development.releaseCandidate !== false || development.runtimeBaseVersion !== '1.0.15') failures.push('release boundary changed before atomic transition')
