param([string]$OutputDirectory = "docs\evidence\post-v115-m1a2-xlsx-scale")

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m1a2-xlsx-scale"))
if ($output -ne $expectedOutput) { throw "M1A2 output must remain inside $expectedOutput" }
$appPort = 14200
$cdpPort = 14413
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1A2 requires free ports $appPort and $cdpPort" }
$sourceCommit = (& git -C $workspace rev-parse HEAD).Trim()
$policy = Get-Content -LiteralPath (Join-Path $workspace "shared\post-v115-m1a2-xlsx-scale-policy.json") -Raw | ConvertFrom-Json
...
if ($LASTEXITCODE -ne 0) { throw "M1A2 Rust build failed" }

$auditRoot = Join-Path $env:TEMP ("longedit-m1a2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
foreach ($tier in $policy.tiers) {
  $name = "M1A2-$($tier.cells)-cells.xlsx"
  ...
  $files += @{ name=$name; path=$path; cells=$tier.cells; rows=$tier.rows; columns=$tier.columns; maximumOpenMs=$tier.maximumOpenMs; maximumBottomPageMs=$tier.maximumBottomPageMs; maximumSaveMs=$tier.maximumSaveMs }
}
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_M1A2_AUDIT_OUTPUT = $output
    $env:LONGEDIT_M1A2_SOURCE_COMMIT = $sourceCommit
    $env:LONGEDIT_M1A2_FILES = ($files | ConvertTo-Json -Compress)
    & node (Join-Path $workspace "scripts\capture-post-v115-m1a2-xlsx-scale.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M1A2 desktop capture failed" }
  } finally {
...
Write-Output "M1A2 XLSX scale audit completed: $output"
