param([string]$OutputDirectory = "docs\evidence\post-v115-m4b2-workspace-object-action-exit-audit")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4b2-workspace-object-action-exit-audit"))
if ($output -ne $expectedOutput) { throw "M4B-2 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14532
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4B-2 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4b2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$pdfPath = Join-Path $library "Workspace Evidence.pdf"
& node (Join-Path $workspace "scripts\create-p1b2b5-pdf-radio-fixture.mjs") $pdfPath | Out-Null
if ($LASTEXITCODE -ne 0) { throw "Unable to create M4B-2 PDF fixture" }
$annotationPath = "$pdfPath.annotations.json"
$annotation = @{
  schemaVersion = 1
  source = @{ pdfFile = "Workspace Evidence.pdf"; size = (Get-Item -LiteralPath $pdfPath).Length; modifiedAt = 1 }
  annotations = @(@{ id = "workspace-review"; kind = "comment"; page = 1; color = "blue"; rects = @(); quote = ""; comment = "Review Workspace PDF annotation entry"; createdAt = 1; updatedAt = 1 })
} | ConvertTo-Json -Depth 8
[IO.File]::WriteAllText($annotationPath, "$annotation`n", [Text.UTF8Encoding]::new($false))
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4B2_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4B2_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4B2_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4b2-workspace-object-action-exit-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4B-2 desktop capture failed" }
...
Write-Output "M4B-2 Workspace object action exit audit completed: $output"
