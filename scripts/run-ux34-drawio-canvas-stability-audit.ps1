param(
  [string]$OutputDirectory = "docs\evidence\ux34-drawio-canvas-stability"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\ux34-drawio-canvas-stability"))
if ($output -ne $expectedOutput) { throw "UX-34 audit output must remain inside $expectedOutput" }

$appPort = 14200
$cdpPort = 14340
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) {
  throw "UX-34 desktop audit requires free local ports $appPort and $cdpPort"
}
...
$auditRoot = Join-Path $env:TEMP ("longedit-ux34-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$canvas = Join-Path $library "UX34 Stability.canvas"
$drawio = Join-Path $library "UX34 Stability.drawio"
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\canvas\valid.canvas") -Destination $canvas -Force
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\formats\drawio-uncompressed.drawio") -Destination $drawio -Force
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_UX34_APP_ORIGIN = "http://127.0.0.1:$appPort"
    $env:LONGEDIT_UX34_CANVAS = $canvas
    $env:LONGEDIT_UX34_DRAWIO = $drawio
    $env:LONGEDIT_UX34_AUDIT_OUTPUT = $output
    $env:LONGEDIT_UX34_SOURCE_COMMIT = $sourceCommit
    & node (Join-Path $workspace "scripts\capture-ux34-drawio-canvas-stability.mjs")
    if ($LASTEXITCODE -ne 0) { throw "UX-34 desktop audit capture failed" }
  } finally {
...
Write-Output "UX-34 Drawio/Canvas stability audit completed: $output"
