...
const script = fs.readFileSync('scripts/apply-v123-version-transition.mjs', 'utf8').replace("import fs from 'node:fs'", '')
...
function fixture() {
  const files = new Map(list.map(file => [file, JSON.stringify({ appVersion: '1.0.22', nextStage: 'V1.0.22-UNSIGNED-PATCH-RELEASE', preserved: 'historical-v1.0.22' }, null, 2)]))
  files.set(listPath, listSource)
  files.set('package.json', '{"version": "1.0.22"}')
  files.set('package-lock.json', '{"version": "1.0.22", "packages": {"": {"version": "1.0.22"}}}')
  files.set('src-tauri/tauri.conf.json', '{"version": "1.0.22"}')
  files.set('src-tauri/Cargo.toml', 'version = "1.0.22"')
  files.set('src-tauri/Cargo.lock', 'name = "tauri-app"\nversion = "1.0.22"\n')
  files.set('shared/development-version-policy.json', JSON.stringify({ publicVersion: '1.0.22', publicTag: 'v1.0.22', runtimeBaseVersion: '1.0.22', developmentTargetVersion: '1.0.23' }))
  files.set('shared/v1-community-release-policy.json', JSON.stringify({ appVersion: '1.0.22', gates: { githubReleasePublished: true }, patchValidation: { managedUpdaterUpgradePath: '1.0.21-to-1.0.22-passed' } }))
  return files
}
...
test('apply changes all 44 identities while preserving public and historical fields', () => {
  ...
  for (const file of list) { const item = JSON.parse(writes.get(file)); assert.equal(item.appVersion, '1.0.23'); assert.equal(item.preserved, 'historical-v1.0.22') }
  const development = JSON.parse(writes.get('shared/development-version-policy.json'))
  assert.equal(development.publicTag, 'v1.0.22'); assert.equal(development.runtimeBaseVersion, '1.0.23')
  ...
  assert.equal(policy.patchValidation.managedUpdaterUpgradePath, '1.0.22-to-1.0.23-pending')
})
for (const scenario of ['late-contract-drift', 'unclosed-predecessor', 'wrong-baseline', 'unknown-argument']) {
  test(`${scenario} fails before any write`, () => {
    ...
    if (scenario === 'late-contract-drift') files.set(list.at(-1), '{"appVersion":"1.0.21"}')
