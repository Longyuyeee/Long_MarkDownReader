param(
  [string]$OutputDirectory = "docs\evidence\b2b-pdf-merge"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\b2b-pdf-merge"))
if ($output -ne $expectedOutput) { throw "B2B audit output must remain inside docs\evidence\b2b-pdf-merge" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "B2B desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-b2b-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$primary = Join-Path $library "B2B Primary.pdf"
$secondary = Join-Path $library "B2B Secondary.pdf"
$fixtureBytes = [Convert]::FromBase64String($pdfFixtureMatch.Groups[1].Value)
[System.IO.File]::WriteAllBytes($primary, $fixtureBytes)
$latin1 = [System.Text.Encoding]::GetEncoding(28591)
$secondarySource = $latin1.GetString($fixtureBytes)
$secondarySource = $secondarySource.Replace("Knowledge Graph Alpha", "Merge Source GammaOne")
$secondarySource = $secondarySource.Replace("Second Page Beta", "Merge Page Delta")
[System.IO.File]::WriteAllBytes($secondary, $latin1.GetBytes($secondarySource))
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_B2B_PRIMARY_PDF = $primary
    $env:LONGEDIT_B2B_SECONDARY_PDF = $secondary
    $env:LONGEDIT_B2B_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-b2b-pdf-merge-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "B2B desktop audit capture failed" }
  }
  finally {
...
Write-Output "B2B PDF merge desktop audit completed: $output"
