param([string]$OutputDirectory = "docs\evidence\post-v115-m1a4b2-xlsx-object-drafts")

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m1a4b2-xlsx-object-drafts"))
if ($output -ne $expectedOutput) { throw "M1A4B2 output must remain inside $expectedOutput" }
$appPort = 14200
$cdpPort = 14416
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1A4B2 requires free ports $appPort and $cdpPort" }
...
$env:TAURI_CONFIG = Get-Content -LiteralPath (Join-Path $workspace "src-tauri\tauri.e2e.conf.json") -Raw -Encoding UTF8
...
$auditRoot = Join-Path $env:TEMP ("longedit-m1a4b2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$xlsx = Join-Path $library "M1A4B2 Object Drafts.xlsx"
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_M1A4B2_AUDIT_OUTPUT = $output
    $env:LONGEDIT_M1A4B2_SOURCE_COMMIT = $sourceCommit
    $env:LONGEDIT_M1A4B2_XLSX = $xlsx
    & node (Join-Path $workspace "scripts\capture-post-v115-m1a4b2-xlsx-object-drafts.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M1A4B2 desktop capture failed" }
  } finally {
  ...
  if (-not $evidence.differenceResolved) { throw "M1A4B2 expected/actual difference was not resolved" }
} finally {
...
Write-Output "M1A4B2 XLSX object-draft audit completed: $output"
