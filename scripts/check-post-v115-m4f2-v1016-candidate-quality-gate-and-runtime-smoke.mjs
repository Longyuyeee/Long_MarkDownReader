...
if (community.currentStatus !== 'v1.0.16-community-release-quality-gate-and-runtime-smoke-passed-installer-pending' || community.releaseCandidate !== false || community.candidate?.artifactSourceCommit !== candidate || community.candidate?.qualityGateCommand !== policy.qualityGate.command) failures.push('community candidate intermediate state drifted')
...
if (development.currentStage !== `${policy.selectedNextStage?.id}-${policy.selectedNextStage?.name}` || development.binaryVersionTransition !== 'v1.0.16-quality-gate-and-runtime-smoke-passed' || development.releaseCandidate !== false) failures.push('M4F-3 handoff drifted')
