...
if (policy.publicVersion !== '1.0.15' || evidence.publicVersion !== '1.0.15' || !['1.0.15', '1.0.16', '1.0.17', '1.0.18', '1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.publicVersion) || development.publicTag !== `v${development.publicVersion}`) failures.push('public release boundary drifted')
...
if (!laterCandidateActive && (!community.gates?.frontendBuildPassed || !community.gates?.rustLockedCheckPassed || !community.gates?.productionDependencyAuditPassed || !community.gates?.qualityGatePassed || !community.gates?.localRuntimeSmokePassed)) failures.push('completed community gates drifted')
const lifecycleAdvanced = laterCandidateActive || ['v1.0.16-community-release-hosted-lifecycle-passed-final-release-audit-pending', 'v1.0.16-community-release-ready-to-publish', 'v1.0.16-community-release-published'].includes(community.currentStatus)
if (lifecycleAdvanced) {
  const releasePublished = laterCandidateActive || community.currentStatus === 'v1.0.16-community-release-published'
  ...
  const postClosureStage = development.binaryVersionTransition === 'v1.0.17-release-and-managed-updater-closed' || development.binaryVersionTransition.startsWith('v1.0.18-') || development.binaryVersionTransition.startsWith('v1.0.19-') || development.binaryVersionTransition.startsWith('v1.0.20-') || (development.binaryVersionTransition.startsWith('v1.0.21-') || (development.binaryVersionTransition.startsWith('v1.0.22-') || ['v1.0.23-quality-gate-pending', 'v1.0.23-release-ready', 'v1.0.23-public-release-published', 'v1.0.23-release-and-managed-updater-closed'].includes(development.binaryVersionTransition)))
  const publishedStage = postClosureStage
...
if ((laterCandidateActive || community.gates?.githubReleasePublished) ? git('rev-list', '-n', '1', 'v1.0.16') !== candidate : Boolean(git('tag', '--list', 'v1.0.16'))) failures.push('v1.0.16 tag state does not match release closure')
if (failures.length) { console.error(`M4F-2 candidate quality/runtime check failed:\n- ${failures.join('\n- ')}`); process.exit(1) }
console.log(`M4F-2 accepted: candidate ${candidate} passed the full quality gate, R5F 11/11 and R5G 6/6 + 11/11; later release stages remain consistent.`)
