param(
  [string]$OutputDirectory = "docs\evidence\b2a-pdf-page-extraction"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\b2a-pdf-page-extraction"))
if ($output -ne $expectedOutput) { throw "B2A audit output must remain inside docs\evidence\b2a-pdf-page-extraction" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "B2A desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-b2a-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$fixture = Join-Path $library "B2A Source.pdf"
[System.IO.File]::WriteAllBytes($fixture, [Convert]::FromBase64String($pdfFixtureMatch.Groups[1].Value))
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_B2A_PDF = $fixture
    $env:LONGEDIT_B2A_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-b2a-pdf-page-extraction-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "B2A desktop audit capture failed" }
  }
  finally {
...
Write-Output "B2A PDF page extraction desktop audit completed: $output"
