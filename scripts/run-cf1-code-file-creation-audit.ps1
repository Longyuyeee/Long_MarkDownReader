param(
  [string]$OutputDirectory = "docs\evidence\cf1-code-file-creation"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\cf1-code-file-creation"))
if ($output -ne $expectedOutput) { throw "CF-1 audit output must remain inside $expectedOutput" }

$appPort = 14200
$cdpPort = 14410
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) {
  throw "CF-1 desktop audit requires free local ports $appPort and $cdpPort"
}
...
$auditRoot = Join-Path $env:TEMP ("longedit-cf1-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CF1_APP_ORIGIN = "http://127.0.0.1:$appPort"
    $env:LONGEDIT_CF1_AUDIT_LIBRARY = $library
    $env:LONGEDIT_CF1_AUDIT_OUTPUT = $output
    $env:LONGEDIT_CF1_SOURCE_COMMIT = $sourceCommit
    & node (Join-Path $workspace "scripts\capture-cf1-code-file-creation.mjs")
    if ($LASTEXITCODE -ne 0) { throw "CF-1 desktop capture failed" }
  } finally {
...
Write-Output "CF-1 code file creation desktop audit completed: $output"
