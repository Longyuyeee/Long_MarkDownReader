param(
  [string]$OutputDirectory = "docs\evidence\c4b-pptx-text-patch"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c4b-pptx-text-patch"))
if ($output -ne $expectedOutput) { throw "C4B audit output must remain inside docs\evidence\c4b-pptx-text-patch" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C4B desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c4b-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C4B WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C4B_WPS = $fixture
    $env:LONGEDIT_C4B_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c4b-pptx-text-patch-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C4B desktop audit capture failed" }
  }
  finally {
...
Write-Output "C4B PPTX text patch desktop audit completed: $output"
