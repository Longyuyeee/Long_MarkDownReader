...
if (release.candidate?.hostedInstalledLifecycleRunId !== 33361759629 || !releaseChecker.includes('hosted.releaseCandidate !== false')) fail('M6-3 release receipt correction drift')
...
if (development.currentStage !== `${policy.selectedNextStage.id}-${policy.selectedNextStage.name}` || development.runtimeBaseVersion !== '1.0.17'
