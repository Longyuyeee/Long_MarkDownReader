param(
  [string]$OutputDirectory = "docs\evidence\c3c4-pptx-closure"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c3c4-pptx-closure"))
if ($output -ne $expectedOutput) { throw "C3C4 audit output must remain inside docs\evidence\c3c4-pptx-closure" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C3C4 desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c3c4-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $fixture -PathType Leaf)) { throw "C3C4 Microsoft PowerPoint fixture is missing" }
$auditPptx = Join-Path $library "C3C4 Lifecycle.pptx"
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C3C4_AUDIT_PPTX = $auditPptx
    $env:LONGEDIT_C3C4_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c3c4-pptx-closure-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C3C4 desktop audit capture failed" }
  } finally {
...
Write-Output "C3C4 PPTX closure audit completed: $output"
