...
if ($LASTEXITCODE -ne 0) { throw "P1-B2B3 custom-protocol build failed" }
$output = Join-Path $workspace "docs\evidence\p1b2b3-pdf-unicode-form-copy"
if (Test-Path -LiteralPath (Join-Path $output "manifest.json")) {
  if (-not $Refresh) { throw "P1-B2B3 accepted evidence already exists; pass -Refresh to replace generated evidence" }
  $expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\p1b2b3-pdf-unicode-form-copy"))
  if ([IO.Path]::GetFullPath($output) -ne $expectedOutput) { throw "P1-B2B3 evidence path mismatch" }
  Remove-Item -LiteralPath $output -Recurse -Force
}
$appPort = 14200
$cdpPort = 14516
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "P1-B2B3 audit requires free ports $appPort and $cdpPort" }
$auditRoot = Join-Path $env:TEMP ("longedit-p1b2b3-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$source = Join-Path $library "P1B2B3 Unicode AcroForm.pdf"
& node (Join-Path $workspace "scripts\create-p1b2a2-pdf-form-fixture.mjs") $source
if ($LASTEXITCODE -ne 0) { throw "P1-B2B3 fixture generation failed" }
...
try {
  ...
  try {
    ...
    & node (Join-Path $workspace "scripts\capture-p1b2b3-pdf-unicode-form-copy.mjs")
    if ($LASTEXITCODE -ne 0) { throw "P1-B2B3 capture failed" }
    $target = Join-Path $library "P1B2B3 Unicode AcroForm-form-filled.pdf"
    $poppler = "C:\Users\Administrator\.cache\codex-runtimes\codex-primary-runtime\dependencies\native\poppler\Library\bin\pdftoppm.exe"
    if (-not (Test-Path -LiteralPath $poppler)) { throw "P1-B2B3 Poppler renderer is unavailable" }
    & $poppler -f 1 -l 1 -singlefile -png -r 120 $target (Join-Path $output "unicode-pdf-poppler")
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath (Join-Path $output "unicode-pdf-poppler.png"))) { throw "P1-B2B3 Poppler render failed" }
    if ($sourceHash -ne (Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash) { throw "P1-B2B3 source fixture changed" }
...
Write-Output "P1-B2B3 PDF Unicode form copy audit completed: $output"
