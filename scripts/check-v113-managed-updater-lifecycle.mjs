...
const policy = json('shared/v113-managed-updater-lifecycle-policy.json')
const previousReceipt = json('docs/evidence/v1.0.12-release/release-receipt.json')
const currentReceipt = json('docs/evidence/v1.0.13-release/release-receipt.json')
const currentManifest = json('docs/evidence/v1.0.13-release/artifact-manifest.json')
...
const audit = read('docs/V1_0_13_Managed_Updater_Lifecycle_Audit_2026-08-21.md')

if (policy.schemaVersion !== 1 || policy.stage !== 'V1.0.13-U1'
  || !['hosted-execution-pending', 'hosted-managed-update-passed'].includes(policy.status)) fail('v1.0.13 updater policy identity drift')
...
if (policy.releases?.previous?.version !== '1.0.12'
...
if (policy.releases?.current?.version !== '1.0.13'
...
requireTokens(workflow, [
  ...
  'shared\\v113-managed-updater-lifecycle-policy.json',
  ...
  'v113-managed-updater-lifecycle-${{ github.run_id }}',
], 'v1.0.13 updater workflow')
...
for (const token of ['v1.0.12', 'v1.0.13', '用户确认', 'SHA-256', '自动重启', '资料保留', '托管 Windows']) {
  if (!audit.includes(token)) fail(`v1.0.13 updater audit missing: ${token}`)
}
...
if (!completed) {
  ...
  if (Object.values(policy.gates ?? {}).some(value => value !== true)
    ...
    || policy.nextAction !== 'v1.0.13-release-closure-complete') fail('completed updater state drift')
  const root = policy.evidenceRoot
  const importPath = `${root}/import-manifest.json`
  if (!fs.existsSync(importPath)) fail('v1.0.13 updater evidence is missing')
  else {
    const manifest = json(importPath)
    if (manifest.stage !== 'V1.0.13-U1I'
      || manifest.status !== 'accepted'
      || manifest.githubRunId !== policy.githubRun.id
      || manifest.headCommit !== policy.githubRun.headCommit
      || manifest.previousVersion !== '1.0.12'
      || manifest.currentVersion !== '1.0.13'
      ...
      || manifest.files.filter(file => file.path.endsWith('.jpg')).some(file => file.visuallyReviewed !== true)) fail('v1.0.13 updater import drift')
    for (const file of manifest.files ?? []) {
      const evidencePath = `${root}/${file.path}`
      if (!fs.existsSync(evidencePath)) fail(`v1.0.13 updater evidence missing: ${file.path}`)
      else if (fs.statSync(evidencePath).size !== file.bytes || sha256(evidencePath) !== file.sha256) fail(`v1.0.13 updater evidence hash drift: ${file.path}`)
    }
  }
}
...
console.log(`V1.0.13 managed updater lifecycle contract passed: ${completed ? 'hosted 1.0.12 -> 1.0.13 evidence accepted' : 'safe hosted execution harness is ready'}.`)
