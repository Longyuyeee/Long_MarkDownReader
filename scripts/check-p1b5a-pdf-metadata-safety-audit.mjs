...
if (!['P1-B5A', 'P1-B5B', 'P1-B5C', 'P1-B5D'].includes(metadata.stage) || !['safety-audit-complete-backend-pending', 'backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending', 'desktop-and-independent-render-verified'].includes(metadata.status) || metadata.currentWriteCapability !== ['P1-B5C', 'P1-B5D'].includes(metadata.stage) || metadata.securityMeaning !== 'descriptive-document-properties-only-not-complete-privacy-scrubbing' || metadata.approvedArchitecture !== 'full-rewrite-source-clone-plus-canonical-info-dictionary') fail('P1-B5A metadata contract identity is stale')
...
if (!['P1-B5A', 'P1-B5B', 'P1-B5C', 'P1-B5D'].includes(advanced.stage) || !['metadata-safety-audit-complete', 'metadata-backend-complete', 'metadata-workspace-complete', 'metadata-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('metadata-copy') !== ['P1-B5C', 'P1-B5D'].includes(advanced.stage) || !['safety-audit-complete-backend-pending', 'backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending', 'completed'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.currentWriteUserFile !== ['P1-B5C', 'P1-B5D'].includes(advanced.stage)) fail('P1-B5A advanced capability boundary is stale')
