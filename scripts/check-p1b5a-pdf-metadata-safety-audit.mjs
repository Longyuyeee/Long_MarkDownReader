...
if (metadata.stage !== 'P1-B5A' || metadata.status !== 'safety-audit-complete-backend-pending' || metadata.currentWriteCapability !== false || metadata.securityMeaning !== 'descriptive-document-properties-only-not-complete-privacy-scrubbing' || metadata.approvedArchitecture !== 'full-rewrite-source-clone-plus-canonical-info-dictionary') fail('P1-B5A metadata contract identity is stale')
...
if (advanced.stage !== 'P1-B5A' || advanced.status !== 'metadata-safety-audit-complete' || advanced.currentCapabilities?.includes('metadata-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.status !== 'safety-audit-complete-backend-pending' || advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.currentWriteUserFile !== false) fail('P1-B5A advanced capability boundary is stale')
for (const token of ['preview_pdf_metadata_copy', 'save_pdf_metadata_copy']) if (commands.includes(token) || tauri.includes(token) || view.includes(token)) fail(`P1-B5A must not expose a premature writer: ${token}`)
