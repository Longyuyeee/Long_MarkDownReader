...
if (!['P1-B5A', 'P1-B5B'].includes(metadata.stage) || !['safety-audit-complete-backend-pending', 'backend-complete-workspace-pending'].includes(metadata.status) || metadata.currentWriteCapability !== false || metadata.securityMeaning !== 'descriptive-document-properties-only-not-complete-privacy-scrubbing' || metadata.approvedArchitecture !== 'full-rewrite-source-clone-plus-canonical-info-dictionary') fail('P1-B5A metadata contract identity is stale')
...
if (!['P1-B5A', 'P1-B5B'].includes(advanced.stage) || !['metadata-safety-audit-complete', 'metadata-backend-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('metadata-copy') || !['safety-audit-complete-backend-pending', 'backend-complete-workspace-pending'].includes(advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.status) || advanced.plannedSlices?.find(item => item.id === 'P1-B5')?.currentWriteUserFile !== false) fail('P1-B5A advanced capability boundary is stale')
