...
const fileNavigation = await read('src/services/fileNavigation.ts')
const failures = []
const ids = new Set()
...
if (!library.includes('openManagedObject') || !fileNavigation.includes("kind === 'workbook-sheet'") || !fileNavigation.includes('sheet: objectId')) failures.push('workbook search result routing is incomplete')
...
requireText(fileNavigation, 'nextManagedLocatorToken', 'C3C2 repeated PPTX result clicks must issue fresh locator tokens')
requireText(fileNavigation, 'locatorKind: kind', 'C3C2 unified navigation must preserve PPTX locator kinds')
...
requireText(pptxReader, '图片替换', 'PPTX image controls must remain inside the existing details panel')
