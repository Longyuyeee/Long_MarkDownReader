param([string]$OutputDirectory = "docs\evidence\post-v115-m4c6-controlled-conversion-exit-audit")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4c6-controlled-conversion-exit-audit"))
if ($output -ne $expectedOutput) { throw "M4C-6 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14534
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4C-6 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4c6-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
$library = Join-Path $auditRoot "library"; $webviewData = Join-Path $auditRoot "webview"; $imports = Join-Path $library "imports"; $maps = Join-Path $library "maps"
Remove-Item -LiteralPath $output -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $imports,$maps,$webviewData,$output -Force | Out-Null
$utf8 = [Text.UTF8Encoding]::new($false)
[IO.File]::WriteAllText((Join-Path $imports "Conversion.csv"), "name,value,done`nalpha,001,false`nbeta,2,true`n", $utf8)
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\formats\mindmap.opml") -Destination (Join-Path $maps "Outline.opml")
[IO.File]::WriteAllText((Join-Path $library "Project Center.md"), "# Project Center`n`nPROJECT BODY MUST NOT COPY`n`n[[Project Peer]]`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Project Peer.md"), "# Project Peer`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Canvas Center.md"), "# Canvas Center`n`n[[Canvas Peer]]`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Canvas Peer.md"), "# Canvas Peer`n", $utf8)
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4C6_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4C6_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4C6_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4c6-controlled-conversion-exit-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4C-6 desktop capture failed" }
...
Write-Output "M4C-6 controlled conversion exit audit completed: $output"
