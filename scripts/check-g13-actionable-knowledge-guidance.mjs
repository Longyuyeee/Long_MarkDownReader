...
for (const key of ['graphHealthPanel', 'consentedObservationPreview', 'aggregateReceipt']) if (policy.surfaces[key] !== true) failures.push(`G13 surface missing: ${key}`)
if (policy.surfaces.workspaceHome !== false) failures.push('G13 workspace guidance must not duplicate graph governance')
...
for (const token of ['data-testid="knowledge-network-guidance"', 'data-guidance-code', 'guidanceCopy', '处理 ${item.currentValue} 个孤立对象']) requireText(graphPanel, token, `G13 graph guidance marker missing: ${token}`)
if (home.includes('data-testid="knowledge-network-guidance"')) failures.push('G13 guidance must not be duplicated on Workspace Home')
...
console.log('G13 actionable knowledge guidance passed: deterministic aggregate-only recommendations appear in graph governance and consented observation without duplicating Workspace Home.')
