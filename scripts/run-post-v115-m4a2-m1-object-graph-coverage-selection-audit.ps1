param([string]$OutputDirectory = "docs\evidence\post-v115-m4a2-m1-object-graph-coverage-selection")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4a2-m1-object-graph-coverage-selection"))
if ($output -ne $expectedOutput) { throw "M4A-2 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14532
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4A-2 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4a2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
Copy-Item -LiteralPath (Join-Path $workspace "fixtures\docx\producers\microsoft-word-16.docx") -Destination (Join-Path $library "M4A2 Word.docx")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\odf-content\longedit-e1c-spreadsheet.ods") -Destination (Join-Path $library "M4A2 Sheet.ods")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\odf-content\longedit-e1c-presentation.odp") -Destination (Join-Path $library "M4A2 Slides.odp")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\workbook\compatibility-baseline.xlsx") -Destination (Join-Path $library "M4A2 Workbook.xlsx")
[IO.File]::WriteAllText((Join-Path $library "M4A2 Control.md"), "# M4A-2 control`n`nSelection audit control node.`n", [Text.UTF8Encoding]::new($false))
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4A2_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4A2_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4A2_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4a2-m1-object-graph-coverage-selection-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4A-2 desktop capture failed" }
...
Write-Output "M4A-2 M1 object graph coverage selection audit completed: $output"
