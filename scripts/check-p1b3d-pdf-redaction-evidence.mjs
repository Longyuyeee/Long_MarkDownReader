...
if (!['P1-B3D', 'P1-B4A', 'P1-B4B'].includes(advanced.stage) || !['permanent-redaction-complete', 'watermark-safety-audit-complete', 'watermark-backend-complete'].includes(advanced.status) || !advanced.currentCapabilities?.includes('permanent-redaction-copy') || advanced.plannedSlices?.find(item => item.id === 'P1-B3')?.status !== 'completed') fail('P1-B3D advanced capability contract is stale')
