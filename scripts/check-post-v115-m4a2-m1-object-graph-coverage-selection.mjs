...
for (const token of ['".pptx"', '".ods"', '".odp"', '".xlsx"']) {
  const currentlyDispatched = graph.includes(token)
  if (token === '".pptx"' ? !currentlyDispatched : currentlyDispatched) failures.push(`unexpected current graph dispatch state for ${token}`)
}
for (const id of ['odp', 'odp_slide', 'workbook', 'workbook_sheet']) if (semantics.objectTypes.some(item => item.id === id)) failures.push(`selection audit must precede semantic implementation: ${id}`)
...
console.log('M4A-2 selection accepted: real M1 locators exist, graph coverage is absent, and M4A-3 is frozen to Workbook sheets plus ODP slides.')
