...
if (policy.status !== 'installed-guidance-outcome-functional-passed-visual-recapture-next' || policy.nextStage !== 'G15C-hosted-installed-visual-recapture-and-evidence-review') failures.push('G15C stage boundary drift')
if (!/^[a-f0-9]{40}$/.test(policy.productSourceCommit) || policy.hostedRunId !== 30693531173) failures.push('G15C execution identity drift')
...
if (policy.attemptHistory.length !== 2 || policy.attemptHistory[0].runId !== 30692454820 || policy.attemptHistory[0].status !== 'failed-before-g15c' || policy.attemptHistory[0].productBuildComplete !== true || policy.attemptHistory[0].g15cExecuted !== false || policy.attemptHistory[1].runId !== 30693531173 || policy.attemptHistory[1].status !== 'functional-passed-visual-recapture-required' || policy.attemptHistory[1].g15cExecuted !== true) failures.push('G15C attempt audit drift')
...
if (policy.qualityGate.hostedInstalledExecutionComplete !== true) failures.push('G15C hosted functional execution must remain accepted')
for (const key of ['screenshotReviewComplete', 'realUserComparisonComplete', 'signedWindowsClientEvidenceComplete']) if (policy.qualityGate[key] !== false) failures.push(`G15C evidence must remain pending: ${key}`)

for (const token of ['installed workspace initialization before route testing', `location.hash.startsWith('#/workspace')`, "scrollIntoView({ block: 'center' })", 'observationSurface.viewportVisible', 'outside the screenshot viewport', 'data-testid="knowledge-observation-export"', 'data-testid="knowledge-observation-compare"', "invokeTauri('export_knowledge_graph_observation'", "invokeTauri('get_knowledge_graph_observation_comparison'", "invokeTauri('export_knowledge_graph_observation_comparison'", "comparisonReceipt.outcome !== 'improved'", 'Installed knowledge comparison leaked synthetic identifier', "stage: 'G15C'", "id: 'installed-consented-knowledge-guidance-outcome'", ...policy.expectedEvidenceFiles]) requireText(capture, token, `G15C installed runner marker missing: ${token}`)
...
console.log('G15C installed guidance outcome runner passed: hosted functional execution is accepted after aggregate baseline, synthetic remediation, comparison, and privacy checks; visual recapture and real-user evidence remain pending.')
