...
if (policy.selectedNextStage?.id !== successor.stage || successor.predecessor !== policy.stage || successor.selectedNextStage?.id !== exitAudit.stage || exitAudit.predecessor !== successor.stage || development.currentStage !== `${exitAudit.selectedNextStage.id}-${exitAudit.selectedNextStage.name}`) failures.push('M4C successor handoff is not aligned')
