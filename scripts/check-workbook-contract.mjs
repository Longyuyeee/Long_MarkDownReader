...
if (!arrayRead || arrayRead.stage !== 'X3-B5' || !['partial_producer_matrix', 'producer_matrix_verified_pending_release_audit'].includes(arrayRead.status) || arrayRead.calculationPolicy !== 'blocked' || arrayRead.contentWritePolicy !== 'blocked-inside-declared-range' || arrayRead.structureWritePolicy !== 'blocked-on-containing-sheet') fail('X3-B5 array read boundary drift')
if (arrayRead.maxFormulasPerSheet !== 1024 || arrayRead.maxDeclaredCellsPerSheet !== 1000000 || !arrayRead.realOfficeProducerEvidence || arrayRead.verifiedProducerCount !== arrayProducerMatrix.verifiedProducers || arrayRead.requiredProducerCount !== 3 || arrayRead.fullProducerMatrixVerified !== (arrayRead.verifiedProducerCount === 3)) fail('X3-B5 array resource/evidence boundary drift')
if (arrayRead.producerEvidenceHandoff?.status !== 'ready' || arrayRead.producerEvidenceHandoff?.trustedMachineConfirmationRequired !== true || arrayRead.producerEvidenceHandoff?.longEditSemanticReparseRequired !== true || arrayRead.producerEvidenceHandoff?.rejectionValidation?.verifiedCaseCount !== 5) fail('X3-B5 producer evidence handoff drift')
...
if (arrayProducerMatrix.stage !== 'X3-B5' || !['partial', 'verified'].includes(arrayProducerMatrix.status) || arrayProducerMatrix.verifiedProducers < 1 || arrayProducerMatrix.verifiedProducers > 3 || arrayProducerMatrix.requiredProducers !== 3 || arrayProducerMatrix.evidenceHandoff?.status !== 'ready' || (arrayProducerMatrix.status === 'verified') !== (arrayProducerMatrix.verifiedProducers === 3)) fail('X3-B5 producer matrix drift')
