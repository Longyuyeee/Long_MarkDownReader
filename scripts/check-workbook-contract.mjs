...
if (!printProtection || printProtection.read !== 'supported' || printProtection.view !== 'limited' || printProtection.edit !== 'limited' || printProtection.roundTrip !== 'supported') fail('print_and_protection S8-5B status drift')
if (!model.includes('pub page_layout: WorkbookPageLayout') || !model.includes('pub protection: WorkbookProtection') || !model.includes('pub sheet_protection: WorkbookCapabilityLevel') || !model.includes('WorkbookPageLayoutPayload') || !model.includes('WorkbookHeaderFooterPayload')) fail('page/protection model evidence missing')
if (!ooxml.includes('parse_page_layout') || !ooxml.includes('read_workbook_protection') || !ooxml.includes('patch_workbook_page_layout') || !ooxml.includes('patch_workbook_header_footer') || !engine.includes('page_layout_round_trips_through_command_boundary') || !engine.includes('header_footer_round_trips_through_command_boundary') || !engine.includes('refuses_to_edit_or_reconfigure_protected_sheet')) fail('page/protection S8-5B backend evidence missing')
if (!view.includes('page-layout-toolbar') || !view.includes('sheetProtected') || !view.includes('savePageLayout') || !view.includes('saveHeaderFooter') || !view.includes("invoke<WorkbookDocument>('update_workbook_page_layout'") || !view.includes("invoke<WorkbookDocument>('update_workbook_header_footer'")) fail('page/protection S8-5B frontend evidence missing')
