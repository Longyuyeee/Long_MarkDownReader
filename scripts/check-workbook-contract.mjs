...
const arrayDesktopEvidence = JSON.parse(read('docs/evidence/x3-b3-xlsx-array-desktop/audit-manifest.json'))
...
if (!arrayRead || arrayRead.stage !== 'X3-B3' || arrayRead.status !== 'partial_producer_matrix' || arrayRead.calculationPolicy !== 'blocked' || arrayRead.contentWritePolicy !== 'blocked-inside-declared-range' || arrayRead.structureWritePolicy !== 'blocked-on-containing-sheet') fail('X3-B3 array read boundary drift')
if (arrayRead.maxFormulasPerSheet !== 1024 || arrayRead.maxDeclaredCellsPerSheet !== 1000000 || !arrayRead.realOfficeProducerEvidence || arrayRead.fullProducerMatrixVerified !== false || arrayRead.verifiedProducerCount !== 1 || arrayRead.requiredProducerCount !== 3) fail('X3-B3 array resource/evidence boundary drift')
if (arrayRead.spillDiagnostics?.diagnosticAddressLimitPerFormula !== 256 || !arrayRead.spillDiagnostics?.conflictAndErrorAddressesLocatable || !arrayRead.visibleFields?.includes('cachedValueTypes') || !arrayRead.visibleFields?.includes('errorCacheCells') || !arrayRead.visibleFields?.includes('conflictCells')) fail('X3-B3 diagnostic contract drift')
...
if (!model.includes('WorkbookArrayFormula') || !model.includes('cached_value_types') || !model.includes('error_cache_cells') || !model.includes('conflict_cells') || !ooxml.includes('read_array_formulas') || !ooxml.includes('MAX_ARRAY_DIAGNOSTIC_CELLS') || !ooxml.includes('reads_wps_array_formula_round_trip_with_recalculated_cache') || !ooxml.includes('potential_conflict') || !view.includes('array-formula-strip') || !view.includes('cacheTypeSummary') || !view.includes('navigateArrayDiagnosticCell')) fail('X3-B3 array implementation evidence missing')
...
if (arrayDesktopEvidence.stage !== 'X3-B3' || !arrayDesktopEvidence.sourceUnchanged || arrayDesktopEvidence.checks?.length !== 10 || arrayDesktopEvidence.evidenceFiles?.length !== 2 || arrayDesktopEvidence.boundaries?.expectedSpillCalculation !== false || arrayDesktopEvidence.boundaries?.arrayWriteback !== false) fail('X3-B3 desktop evidence drift')
if (dynamicArrayFeature.read !== 'limited' || dynamicArrayFeature.view !== 'limited' || dynamicArrayFeature.edit !== 'limited') fail('X3-B3 array capability status drift')
