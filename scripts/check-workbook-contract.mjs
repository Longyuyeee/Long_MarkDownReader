...
if (!drawings || drawings.read !== 'supported' || drawings.view !== 'limited' || drawings.edit !== 'limited' || drawings.roundTrip !== 'supported') fail('charts_and_drawings S8-4D1 status drift')
...
if (!ooxml.includes('read_sheet_drawings') || !ooxml.includes('parse_chart_part') || !ooxml.includes('patch_workbook_drawing') || !ooxml.includes('patch_chart_presentation_xml') || !ooxml.includes('updates_two_cell_drawing_metadata_and_anchor_without_rebuilding_chart_parts') || !ooxml.includes('updates_chart_title_and_internal_series_references_with_semantic_verification') || !ooxml.includes('creates_changes_and_deletes_standard_chart_lifecycle')) fail('drawing S8-4D1 parser/transaction evidence missing')
if (!generator.includes('Chart::new') || !generator.includes('Image::new')) fail('chart/image fixture evidence missing')
if (!view.includes('drawing-toolbar') || !view.includes('navigateDrawing') || !view.includes('editDrawingMetadata') || !view.includes('applyDrawingSelection') || !view.includes('editChartTitle') || !view.includes('editChartSeries') || !view.includes('createChartFromSelection') || !view.includes('changeSelectedChartType') || !view.includes('editChartAxes') || !view.includes('applyChartLegendPosition') || !view.includes('deleteSelectedChart') || !view.includes('TableChartEditor') || !view.includes('loadChartPreview') || !view.includes("invoke<WorkbookDocument>('update_workbook_drawing'")) fail('drawing S8-4D1 selection/edit/preview evidence missing')
