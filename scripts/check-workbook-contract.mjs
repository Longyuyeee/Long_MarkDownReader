...
if (!engine.includes('variants.package_variant_count, 7') || !engine.includes('variants.semantic_variant_count, 3') || !engine.includes('(\"multi_measure\", 1, 1, 3)') || !ooxml.includes('rebuilds_all_pivot_aggregations_from_raw_records_before_totals')) fail('S8-7E2E variant verification regression evidence missing')
if (!view.includes("invoke<WorkbookPivotVariantVerificationResult>('verify_workbook_pivot_variants_isolated_copy'") || !view.includes('聚合与布局变体已通过') || !view.includes('单行轴') || !view.includes('多度量')) fail('S8-7E2E variant verification UI evidence missing')
