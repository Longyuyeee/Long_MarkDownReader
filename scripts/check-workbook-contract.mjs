...
const arrayFormulaWpsFixture = JSON.parse(read('src-tauri/tests/fixtures/workbook/array-formula-wps-spreadsheets.json'))
const arrayConflictFixture = JSON.parse(read('src-tauri/tests/fixtures/workbook/array-formula-conflict-diagnostic.json'))
const arrayProducerMatrix = JSON.parse(read('docs/evidence/x3-b2-xlsx-array-producers/matrix.json'))
const arrayDesktopBaselineEvidence = JSON.parse(read('docs/evidence/x3-b3-xlsx-array-desktop/audit-manifest.json'))
const arrayConflictDesktopEvidence = JSON.parse(read('docs/evidence/x3-b4-xlsx-array-conflict-desktop/audit-manifest.json'))
const arrayProducerAudit = read('scripts/run-x3-b2-array-producer-audit.ps1')
...
if (!dynamicArrayFeature || dynamicArrayFeature.calculate !== 'limited' || !ooxml.includes('validate_workbook_calculation_boundary')) fail('E1A array boundary status drift')
const arrayRead = formulaCapabilities.arrayFormulaReadContract
if (!arrayRead || arrayRead.stage !== 'X3-B6' || !['partial_producer_matrix', 'producer_matrix_verified_pending_release_audit'].includes(arrayRead.status) || arrayRead.calculationPolicy !== 'blocked' || arrayRead.contentWritePolicy !== 'blocked-inside-declared-range' || arrayRead.structureWritePolicy !== 'blocked-on-containing-sheet') fail('X3-B6 array read boundary drift')
if (arrayRead.maxFormulasPerSheet !== 1024 || arrayRead.maxDeclaredCellsPerSheet !== 1000000 || !arrayRead.realOfficeProducerEvidence || arrayRead.verifiedProducerCount !== arrayProducerMatrix.verifiedProducers || arrayRead.requiredProducerCount !== 3 || arrayRead.fullProducerMatrixVerified !== (arrayRead.verifiedProducerCount === 3)) fail('X3-B6 array resource/evidence boundary drift')
if (arrayRead.producerEvidenceHandoff?.status !== 'ready' || arrayRead.producerEvidenceHandoff?.trustedMachineConfirmationRequired !== true || arrayRead.producerEvidenceHandoff?.longEditSemanticReparseRequired !== true || arrayRead.producerEvidenceHandoff?.rejectionValidation?.verifiedCaseCount !== 5 || arrayRead.producerEvidenceHandoff?.atomicPairClosure?.status !== 'verified') fail('X3-B6 producer evidence handoff drift')
if (arrayRead.spillDiagnostics?.diagnosticAddressLimitPerFormula !== 256 || !arrayRead.spillDiagnostics?.conflictAndErrorAddressesLocatable || !arrayRead.spillDiagnostics?.truncationVisible || !arrayRead.spillDiagnostics?.distinctConflictAndErrorLocatorEvidence || !arrayRead.visibleFields?.includes('cachedValueTypes') || !arrayRead.visibleFields?.includes('errorCacheCells') || !arrayRead.visibleFields?.includes('conflictCells')) fail('X3-B4 diagnostic contract drift')
const dynamicPreview = formulaCapabilities.dynamicArrayPreviewContract
if (dynamicPreview?.stage !== 'E1A' || dynamicPreview?.status !== 'verified-bounded-preview' || dynamicPreview?.supportedFunctions?.join(',') !== 'SEQUENCE' || dynamicPreview?.maxPreviewCells !== 10000 || dynamicPreview?.writesUserFile !== false || dynamicPreview?.writesFormulaCache !== false || dynamicPreview?.resultPersistence !== 'none' || dynamicPreview?.formulaDependencies !== 'blocked' || !dynamicPreview?.unsavedNumericDependencies || !dynamicPreview?.sourcePackageUnchanged) fail('E1A dynamic array preview contract drift')
...
if (arrayFormulaWpsFixture.stage !== 'X3-B2' || arrayFormulaWpsFixture.producer !== 'WPS Spreadsheets' || !arrayFormulaWpsFixture.independentNativeReopen || arrayFormulaWpsFixture.arrayDeclarationCount !== 2) fail('X3-B2 WPS producer manifest drift')
if (arrayProducerMatrix.stage !== 'X3-B6' || !['partial', 'verified'].includes(arrayProducerMatrix.status) || arrayProducerMatrix.verifiedProducers < 1 || arrayProducerMatrix.verifiedProducers > 3 || arrayProducerMatrix.requiredProducers !== 3 || arrayProducerMatrix.evidenceHandoff?.status !== 'ready' || arrayProducerMatrix.evidenceHandoff?.atomicPairClosure?.status !== 'verified' || (arrayProducerMatrix.status === 'verified') !== (arrayProducerMatrix.verifiedProducers === 3)) fail('X3-B6 producer matrix drift')
const wpsArrayProducer = arrayProducerMatrix.producers.find(item => item.id === 'wps-spreadsheets')
if (!wpsArrayProducer || wpsArrayProducer.status !== 'verified' || !wpsArrayProducer.nativeSave || !wpsArrayProducer.independentReopen) fail('X3-B2 WPS producer evidence missing')
const wpsArrayBytes = fs.readFileSync(path.join(root, arrayFormulaWpsFixture.fixture.startsWith('src-tauri/') ? arrayFormulaWpsFixture.fixture : `src-tauri/tests/fixtures/workbook/${arrayFormulaWpsFixture.fixture}`))
if (createHash('sha256').update(wpsArrayBytes).digest('hex') !== arrayFormulaWpsFixture.sha256) fail('X3-B2 WPS fixture hash drift')
const conflictFixtureBytes = fs.readFileSync(path.join(root, 'src-tauri/tests/fixtures/workbook', arrayConflictFixture.fixture))
if (arrayConflictFixture.stage !== 'X3-B4' || arrayConflictFixture.expectedConflictCells?.join(',') !== 'D3' || arrayConflictFixture.expectedErrorCacheCells?.join(',') !== 'D4' || arrayConflictFixture.expectedErrorCacheValue !== '#DIV/0!' || createHash('sha256').update(conflictFixtureBytes).digest('hex') !== arrayConflictFixture.sha256) fail('X3-B4 controlled diagnostic fixture drift')
if (!model.includes('WorkbookArrayFormula') || !model.includes('cached_value_types') || !model.includes('error_cache_cells') || !model.includes('conflict_cells') || !ooxml.includes('read_array_formulas') || !ooxml.includes('MAX_ARRAY_DIAGNOSTIC_CELLS') || !ooxml.includes('reads_controlled_array_conflict_and_error_cache_fixture') || !ooxml.includes('bounds_array_formula_diagnostic_addresses_without_hiding_totals') || !view.includes('array-formula-strip') || !view.includes('cacheTypeSummary') || !view.includes('navigateArrayDiagnosticCell') || !view.includes('诊断地址已截断（最多显示 256 个）')) fail('X3-B4 array implementation evidence missing')
if (!arrayProducerAudit.includes('KET.Application') || !arrayProducerAudit.includes('independentNativeReopen')) fail('X3-B2 producer audit protocol missing')
if (arrayDesktopBaselineEvidence.stage !== 'X3-B3' || !arrayDesktopBaselineEvidence.sourceUnchanged || arrayDesktopBaselineEvidence.checks?.length !== 10) fail('X3-B3 desktop baseline evidence drift')
if (arrayConflictDesktopEvidence.stage !== 'X3-B4' || !arrayConflictDesktopEvidence.sourceUnchanged || arrayConflictDesktopEvidence.checks?.length !== 12 || arrayConflictDesktopEvidence.evidenceFiles?.length !== 2 || !arrayConflictDesktopEvidence.boundaries?.conflictFixtureVisualEvidence || !arrayConflictDesktopEvidence.boundaries?.addressTruncationCoveredByRustAndUiContract || arrayConflictDesktopEvidence.boundaries?.expectedSpillCalculation !== false || arrayConflictDesktopEvidence.boundaries?.arrayWriteback !== false) fail('X3-B4 desktop evidence drift')
if (dynamicArrayFeature.read !== 'limited' || dynamicArrayFeature.view !== 'limited' || dynamicArrayFeature.edit !== 'limited') fail('X3-B4 array capability status drift')
...
if (!packageLayouts || packageLayouts.rowOnly?.rowFields !== 1 || packageLayouts.rowOnly?.columnFields !== 0 || packageLayouts.rowOnly?.dataFields !== 1 || packageLayouts.rowOnly?.outputRange !== 'A3:B6' || packageLayouts.rowOnly?.outputCellCount !== 8) fail('S8-7E2F row-only package drift')
if (packageLayouts.columnOnly?.rowFields !== 0 || packageLayouts.columnOnly?.columnFields !== 1 || packageLayouts.columnOnly?.dataFields !== 1 || packageLayouts.columnOnly?.outputRange !== 'A3:D5' || packageLayouts.columnOnly?.outputCellCount !== 12) fail('S8-7E2F column-only package drift')
if (packageLayouts.multiMeasure?.rowFields !== 1 || packageLayouts.multiMeasure?.columnFields !== 1 || packageLayouts.multiMeasure?.dataFields !== 3 || packageLayouts.multiMeasure?.aggregations?.join(',') !== 'sum,count,average' || packageLayouts.multiMeasure?.outputRange !== 'A3:J8' || packageLayouts.multiMeasure?.outputCellCount !== 60 || packageLayouts.multiMeasure?.dataAxisField !== -2) fail('S8-7E2F multi-measure package drift')
...
if (pivotCopySave.targetScope !== 'same_directory_xlsx' || pivotCopySave.opensSavedCopy !== true || pivotCopySave.sourceOverwrite !== 'blocked' || pivotCopySave.existingTargetOverwrite !== 'blocked' || pivotCopySave.unsavedDrafts !== 'blocked' || pivotCopySave.multiMeasureSave !== 'verified_in_S8-7E3C' || pivotCopySave.multiLevelAxis !== 'blocked' || pivotCopySave.pageFields !== 'blocked' || pivotCopySave.externalData !== 'blocked' || pivotCopySave.desktopProducerRoundTrip !== 'verified_in_S8-7E3B') fail('S8-7E3A release boundary drift')
...
if (!engine.includes('pub fn generate_workbook_pivot_audit_copy') || !engine.includes('pivot_producer_round_trip_outputs_reopen_with_stable_semantics') || !tauriLib.includes('generate_workbook_pivot_audit_copy')) fail('S8-7E3B LongEdit audit-copy or reverse-reopen evidence missing')
if (!pivotAuditCli.includes('generate_workbook_pivot_layout_audit_copy') || !pivotProducerAudit.includes('Excel.Application') || !pivotProducerAudit.includes('KET.Application') || !pivotProducerAudit.includes('processRestarted = $true') || !pivotLibreOfficeAudit.includes('pivot.refresh()')) fail('S8-7E3B producer automation evidence missing')
const pivotLayoutCopySave = pivotWritebackAudit.layoutVariantCopySave
if (!pivotLayoutCopySave || pivotLayoutCopySave.stage !== 'S8-7E3C' || pivotLayoutCopySave.status !== 'verified' || pivotLayoutCopySave.verifiedRoundTrips !== 9 || pivotLayoutCopySave.requiredRoundTrips !== 9) fail('S8-7E3C layout save matrix policy drift')
if (pivotLayoutCopySave.requiredLayoutIds?.join(',') !== 'row_only,column_only,multi_measure' || pivotLayoutCopySave.requiredProducerIds?.join(',') !== 'microsoft-excel,wps-spreadsheets,libreoffice-calc') fail('S8-7E3C required matrix drift')
if (pivotLayoutCopySave.layouts?.rowOnly?.outputRange !== 'A3:B6' || pivotLayoutCopySave.layouts?.columnOnly?.outputRange !== 'A3:D5' || pivotLayoutCopySave.layouts?.columnOnly?.libreOfficeNormalizedRange !== 'A3:C5' || pivotLayoutCopySave.layouts?.multiMeasure?.aggregations?.join(',') !== 'sum,count,average' || pivotLayoutCopySave.layouts?.multiMeasure?.sourceFields?.join(',') !== '1,0,1' || pivotLayoutCopySave.layouts?.multiMeasure?.outputRange !== 'A3:J8') fail('S8-7E3C verified layout drift')
if (pivotLayoutCopySave.producerNormalizationAllowed !== true || pivotLayoutCopySave.normalizedStateMustSurviveProcessRestart !== true || pivotLayoutCopySave.repairPromptObserved !== false || pivotLayoutCopySave.sourceOverwrite !== 'blocked' || pivotLayoutCopySave.existingTargetOverwrite !== 'blocked' || pivotLayoutCopySave.multiLevelAxis !== 'blocked' || pivotLayoutCopySave.pageFields !== 'blocked' || pivotLayoutCopySave.externalData !== 'blocked') fail('S8-7E3C release boundary drift')
for (const step of ['create_digest_bound_layout_copy', 'refresh_in_desktop_producer', 'save_xlsx', 'quit_process', 'reopen_in_new_process', 'verify_normalized_semantics', 'reparse_ooxml_identity_and_fields_in_longedit']) {
  if (!pivotLayoutCopySave.workflow?.includes(step)) fail(`S8-7E3C workflow step ${step} missing`)
}
...
const pivotMultiLevelAxisOutputRebuild = pivotWritebackAudit.multiLevelAxisOutputRebuild
if (!pivotMultiLevelAxisOutputRebuild || pivotMultiLevelAxisOutputRebuild.stage !== 'S8-7E3F' || pivotMultiLevelAxisOutputRebuild.execution !== 'temporary_copy_only' || pivotMultiLevelAxisOutputRebuild.status !== 'multi_axis_output_rebuilt') fail('S8-7E3F multi-axis output rebuild policy drift')
const multiAxisOutputLayout = pivotMultiLevelAxisOutputRebuild.verifiedLayout
if (!multiAxisOutputLayout || multiAxisOutputLayout.rowFields !== 2 || multiAxisOutputLayout.columnFields !== 2 || multiAxisOutputLayout.dataFields !== 1 || multiAxisOutputLayout.pageFields !== 0 || multiAxisOutputLayout.sourceRecordCount !== 16 || multiAxisOutputLayout.previewGroupCount !== 16 || multiAxisOutputLayout.outputRange !== 'A3:I12' || multiAxisOutputLayout.outputCellCount !== 80 || multiAxisOutputLayout.grandTotal !== 424) fail('S8-7E3F verified layout drift')
for (const structure of ['pivot_field_items', 'row_fields', 'column_fields', 'compressed_row_items', 'compressed_column_items', 'multi_axis_headers', 'detail_output_cells', 'parent_subtotals', 'row_totals', 'column_totals', 'grand_total', 'cache_definition', 'cache_records', 'output_cells']) {
  if (!pivotMultiLevelAxisOutputRebuild.synchronizedStructures?.includes(structure)) fail(`S8-7E3F synchronized structure ${structure} missing`)
}
for (const gate of ['signature_check', 'multi_axis_field_inventory', 'compressed_hierarchy_decode', 'cache_definition_rebuild', 'cache_records_rebuild', 'pivot_definition_rebuild', 'output_worksheet_rebuild', 'package_validation', 'semantic_reparse', 'output_value_reparse', 'untouched_part_preservation', 'source_package_unchanged']) {
  if (!pivotMultiLevelAxisOutputRebuild.requiredGates?.includes(gate)) fail(`S8-7E3F required gate ${gate} missing`)
}
if (pivotMultiLevelAxisOutputRebuild.pivotDefinition !== 'rebuilt_in_temporary_copy' || pivotMultiLevelAxisOutputRebuild.outputWorksheet !== 'rebuilt_in_temporary_copy' || pivotMultiLevelAxisOutputRebuild.multiAxisOutputRebuild !== 'verified' || pivotMultiLevelAxisOutputRebuild.reliableCopySave !== 'blocked' || pivotMultiLevelAxisOutputRebuild.producerRoundTrip !== 'pending' || pivotMultiLevelAxisOutputRebuild.writesUserFile !== false || pivotMultiLevelAxisOutputRebuild.sourceOverwrite !== 'blocked' || pivotMultiLevelAxisOutputRebuild.pageFields !== 'blocked' || pivotMultiLevelAxisOutputRebuild.externalData !== 'blocked') fail('S8-7E3F release boundary drift')
const pivotMultiLevelAxisProducerRoundTrip = pivotWritebackAudit.multiLevelAxisProducerRoundTrip
if (!pivotMultiLevelAxisProducerRoundTrip || pivotMultiLevelAxisProducerRoundTrip.stage !== 'S8-7E3G' || pivotMultiLevelAxisProducerRoundTrip.execution !== 'incremental_producer_roundtrip' || pivotMultiLevelAxisProducerRoundTrip.status !== 'partial') fail('S8-7E3G multi-axis producer policy drift')
if (pivotMultiLevelAxisProducerRoundTrip.baselineStatus !== 'audit_copy_verified' || pivotMultiLevelAxisProducerRoundTrip.baselineOutputRange !== 'A3:I12' || pivotMultiLevelAxisProducerRoundTrip.baselineOutputCellCount !== 80 || pivotMultiLevelAxisProducerRoundTrip.baselinePreviewGroupCount !== 16 || pivotMultiLevelAxisProducerRoundTrip.baselineGrandTotal !== 424) fail('S8-7E3G multi-axis baseline drift')
if (pivotMultiLevelAxisProducerRoundTrip.requiredProducerIds?.join(',') !== 'microsoft-excel,wps-spreadsheets,libreoffice-calc' || pivotMultiLevelAxisProducerRoundTrip.verifiedProducerIds?.join(',') !== 'wps-spreadsheets,libreoffice-calc' || pivotMultiLevelAxisProducerRoundTrip.verifiedProducerCount !== 2 || pivotMultiLevelAxisProducerRoundTrip.requiredProducerCount !== 3) fail('S8-7E3G producer matrix boundary drift')
if (pivotMultiLevelAxisProducerRoundTrip.producerRoundTrip !== 'partial_2_of_3' || pivotMultiLevelAxisProducerRoundTrip.reliableCopySave !== 'blocked' || pivotMultiLevelAxisProducerRoundTrip.writesUserFile !== false || pivotMultiLevelAxisProducerRoundTrip.sourceOverwrite !== 'blocked' || pivotMultiLevelAxisProducerRoundTrip.existingTargetOverwrite !== 'blocked' || pivotMultiLevelAxisProducerRoundTrip.pageFields !== 'blocked' || pivotMultiLevelAxisProducerRoundTrip.externalData !== 'blocked' || pivotMultiLevelAxisProducerRoundTrip.slicers !== 'blocked') fail('S8-7E3G release boundary drift')
const pivotExcelEvidenceHandoff = pivotMultiLevelAxisProducerRoundTrip.excelEvidenceHandoff
if (!pivotExcelEvidenceHandoff || pivotExcelEvidenceHandoff.stage !== 'S8-7E3G-D' || pivotExcelEvidenceHandoff.status !== 'ready' || pivotExcelEvidenceHandoff.environmentStatus !== 'compatible_server_not_microsoft_excel') fail('S8-7E3G-D Excel evidence handoff policy drift')
if (pivotExcelEvidenceHandoff.requiredBundleMembers?.join(',') !== 'manifest.json,producer.json,s8-7e3g-microsoft-excel.xlsx' || pivotExcelEvidenceHandoff.trustedMachineConfirmationRequired !== true || pivotExcelEvidenceHandoff.producerIdentityBound !== true || pivotExcelEvidenceHandoff.baselineDigestBound !== true || pivotExcelEvidenceHandoff.longEditSemanticReparseRequired !== true || pivotExcelEvidenceHandoff.existingEvidenceOverwrite !== 'blocked') fail('S8-7E3G-D Excel evidence handoff boundary drift')
...
if (!model.includes('WorkbookPivotMultiAxisAuditResult') || !model.includes('output_cell_count') || !ooxml.includes('audit_workbook_pivot_multi_axis_isolated') || !ooxml.includes('build_pivot_multi_axis_output_edits') || !ooxml.includes('multi_axis_output_rebuilt') || !engine.includes('audit_workbook_pivot_multi_axis_isolated_copy') || !engine.includes('output_cell_count, 80') || !engine.includes('generate_workbook_pivot_multi_axis_audit_copy') || !tauriLib.includes('audit_workbook_pivot_multi_axis_isolated_copy') || !tauriLib.includes('generate_workbook_pivot_multi_axis_audit_copy')) fail('S8-7E3F/E3G isolated audit command evidence missing')
