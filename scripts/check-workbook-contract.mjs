...
if (!arrayRead || arrayRead.stage !== 'X3-B2' || arrayRead.status !== 'partial_producer_matrix' || arrayRead.calculationPolicy !== 'blocked' || arrayRead.contentWritePolicy !== 'blocked-inside-declared-range' || arrayRead.structureWritePolicy !== 'blocked-on-containing-sheet') fail('X3-B2 array read boundary drift')
if (arrayRead.maxFormulasPerSheet !== 1024 || arrayRead.maxDeclaredCellsPerSheet !== 1000000 || !arrayRead.realOfficeProducerEvidence || arrayRead.fullProducerMatrixVerified !== false || arrayRead.verifiedProducerCount !== 1 || arrayRead.requiredProducerCount !== 3) fail('X3-B2 array resource/evidence boundary drift')
...
if (!model.includes('WorkbookArrayFormula') || !model.includes('foreign_formula_cell_count') || !ooxml.includes('read_array_formulas') || !ooxml.includes('reads_wps_array_formula_round_trip_with_recalculated_cache') || !ooxml.includes('potential_conflict') || !view.includes('array-formula-strip') || !view.includes('spillStatusLabel')) fail('X3-B2 array implementation evidence missing')
...
if (dynamicArrayFeature.read !== 'limited' || dynamicArrayFeature.view !== 'limited' || dynamicArrayFeature.edit !== 'limited') fail('X3-B2 array capability status drift')
