...
if ($LASTEXITCODE -ne 0) { throw "P1-B2B2 custom-protocol build failed" }
$output = Join-Path $workspace "docs\evidence\p1b2b2-pdf-form-copy"
if (Test-Path -LiteralPath (Join-Path $output "manifest.json")) {
  if (-not $Refresh) { throw "P1-B2B2 accepted evidence already exists; pass -Refresh to replace generated evidence" }
  $expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\p1b2b2-pdf-form-copy"))
  if ([IO.Path]::GetFullPath($output) -ne $expectedOutput) { throw "P1-B2B2 evidence path mismatch" }
  Remove-Item -LiteralPath $output -Recurse -Force
}
$appPort = 14200
$cdpPort = 14516
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "P1-B2B2 audit requires free ports $appPort and $cdpPort" }
$auditRoot = Join-Path $env:TEMP ("longedit-p1b2b2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$source = Join-Path $library "P1B2B2 AcroForm.pdf"
& node (Join-Path $workspace "scripts\create-p1b2a2-pdf-form-fixture.mjs") $source
if ($LASTEXITCODE -ne 0) { throw "P1-B2B2 fixture generation failed" }
...
try {
  ...
  try {
    ...
    & node (Join-Path $workspace "scripts\capture-p1b2b2-pdf-form-copy.mjs")
    if ($LASTEXITCODE -ne 0) { throw "P1-B2B2 capture failed" }
    if ($sourceHash -ne (Get-FileHash -LiteralPath $source -Algorithm SHA256).Hash) { throw "P1-B2B2 source fixture changed" }
...
Write-Output "P1-B2B2 PDF form copy audit completed: $output"
