...
replaceExact('package.json', /"version": "1\.0\.21"/g, '"version": "1.0.22"', 1)
replaceExact('package-lock.json', /"version": "1\.0\.21"/g, '"version": "1.0.22"', 2)
replaceExact('src-tauri/tauri.conf.json', /"version": "1\.0\.21"/g, '"version": "1.0.22"', 1)
replaceExact('src-tauri/Cargo.toml', /^version = "1\.0\.21"$/gm, 'version = "1.0.22"', 1)
replaceExact('src-tauri/Cargo.lock', /(name = "tauri-app"\r?\nversion = ")1\.0\.21("\r?\n)/g, '$11.0.22$2', 1)

for (const file of activeSharedFiles) {
  if (json(file).appVersion !== '1.0.21') throw new Error(`${file}: expected appVersion 1.0.21`)
  replaceExact(file, /"appVersion": "1\.0\.21"/g, '"appVersion": "1.0.22"', 1)
}
replaceExact('shared/p1-final-capability-closure.json', /"nextStage": "V1\.0\.21-UNSIGNED-PATCH-RELEASE"/g, '"nextStage": "V1.0.22-UNSIGNED-PATCH-RELEASE"', 1)

const development = json('shared/development-version-policy.json')
if (development.publicVersion !== '1.0.21' || development.runtimeBaseVersion !== '1.0.21' || development.developmentTargetVersion !== '1.0.22') throw new Error('Unexpected development baseline')
Object.assign(development, {
  runtimeBaseVersion: '1.0.22',
  releaseCandidate: false,
  currentStage: 'M8-14-v1.0.22-search-candidate',
  binaryVersionTransition: 'v1.0.22-quality-gate-pending',
  displayLabel: 'v1.0.22 搜索修复候选 · 当前公开 v1.0.21',
  activeSlice: { id: 'v1.0.22-search-scope-freeze', status: 'runtime-transitioned-candidate-validation-pending', document: 'docs/V1_0_22_Unsigned_Community_Release_Audit_2026-09-12.md' },
})
...
if (community.appVersion !== '1.0.21' || community.gates?.githubReleasePublished !== true || community.patchValidation?.managedUpdaterUpgradePath !== '1.0.20-to-1.0.21-passed') throw new Error('Predecessor release/update must be closed')
Object.assign(community, {
  appVersion: '1.0.22',
  releaseCandidate: false,
  generatedAt: '2026-09-12',
  currentStatus: 'v1.0.22-community-release-quality-gate-pending',
  gates: {
  ...
  patchValidation: {
    ...
    managedUpdaterUpgradePath: '1.0.21-to-1.0.22-pending',
    scope: 'search-recovery-and-compact-results',
  },
  targetRelease: {
    tag: 'v1.0.22',
    url: 'https://github.com/Longyuyeee/Long_MarkDownReader/releases/tag/v1.0.22',
  ...
  releaseWarnings: [
    ...
    'v1.0.21-to-v1.0.22-managed-update-observation-pending',
    'download-only-from-official-github-release',
    'verify-published-sha256-before-install',
  ],
  nextAction: 'pass-v1.0.22-quality-gate-and-build-real-candidate-installers',
})
...
console.log(`${process.argv.includes('--apply') ? 'Applied' : 'Dry run'}: ${planned.size} version identity files; public v1.0.21 remains frozen.`)
