...
if (library.includes('activeRelationSummary') || library.includes('active-relation-summary')) failures.push('the editor tab bar must not expose relation counts or isolation risk')
...
if (app.includes('has-relation-context') || /padding-right:\s*42px/.test(app)) failures.push('the closed relation context trigger must not reserve a full-height layout column')
requireText(contextPanel, '.relation-context-host { position: fixed;', 'G8-2 context trigger must float over the workspace edge')
requireText(contextPanel, '.relation-context-host.open { width: 326px; }', 'G8-2 context host must only gain panel width while open')
