...
$output = Join-Path $workspace 'docs\evidence\post-v115-m1dc1-subtitle-playback'
$appPort = 14200; $cdpPort = 14537
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1D-C1 audit requires free ports $appPort and $cdpPort" }
Remove-Item -LiteralPath $output -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $output -Force | Out-Null
if (-not $SkipBuild) {
  & npm run build
  if ($LASTEXITCODE -ne 0) { throw 'M1D-C1 production build failed' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'M1D-C1 Tauri build failed' }
}
$root = Join-Path $env:TEMP ("longedit-m1dc1-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M1DC1_LIBRARY=$library; $env:LONGEDIT_M1DC1_OUTPUT=$output
    & node (Join-Path $workspace 'scripts\capture-post-v115-m1dc1-subtitle-playback-audit.mjs')
    if ($LASTEXITCODE -ne 0) { throw 'M1D-C1 real desktop subtitle audit failed' }
...
Write-Output "M1D-C1 real subtitle playback audit completed: $output"
