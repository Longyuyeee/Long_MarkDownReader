param(
  [string]$OutputDirectory = "docs\evidence\c4a-pptx-edit-baseline"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c4a-pptx-edit-baseline"))
if ($output -ne $expectedOutput) { throw "C4A audit output must remain inside docs\evidence\c4a-pptx-edit-baseline" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C4A desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c4a-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C4A WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C4A_WPS = $fixture
    $env:LONGEDIT_C4A_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c4a-pptx-edit-baseline-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C4A desktop audit capture failed" }
  }
  finally {
...
Write-Output "C4A PPTX edit baseline desktop audit completed: $output"
