param(
  [string]$OutputDirectory = "docs\evidence\c5c-pptx-slide-lifecycle"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c5c-pptx-slide-lifecycle"))
if ($output -ne $expectedOutput) { throw "C5C audit output must remain inside docs\evidence\c5c-pptx-slide-lifecycle" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C5C desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c5c-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C5C WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C5C_WPS = $fixture
    $env:LONGEDIT_C5C_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c5c-pptx-slide-lifecycle-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C5C desktop audit capture failed" }
    $artifactDirectory = Join-Path $workspace "fixtures\pptx\output-reopen"
    New-Item -ItemType Directory -Path $artifactDirectory -Force | Out-Null
    foreach ($file in @(
      "c5c-add-copy.pptx",
      "c5c-copy-copy.pptx",
      "c5c-delete-copy.pptx",
      "c5c-reorder-copy.pptx"
    )) {
...
Write-Output "C5C PPTX slide lifecycle desktop audit completed: $output"
