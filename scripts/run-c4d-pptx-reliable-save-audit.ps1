param(
  [string]$OutputDirectory = "docs\evidence\c4d-pptx-reliable-save"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c4d-pptx-reliable-save"))
if ($output -ne $expectedOutput) { throw "C4D audit output must remain inside docs\evidence\c4d-pptx-reliable-save" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C4D desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c4d-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C4D WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C4D_WPS = $fixture
    $env:LONGEDIT_C4D_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c4d-pptx-reliable-save-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C4D desktop audit capture failed" }
  }
  finally {
...
Write-Output "C4D PPTX reliable save-copy desktop audit completed: $output"
