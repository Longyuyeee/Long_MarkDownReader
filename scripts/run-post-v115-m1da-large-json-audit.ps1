...
$output = Join-Path $workspace 'docs\evidence\post-v115-m1da-large-json'
$appPort = 14200; $cdpPort = 14534
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1D-A audit requires free ports $appPort and $cdpPort" }
Remove-Item -LiteralPath $output -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $output -Force | Out-Null
if (-not $SkipBuild) {
  & npm run build
  if ($LASTEXITCODE -ne 0) { throw 'M1D-A production build failed' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'M1D-A Tauri build failed' }
}
$root = Join-Path $env:TEMP ("longedit-m1da-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M1DA_LIBRARY=$library; $env:LONGEDIT_M1DA_OUTPUT=$output
    & node (Join-Path $workspace 'scripts\capture-post-v115-m1da-large-json-audit.mjs')
    if ($LASTEXITCODE -ne 0) { throw 'M1D-A real desktop audit failed' }
...
Write-Output "M1D-A real desktop audit completed: $output"
