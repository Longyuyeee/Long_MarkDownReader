param([switch]$SkipBuild)

$ErrorActionPreference = 'Stop'
$workspace = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace 'docs\evidence\post-v118-m7-2-local-json-schema-desktop'))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace 'docs\evidence\post-v118-m7-2-local-json-schema-desktop'))
if ($output -ne $expectedOutput) { throw 'M7-2 evidence output escaped the repository evidence directory' }
$appPort = 14200
$cdpPort = 14733
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw 'M7-2 audit requires free ports 14720 and 14733' }

if (Test-Path -LiteralPath $output) { Remove-Item -LiteralPath $output -Recurse -Force }
New-Item -ItemType Directory -Path $output -Force | Out-Null
if (-not $SkipBuild) {
  & npm run build
  if ($LASTEXITCODE -ne 0) { throw 'M7-2 frontend build failed' }
  $env:TAURI_CONFIG = Get-Content -LiteralPath (Join-Path $workspace 'src-tauri\tauri.e2e.conf.json') -Raw
  & cargo build --locked --manifest-path (Join-Path $workspace 'src-tauri\Cargo.toml') --bin tauri-app
  if ($LASTEXITCODE -ne 0) { throw 'M7-2 Tauri build failed' }
}

$auditRoot = Join-Path $env:TEMP ("longedit-m7-2-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
$library = Join-Path $auditRoot 'library'
$webview = Join-Path $auditRoot 'webview'
New-Item -ItemType Directory -Path $library,$webview -Force | Out-Null
$vite = Start-Process npm.cmd -ArgumentList 'run','dev','--','--host','127.0.0.1','--port',"$appPort" -WorkingDirectory $workspace -WindowStyle Hidden -PassThru
try {
  for ($i=0; $i -lt 300 -and -not (Get-NetTCPConnection -LocalPort $appPort -State Listen -ErrorAction SilentlyContinue); $i++) { Start-Sleep -Milliseconds 100 }
  $env:LONGEDIT_E2E_LIBRARY = $library
  $env:LONGEDIT_E2E_THEME = 'dark'
  $env:LONGEDIT_E2E_STYLE = 'sharp'
  $env:LONGEDIT_E2E_CODE_THEME = 'github-dark'
  $env:LONGEDIT_E2E_MOTION = 'reduced'
  $env:WEBVIEW2_USER_DATA_FOLDER = $webview
  $env:WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS = "--remote-debugging-port=$cdpPort --remote-allow-origins=*"
  $app = Start-Process (Join-Path $workspace 'src-tauri\target\debug\tauri-app.exe') -WorkingDirectory (Join-Path $workspace 'src-tauri') -WindowStyle Hidden -PassThru
  try {
    for ($i=0; $i -lt 300 -and -not (Get-NetTCPConnection -LocalPort $cdpPort -State Listen -ErrorAction SilentlyContinue); $i++) { Start-Sleep -Milliseconds 100 }
    $env:LONGEDIT_CDP_ENDPOINT = "http://127.0.0.1:$cdpPort"
    $env:LONGEDIT_M7_2_LIBRARY = $library
    $env:LONGEDIT_M7_2_OUTPUT = $output
    & node (Join-Path $workspace 'scripts\capture-post-v118-m7-2-local-json-schema-desktop.mjs')
    if ($LASTEXITCODE -ne 0) { throw 'M7-2 real desktop capture failed' }
  } finally {
    if ($app -and -not $app.HasExited) { Stop-Process -Id $app.Id -Force }
  }
} finally {
  if ($vite -and -not $vite.HasExited) { Stop-Process -Id $vite.Id -Force }
  Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }
  if (Test-Path -LiteralPath $auditRoot) { Remove-Item -LiteralPath $auditRoot -Recurse -Force }
}
Write-Output "M7-2 local JSON Schema desktop audit completed: $output"
