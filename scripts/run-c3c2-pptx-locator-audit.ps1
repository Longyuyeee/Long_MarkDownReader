param(
  [string]$OutputDirectory = "docs\evidence\c3c2-pptx-locator"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c3c2-pptx-locator"))
if ($output -ne $expectedOutput) {
  throw "C3C2 desktop audit output must remain inside docs\evidence\c3c2-pptx-locator"
}

$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) {
  throw "C3C2 desktop audit requires free local ports 9000 and 9333"
}
...
$auditRoot = Join-Path $env:TEMP ("longedit-c3c2-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
$pptxFixture = Join-Path $workspace "fixtures\pptx\producers\microsoft-powerpoint-16.pptx"
if (-not (Test-Path -LiteralPath $pptxFixture -PathType Leaf)) {
  throw "C3C2 Microsoft PowerPoint fixture is missing"
}
$auditPptx = Join-Path $library "C3C2 Search Target.pptx"
Copy-Item -LiteralPath $pptxFixture -Destination $auditPptx -Force
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C3C2_AUDIT_LIBRARY = $library
    $env:LONGEDIT_C3C2_AUDIT_PPTX = $auditPptx
    $env:LONGEDIT_C3C2_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c3c2-pptx-locator-audit.mjs")
    if ($LASTEXITCODE -ne 0) {
      throw "C3C2 desktop audit capture failed"
    }
  }
  finally {
...
Write-Output "C3C2 PPTX locator audit completed: $output"
