...
for (const key of ['graphHealthPanelVisible', 'centeredGraphNavigation', 'emptyStateGuidance', 'relationTypesDeterministic']) if (policy.productIntegration[key] !== true) failures.push(`G9 product integration must pass: ${key}`)
if (policy.productIntegration.workspaceHomeVisible !== false) failures.push('G9 workspace must not duplicate graph pulse')
...
for (const [source, token, message] of [
  ...
  [graphPanel, '知识网络脉搏', 'graph governance pulse UI missing'],
  [graphPanel, 'pulse.coveragePercent', 'coverage UI missing'],
  [graphPanel, "emit('focusNode', node.id)", 'centered topic navigation missing'],
]) requireText(source, token, message)
if (workspace.includes('data-testid="knowledge-network-pulse"')) failures.push('G9 pulse must not be duplicated on Workspace Home')
...
console.log('G9 knowledge network pulse passed: graph governance coverage, relation types, bounded top topics, and centered navigation are implemented without duplicating Workspace Home.')
