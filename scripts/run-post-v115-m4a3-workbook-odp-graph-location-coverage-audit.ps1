param([string]$OutputDirectory = "docs\evidence\post-v115-m4a3-workbook-odp-graph-location-coverage")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4a3-workbook-odp-graph-location-coverage"))
if ($output -ne $expectedOutput) { throw "M4A-3 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14534
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4A-3 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4a3-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\workbook\compatibility-baseline.xlsx") -Destination (Join-Path $library "M4A3 Workbook.xlsx")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\odf-content\longedit-e1c-presentation.odp") -Destination (Join-Path $library "M4A3 Slides.odp")
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4A3_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4A3_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4A3_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4a3-workbook-odp-graph-location-coverage-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4A-3 desktop capture failed" }
...
Write-Output "M4A-3 Workbook/ODP graph location audit completed: $output"
