...
const workflow = read('.github/workflows/v118-managed-updater-lifecycle.yml')
const probe = read('scripts/capture-v109-managed-updater-lifecycle.mjs')
const audit = read('docs/Post_v1.0.17_M6_8_v1.0.18_Official_Managed_Updater_Observation_Audit_2026-08-31.md')
...
if (policy.schemaVersion !== 1 || policy.stage !== 'V1.0.18-U1' || !['hosted-execution-pending', 'hosted-managed-update-passed'].includes(policy.status)) fail('v1.0.18 updater policy identity drift')
if (policy.releases?.previous?.version !== '1.0.17' || policy.releases?.previous?.tag !== previousReceipt.release?.tag || policy.releases?.previous?.installer?.fileName !== previousAsset?.name || policy.releases?.previous?.installer?.sizeBytes !== previousAsset?.sizeBytes || policy.releases?.previous?.installer?.sha256 !== previousAsset?.sha256) fail('previous official release drift')
if (policy.releases?.current?.version !== '1.0.18' || policy.releases?.current?.tag !== currentReceipt.release?.tag || policy.releases?.current?.url !== currentReceipt.release?.url || policy.releases?.current?.taggedCommit !== currentReceipt.release?.taggedCommit || policy.releases?.current?.installer?.fileName !== currentAsset?.name || policy.releases?.current?.installer?.sizeBytes !== currentAsset?.sizeBytes || policy.releases?.current?.installer?.sha256 !== currentAsset?.sha256 || policy.releases?.current?.standaloneReleaseExecutableReferenceSha256 !== currentInstalled.installedExecutable?.sha256 || policy.releases?.current?.installedPackageExecutable?.sizeBytes !== currentInstalled.installedExecutable?.sizeBytes) fail('current official release drift')
...
for (const token of ['workflow_dispatch:', 'runs-on: windows-latest', 'LONGEDIT_MANAGED_UPDATER_DISPOSABLE: "1"', 'shared\\v118-managed-updater-lifecycle-policy.json', 'gh release download $env:PREVIOUS_TAG', 'gh release download $env:CURRENT_TAG', 'releases/latest', 'refs/tags/${env:CURRENT_TAG}:refs/tags/${env:CURRENT_TAG}', 'run-v109-managed-updater-lifecycle.ps1', 'capture-v109-managed-updater-lifecycle.mjs', 'v118-managed-updater-lifecycle-${{ github.run_id }}']) if (!workflow.includes(token)) fail(`updater workflow missing: ${token}`)
...
const readmeVersion = ['1.0.19', '1.0.20', '1.0.21', '1.0.22', '1.0.23'].includes(development.publicVersion) ? development.publicVersion : '1.0.18'
...
if (!completed) {
  ...
  if (Object.values(policy.gates ?? {}).some(value => value !== true) || !Number.isInteger(policy.githubRun?.id) || policy.githubRun?.conclusion !== 'success' || !/^[0-9a-f]{40}$/.test(policy.githubRun?.headCommit ?? '') || policy.nextAction !== 'v1.0.18-release-and-managed-updater-closure-complete') fail('completed updater state drift')
  ...
  if (!fs.existsSync(manifestPath)) fail('v1.0.18 updater evidence is missing')
  else {
    const manifest = json(manifestPath)
    if (manifest.stage !== 'V1.0.18-U1I' || manifest.status !== 'accepted' || manifest.githubRunId !== policy.githubRun.id || manifest.artifactId !== policy.githubRun.artifactId || manifest.headCommit !== policy.githubRun.headCommit || manifest.previousVersion !== '1.0.17' || manifest.currentVersion !== '1.0.18' || manifest.officialInstallerSha256 !== policy.releases.current.installer.sha256 || manifest.installedExecutableSha256 !== policy.releases.current.installedPackageExecutable.sha256 || manifest.lifecycleChecks?.passed !== 12 || manifest.lifecycleChecks?.failed !== 0 || manifest.releaseMessaging !== 'official-published-copy-observed' || manifest.sourceUserContentIncluded !== false || manifest.files?.length !== 9 || manifest.files.filter(file => file.path.endsWith('.jpg')).some(file => file.visuallyReviewed !== true)) fail('v1.0.18 updater import drift')
    for (const file of manifest.files ?? []) {
      const evidencePath = `${root}/${file.path}`
      if (!fs.existsSync(evidencePath)) fail(`v1.0.18 updater evidence missing: ${file.path}`)
