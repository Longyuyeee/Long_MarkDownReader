...
$output = Join-Path $workspace 'docs\evidence\post-v115-m1dc-selection'
$appPort = 14200; $cdpPort = 14536
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M1D-C audit requires free ports $appPort and $cdpPort" }
Remove-Item -LiteralPath $output -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $output -Force | Out-Null
if (-not $SkipBuild) {
  & npm run build
  if ($LASTEXITCODE -ne 0) { throw 'M1D-C production build failed' }
  ...
  if ($LASTEXITCODE -ne 0) { throw 'M1D-C Tauri build failed' }
}
$root = Join-Path $env:TEMP ("longedit-m1dc-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M1DC_LIBRARY=$library; $env:LONGEDIT_M1DC_OUTPUT=$output
    & node (Join-Path $workspace 'scripts\capture-post-v115-m1dc-selection-audit.mjs')
    if ($LASTEXITCODE -ne 0) { throw 'M1D-C real desktop selection audit failed' }
...
Write-Output "M1D-C real selection audit completed: $output"
