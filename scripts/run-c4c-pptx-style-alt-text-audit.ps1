param(
  [string]$OutputDirectory = "docs\evidence\c4c-pptx-style-alt-text"
)

$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [System.IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [System.IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\c4c-pptx-style-alt-text"))
if ($output -ne $expectedOutput) { throw "C4C audit output must remain inside docs\evidence\c4c-pptx-style-alt-text" }
$busyPorts = Get-NetTCPConnection -LocalPort 9000, 9333 -State Listen -ErrorAction SilentlyContinue
if ($busyPorts) { throw "C4C desktop audit requires free local ports 9000 and 9333" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-c4c-{0}-{1}" -f $PID, [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "C4C WPS fixture is missing: $source" }
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_C4C_WPS = $fixture
    $env:LONGEDIT_C4C_AUDIT_OUTPUT = $output
    & node (Join-Path $workspace "scripts\capture-c4c-pptx-style-alt-text-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "C4C desktop audit capture failed" }
  }
  finally {
...
Write-Output "C4C PPTX style and alt-text desktop audit completed: $output"
