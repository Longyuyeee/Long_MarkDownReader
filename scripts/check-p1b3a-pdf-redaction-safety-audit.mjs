...
if (!['P1-B3A', 'P1-B3B', 'P1-B3C', 'P1-B3D', 'P1-B4A', 'P1-B4B'].includes(advanced.stage) || !['permanent-redaction-safety-audit-complete', 'permanent-redaction-backend-complete', 'permanent-redaction-workspace-complete', 'permanent-redaction-complete', 'watermark-safety-audit-complete', 'watermark-backend-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('permanent-redaction-copy') !== ['P1-B3C', 'P1-B3D', 'P1-B4A', 'P1-B4B'].includes(advanced.stage)) fail('P1-B3A advanced capability lineage is stale')
const slice = advanced.plannedSlices?.find(item => item.id === 'P1-B3')
if (!['safety-audit-complete-raster-backend-pending', 'raster-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending', 'completed'].includes(slice?.status) || slice?.deliveredContract !== 'shared/pdf-redaction-safety-contract.json' || slice?.currentWriteUserFile !== ['P1-B3C', 'P1-B3D', 'P1-B4A', 'P1-B4B'].includes(advanced.stage)) fail('P1-B3A implementation boundary is stale')
