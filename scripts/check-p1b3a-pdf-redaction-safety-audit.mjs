...
if (!['P1-B3A', 'P1-B3B', 'P1-B3C'].includes(safety.stage) || !['safety-audit-complete', 'raster-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending'].includes(safety.status) || safety.currentWriteCapability !== (safety.stage === 'P1-B3C') || safety.approvedArchitecture !== 'fresh-document-full-page-rasterization') fail('P1-B3A safety contract stage is stale')
...
if (!['P1-B3A', 'P1-B3B', 'P1-B3C'].includes(advanced.stage) || !['permanent-redaction-safety-audit-complete', 'permanent-redaction-backend-complete', 'permanent-redaction-workspace-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('permanent-redaction-copy') !== (advanced.stage === 'P1-B3C')) fail('P1-B3A advanced capability lineage is stale')
const slice = advanced.plannedSlices?.find(item => item.id === 'P1-B3')
if (!['safety-audit-complete-raster-backend-pending', 'raster-backend-complete-workspace-pending', 'workspace-complete-desktop-evidence-pending'].includes(slice?.status) || slice?.deliveredContract !== 'shared/pdf-redaction-safety-contract.json' || slice?.currentWriteUserFile !== (advanced.stage === 'P1-B3C')) fail('P1-B3A implementation boundary is stale')
...
for (const token of ['preview_pdf_redaction_copy', 'save_pdf_redaction_copy']) {
  ...
  if (safety.stage === 'P1-B3C' && (!commands.includes(token) || !tauri.includes(token) || !view.includes(token))) fail(`P1-B3C workspace command wiring is missing: ${token}`)
}
