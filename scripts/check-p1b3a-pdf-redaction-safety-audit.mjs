...
if (!['P1-B3A', 'P1-B3B'].includes(safety.stage) || !['safety-audit-complete', 'raster-backend-complete-workspace-pending'].includes(safety.status) || safety.currentWriteCapability !== false || safety.approvedArchitecture !== 'fresh-document-full-page-rasterization') fail('P1-B3A safety contract stage is stale')
...
if (!['P1-B3A', 'P1-B3B'].includes(advanced.stage) || !['permanent-redaction-safety-audit-complete', 'permanent-redaction-backend-complete'].includes(advanced.status) || advanced.currentCapabilities?.includes('permanent-redaction-copy')) fail('P1-B3A advanced capability must remain unavailable to users')
const slice = advanced.plannedSlices?.find(item => item.id === 'P1-B3')
if (!['safety-audit-complete-raster-backend-pending', 'raster-backend-complete-workspace-pending'].includes(slice?.status) || slice?.deliveredContract !== 'shared/pdf-redaction-safety-contract.json' || slice?.currentWriteUserFile !== false) fail('P1-B3A implementation boundary is stale')
