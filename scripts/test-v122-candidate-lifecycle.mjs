...
test('quality gate precedes build; installed search has exact independent evidence and publication stays closed', () => {
  ...
  assert.equal(policy.officialManagedUpdateObservation, 'passed-official-update-run-35745952274')
  const observation = JSON.parse(fs.readFileSync('shared/v122-managed-updater-lifecycle-policy.json'))
  assert.equal(observation.status, 'hosted-managed-update-passed')
  assert.equal(observation.githubRun.id, 35745952274)
  assert.equal(observation.githubRun.conclusion, 'success')
