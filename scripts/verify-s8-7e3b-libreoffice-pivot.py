...
def inspect_pivot(document):
    ...
    return sheet, pivot, {
        ...
        "keyCell": "D7",
        "keyValue": sheet.getCellRangeByName("D7").getValue(),
    }


def main():
    if len(sys.argv) != 4:
        raise RuntimeError("usage: verify-s8-7e3b-libreoffice-pivot.py PORT MODE XLSX")
    port = int(sys.argv[1])
    mode = sys.argv[2]
    ...
    try:
        ...
        if after["outputRange"] != "A3:D7" or after["keyValue"] != 4:
