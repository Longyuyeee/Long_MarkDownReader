...
if (manifest.stage !== policy.stage || manifest.status !== 'ready-to-publish-hosted-lifecycle-and-runtime-smoke-passed' || manifest.sourceCommit !== policy.candidateSourceCommit || manifest.sourceVersion !== '1.0.19') fail('M7-6 manifest identity drift')
...
if (community.currentStatus !== 'v1.0.19-community-release-ready-to-publish' || !community.releaseCandidate || community.release !== null || community.nextAction !== 'execute-m7-7-v1.0.19-tag-release-and-remote-asset-verification') fail('M7-6 community ready state drift')
if (development.currentStage !== 'M7-7-v1.0.19-tag-release-and-remote-asset-verification' || development.binaryVersionTransition !== 'v1.0.19-release-ready' || development.publicVersion !== '1.0.18') fail('M7-6 development handoff drift')
if (execFileSync('git', ['tag', '--list', 'v1.0.19'], { encoding: 'utf8' }).trim()) fail('v1.0.19 tag must not exist before M7-7 publication')
