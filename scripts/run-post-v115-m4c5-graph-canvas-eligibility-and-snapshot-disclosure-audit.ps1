param([string]$OutputDirectory = "docs\evidence\post-v115-m4c5-graph-canvas-eligibility-and-snapshot-disclosure")
$ErrorActionPreference = "Stop"
$workspace = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$output = [IO.Path]::GetFullPath((Join-Path $workspace $OutputDirectory))
$expectedOutput = [IO.Path]::GetFullPath((Join-Path $workspace "docs\evidence\post-v115-m4c5-graph-canvas-eligibility-and-snapshot-disclosure"))
if ($output -ne $expectedOutput) { throw "M4C-5 output must remain inside $expectedOutput" }
$appPort = 14200; $cdpPort = 14533
if (Get-NetTCPConnection -LocalPort $appPort,$cdpPort -State Listen -ErrorAction SilentlyContinue) { throw "M4C-5 requires free ports $appPort and $cdpPort" }
...
$auditRoot = Join-Path $env:TEMP ("longedit-m4c5-{0}-{1}" -f $PID,[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())
...
[IO.File]::WriteAllText((Join-Path $library "Graph Center.md"), "# Graph Center`n`n[[Graph Peer]]`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Graph Peer.md"), "# Graph Peer`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Data.csv"), "name,value`nalpha,1`n", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Data.tsv"), "name`tvalue`nbeta`t2`n", $utf8)
Copy-Item -LiteralPath (Join-Path $workspace "fixtures\post-v115-m4b0\workspace\M4B0 Tasks.table.json") -Destination (Join-Path $library "Data Board.table.json")
Copy-Item -LiteralPath (Join-Path $workspace "fixtures\post-v115-m0\workspace\research\paper.pdf") -Destination (Join-Path $library "Paper.pdf")
Copy-Item -LiteralPath (Join-Path $workspace "src-tauri\tests\fixtures\formats\mindmap.opml") -Destination (Join-Path $library "Outline.opml")
[IO.File]::WriteAllText((Join-Path $library "Other.json"), "{}", $utf8)
[IO.File]::WriteAllText((Join-Path $library "Existing.canvas"), '{"nodes":[],"edges":[]}', $utf8)
...
try {
  ...
  try {
    ...
    $env:LONGEDIT_CDP_ENDPOINT="http://127.0.0.1:$cdpPort"; $env:LONGEDIT_M4C5_AUDIT_OUTPUT=$output; $env:LONGEDIT_M4C5_AUDIT_LIBRARY=$library; $env:LONGEDIT_M4C5_SOURCE_COMMIT=$sourceCommit
    & node (Join-Path $workspace "scripts\capture-post-v115-m4c5-graph-canvas-eligibility-and-snapshot-disclosure-audit.mjs")
    if ($LASTEXITCODE -ne 0) { throw "M4C-5 desktop capture failed" }
...
Write-Output "M4C-5 graph Canvas eligibility and snapshot disclosure audit completed: $output"
