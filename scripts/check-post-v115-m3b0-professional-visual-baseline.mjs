...
requireFact(policy.preChange.edgeVisibilityZoomThreshold === 0.3 && graphView.includes('if (zoom > 0.3)'), 'edge visibility baseline drifted')
requireFact(policy.preChange.labelVisibilityZoomThreshold === 0.4 && graphView.includes('if (zoom > 0.4)'), 'label visibility baseline drifted')
requireFact(policy.preChange.glyphVisibilityZoomThreshold === 0.55 && graphView.includes('if (zoom > 0.55'), 'glyph visibility baseline drifted')
...
console.log(`M3B-0 professional visual baseline accepted: fixed zoom thresholds and missing camera/path affordances are recorded against safe 17/1000/5000-node evidence; next stage is ${policy.selectedNextStage.id} ${policy.selectedNextStage.name}${desktop ? ', with real Tauri screenshots' : ''}.`)
