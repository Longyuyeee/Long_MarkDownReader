...
export function verifyV123InstallerReceipt(receipt, policy, previousReceipt, readArtifact) {
  const fail = message => { throw new Error(`v1.0.23 installer receipt: ${message}`) }
  if (receipt.stage !== 'M8-19' || receipt.status !== 'hosted-installers-built'
    || receipt.candidateSourceCommit !== policy.candidateCommit
    || receipt.previousSourceCommit !== policy.previousTagCommit
    || receipt.appVersion !== '1.0.23' || receipt.previousVersion !== '1.0.22'
    || receipt.releaseCandidate !== false || receipt.sourceUserContentIncluded !== false) fail('candidate identity/privacy drift')
  const previous = previousReceipt.assets?.filter(a => a.name === 'LongEdit_1.0.22_x64-setup.exe') ?? []
  if (previousReceipt.release?.tag !== 'v1.0.22' || previousReceipt.release?.taggedCommit !== policy.previousTagCommit
  ...
  for (const artifact of artifacts) {
    const suffix = artifact.target === 'msi' ? '_1.0.23_x64_zh-CN.msi' : artifact.target === 'nsis' ? '_1.0.23_x64-setup.exe' : null
  ...
  return { status: 'installer-bytes-verified-only', candidateCommit: policy.candidateCommit, installersVerified: 3, installedLifecycleAccepted: false, versionIdentityAccepted: false }
}
