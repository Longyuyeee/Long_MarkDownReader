  ...
  <div class="table-view" tabindex="-1" @keydown="handleKeydown">
    <header class="table-toolbar">
      ...
      <div v-if="table" class="table-tools">
        <label class="table-filter"><span>⌕</span><input v-model="filterQuery" placeholder="筛选所有字段" @input="markViewDirty" /><button v-if="filterQuery" @click="clearFilter">×</button></label>
        <button v-if="activeViewKind === 'grid'" :class="{ active: freezeFirstColumn }" @click="toggleFreeze">冻结首列</button>
    ...
    <nav v-if="table?.format === 'longedit-table'" class="view-tabs" aria-label="数据视图">
      <button v-for="view in views" :key="view.id" :class="{ active: view.id === activeViewId }" @click="switchView(view.id)" @dblclick="renameView(view)">
        <span>{{ view.kind === 'grid' ? '▦' : view.kind === 'board' ? '▤' : view.kind === 'chart' ? '▥' : '▦' }}</span>{{ view.name }}
        <i v-if="views.length > 1" title="删除视图" @click.stop="deleteView(view)">×</i>
      </button>
      <div class="view-add">
...
<style scoped>
.table-view { height: 100vh; display: flex; flex-direction: column; overflow: hidden; color: var(--theme-text); background: color-mix(in srgb, var(--theme-bg) 94%, #dbe5ee); outline: none; }
...
.table-title,.table-tools { display: flex; align-items: center; gap: 8px; }.table-title > button,.table-tools > button { height: 32px; padding: 0 10px; border: 1px solid rgba(0,0,0,.1); border-radius: 7px; color: var(--theme-text); background: rgba(0,0,0,.035); cursor: pointer; }.table-title > button { width: 32px; padding: 0; font-size: 18px; }.table-title div { display: flex; flex-direction: column; }.table-title strong { max-width: 320px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 13px; }.table-title span { color: var(--theme-text-secondary); font-size: 9px; }.table-tools > button.active { color: var(--theme-primary); border-color: rgba(var(--theme-primary-rgb),.4); background: rgba(var(--theme-primary-rgb),.08); }.table-tools .save-button { min-width: 72px; color: #fff; border-color: var(--theme-primary); background: var(--theme-primary); }.table-tools .save-button:disabled { color: var(--theme-text-secondary); border-color: rgba(0,0,0,.08); background: rgba(0,0,0,.04); cursor: default; }
.table-filter { width: 220px; height: 32px; display: flex; align-items: center; gap: 5px; padding: 0 8px; border: 1px solid rgba(0,0,0,.1); border-radius: 7px; background: rgba(0,0,0,.025); }.table-filter input { min-width: 0; flex: 1; border: 0; outline: 0; color: var(--theme-text); background: transparent; font-size: 10px; }.table-filter button { border: 0; color: var(--theme-text-secondary); background: transparent; cursor: pointer; }
.view-tabs { min-height: 38px; display: flex; align-items: end; gap: 4px; padding: 5px 12px 0; border-bottom: 1px solid rgba(0,0,0,.09); background: color-mix(in srgb, var(--theme-card) 97%, #dce6ef); }.view-tabs > button,.view-add button { height: 32px; display: flex; align-items: center; gap: 5px; padding: 0 10px; border: 1px solid transparent; border-bottom: 0; border-radius: 7px 7px 0 0; color: var(--theme-text-secondary); background: transparent; cursor: pointer; font-size: 10px; }.view-tabs > button.active { color: var(--theme-primary); border-color: rgba(0,0,0,.1); background: var(--theme-card); }.view-tabs > button i { margin-left: 4px; font-style: normal; opacity: .4; }.view-tabs > button i:hover { opacity: 1; }.view-add { display: flex; margin-left: 5px; }.view-add button { height: 27px; padding: 0 7px; border-radius: 5px; }.view-add button:hover { color: var(--theme-primary); background: rgba(var(--theme-primary-rgb),.08); }.view-tabs > small { margin: 0 4px 9px auto; color: var(--theme-text-secondary); font-size: 8px; }
.table-workspace { min-height: 0; flex: 1; display: flex; flex-direction: column; }.table-meta-bar { height: 30px; flex: none; display: flex; align-items: center; gap: 14px; padding: 0 14px; border-bottom: 1px solid rgba(0,0,0,.07); color: var(--theme-text-secondary); background: color-mix(in srgb, var(--theme-card) 94%, transparent); font-size: 9px; }.table-meta-bar button { padding: 2px 6px; border: 0; color: var(--theme-primary); background: transparent; cursor: pointer; }.table-meta-bar i { margin-left: auto; font-style: normal; }
.table-meta-bar label { display: flex; align-items: center; gap: 4px; }.table-meta-bar select { height: 22px; max-width: 130px; border: 1px solid rgba(0,0,0,.1); border-radius: 4px; color: var(--theme-text); background: var(--theme-card); font-size: 9px; }.card-fields { display: flex; align-items: center; gap: 3px; }.card-fields b { font-weight: 500; }.card-fields button { border-radius: 4px; color: var(--theme-text-secondary); background: rgba(0,0,0,.035); }.card-fields button.active { color: var(--theme-primary); background: rgba(var(--theme-primary-rgb),.1); }
...
.header-cell,.data-cell,.row-number { min-width: 0; box-sizing: border-box; border-right: 1px solid rgba(0,0,0,.07); border-bottom: 1px solid rgba(0,0,0,.07); background: var(--theme-card); }.row-number { position: sticky; left: 0; z-index: 6; display: grid; place-items: center; padding: 0; border-top: 0; border-left: 0; color: var(--theme-text-secondary); background: color-mix(in srgb, var(--theme-card) 92%, #dce4ec); cursor: pointer; font-size: 9px; }.header-number { z-index: 25; cursor: default; }.header-cell { position: relative; display: grid; grid-template-columns: minmax(0,1fr) 24px; grid-template-rows: 27px 14px; padding: 3px 5px 2px; }.header-cell input,.data-cell input { width: 100%; min-width: 0; box-sizing: border-box; border: 0; outline: 0; color: var(--theme-text); background: transparent; font: inherit; }.header-cell input { font-size: 10px; font-weight: 700; }.header-cell > button { grid-column: 2; grid-row: 1; border: 0; border-radius: 4px; color: var(--theme-text-secondary); background: transparent; cursor: pointer; }.header-cell > button:hover { color: var(--theme-primary); background: rgba(var(--theme-primary-rgb),.08); }.header-cell small { grid-column: 1 / -1; grid-row: 2; color: var(--theme-text-secondary); font-size: 7px; }.data-cell { padding: 0 7px; }.data-cell input { height: 33px; font-size: 10px; }.data-cell:focus-within { position: relative; z-index: 4; outline: 2px solid var(--theme-primary); outline-offset: -2px; }.header-cell.frozen,.data-cell.frozen { position: sticky; left: 52px; z-index: 5; box-shadow: 2px 0 5px rgba(0,0,0,.08); }.header-cell.frozen { z-index: 24; }
