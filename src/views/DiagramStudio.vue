  ...
  <div class="diagram-studio" tabindex="-1" @keydown="handleKeydown">
    <header class="studio-toolbar">
      ...
      <div class="studio-actions">
        ...
        <button class="save-button" :disabled="!dirty || saving || !!parseError" @click="saveDiagram">{{ saving ? '保存中…' : dirty ? '保存' : '已保存' }}</button>
      </div>
    </header>
    ...
    <main v-if="loading" class="studio-state"><div class="loader"></div><strong>正在打开 Mermaid 图表</strong></main>
    <main v-else-if="loadError" class="studio-state error"><strong>无法打开图表</strong><p>{{ loadError }}</p><button @click="loadDiagram">重新加载</button></main>
    <main v-else class="studio-workspace" :class="{ 'with-inspector': showStructure }">
      <section class="source-panel">
        ...
        <button v-if="parseError" class="parse-error" @click="focusErrorLine">
          <strong>{{ errorLine ? `第 ${errorLine} 行` : '语法错误' }}</strong>
          <span>{{ parseError }}</span>
        </button>
        <footer v-else><span class="valid-dot"></span>语法有效<span v-if="rendering"> · 正在渲染…</span><i>{{ notice }}</i></footer>
      </section>
...
<style scoped>
.diagram-studio { height: 100vh; display: flex; flex-direction: column; overflow: hidden; color: var(--theme-text); background: color-mix(in srgb, var(--theme-bg) 95%, #dce7f1); outline: none; }.studio-toolbar { min-height: 58px; display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 0 16px; border-bottom: 1px solid rgba(0,0,0,.09); background: var(--theme-card); box-shadow: 0 2px 10px rgba(0,0,0,.05); z-index: 4; }.studio-title,.studio-actions { display: flex; align-items: center; gap: 7px; }.studio-title > button,.studio-actions > button { height: 31px; padding: 0 9px; border: 1px solid rgba(0,0,0,.1); border-radius: 7px; color: var(--theme-text); background: rgba(0,0,0,.03); cursor: pointer; }.studio-title > button { width: 31px; padding: 0; font-size: 18px; }.studio-title div { display: flex; flex-direction: column; }.studio-title strong { max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 12px; }.studio-title span,.studio-actions label { color: var(--theme-text-secondary); font-size: 9px; }.studio-actions label { display: flex; align-items: center; gap: 4px; }.studio-actions select { height: 29px; max-width: 125px; border: 1px solid rgba(0,0,0,.1); border-radius: 6px; color: var(--theme-text); background: var(--theme-card); font-size: 9px; }.studio-actions .zoom-value { min-width: 48px; }.studio-actions .save-button { min-width: 68px; color: #fff; border-color: var(--theme-primary); background: var(--theme-primary); }.studio-actions .save-button:disabled { color: var(--theme-text-secondary); border-color: transparent; background: rgba(0,0,0,.05); cursor: default; }
.studio-workspace { min-height: 0; flex: 1; display: grid; grid-template-columns: minmax(330px, 42%) minmax(0, 1fr); }.source-panel,.preview-panel { min-width: 0; min-height: 0; display: grid; grid-template-rows: 40px minmax(0,1fr) auto; }.source-panel { border-right: 1px solid rgba(0,0,0,.09); background: color-mix(in srgb, var(--theme-card) 97%, #edf2f7); }.preview-panel { grid-template-rows: 40px minmax(0,1fr); }.source-panel > header,.preview-panel > header { display: flex; align-items: center; justify-content: space-between; padding: 0 13px; border-bottom: 1px solid rgba(0,0,0,.07); background: var(--theme-card); }.source-panel header strong,.preview-panel header strong { font-size: 10px; }.source-panel header span,.preview-panel header span { color: var(--theme-text-secondary); font-size: 8px; }
.source-editor { min-height: 0; display: grid; grid-template-columns: 48px minmax(0,1fr); overflow: hidden; background: #18202b; }.source-editor pre { height: 100%; margin: 0; padding: 14px 10px; overflow: hidden; box-sizing: border-box; color: #66778b; border-right: 1px solid rgba(255,255,255,.07); text-align: right; font: 12px/22px 'Fira Code', monospace; user-select: none; }.source-editor textarea { width: 100%; height: 100%; padding: 14px; overflow: auto; resize: none; box-sizing: border-box; border: 0; outline: 0; color: #dbe7f3; background: transparent; caret-color: #7ea6ff; tab-size: 2; font: 12px/22px 'Fira Code', monospace; white-space: pre; }.source-panel footer { min-height: 32px; display: flex; align-items: center; gap: 6px; padding: 0 11px; color: var(--theme-text-secondary); background: var(--theme-card); font-size: 8px; }.source-panel footer i { margin-left: auto; font-style: normal; }.valid-dot { width: 7px; height: 7px; border-radius: 50%; background: #20a47b; }.parse-error { min-height: 54px; display: grid; grid-template-columns: auto minmax(0,1fr); align-items: start; gap: 8px; padding: 9px 12px; border: 0; border-top: 1px solid rgba(196,72,89,.25); color: #b53e50; background: rgba(196,72,89,.08); text-align: left; cursor: pointer; }.parse-error strong { font-size: 9px; }.parse-error span { overflow: hidden; font: 9px/1.5 'Fira Code', monospace; }
.preview-scroll { min-height: 0; display: grid; place-items: center; padding: 24px; overflow: auto; transform-origin: center; }.svg-stage { width: min(100%, 1200px); transform-origin: center; transition: transform .15s ease; }.svg-stage :deep(svg) { width: 100%; height: auto; max-height: calc(100vh - 150px); }.preview-empty { display: flex; flex-direction: column; align-items: center; gap: 7px; color: var(--theme-text-secondary); font-size: 10px; }.preview-empty strong { color: var(--theme-text); font-size: 13px; }.studio-state { flex: 1; display: grid; place-content: center; justify-items: center; gap: 10px; color: var(--theme-text-secondary); }.studio-state.error strong { color: #c44859; }.studio-state button { height: 30px; border: 1px solid rgba(0,0,0,.1); border-radius: 6px; color: var(--theme-text); background: var(--theme-card); cursor: pointer; }.loader { width: 24px; height: 24px; border: 2px solid rgba(var(--theme-primary-rgb),.2); border-top-color: var(--theme-primary); border-radius: 50%; animation: spin .7s linear infinite; }@keyframes spin { to { transform: rotate(360deg); } }
...
.export-panel { position: absolute; top: 52px; right: 84px; z-index: 20; width: 265px; display: grid; gap: 10px; padding: 13px; box-sizing: border-box; border: 1px solid rgba(0,0,0,.11); border-radius: 10px; background: var(--theme-card); box-shadow: 0 14px 38px rgba(0,0,0,.18); }.export-panel > header { display: flex; align-items: center; justify-content: space-between; }.export-panel > header div { display: flex; flex-direction: column; }.export-panel > header strong { font-size: 11px; }.export-panel > header span,.export-panel p { margin: 0; color: var(--theme-text-secondary); font-size: 8px; line-height: 1.5; }.export-panel > header button { width: 25px; height: 25px; border: 0; color: var(--theme-text-secondary); background: transparent; cursor: pointer; font-size: 16px; }.export-panel label { display: grid; grid-template-columns: 70px 1fr; align-items: center; color: var(--theme-text-secondary); font-size: 9px; }.export-panel select { height: 31px; border: 1px solid rgba(0,0,0,.1); border-radius: 6px; color: var(--theme-text); background: var(--theme-bg); font-size: 9px; }.export-confirm { height: 33px; border: 0; border-radius: 7px; color: #fff; background: var(--theme-primary); cursor: pointer; font-size: 9px; }.export-confirm:disabled { opacity: .45; cursor: default; }
.structure-panel { min-width: 0; min-height: 0; display: flex; flex-direction: column; border-left: 1px solid rgba(0,0,0,.09); background: var(--theme-card); }.structure-panel > header { min-height: 48px; display: flex; align-items: center; justify-content: space-between; padding: 0 10px 0 13px; border-bottom: 1px solid rgba(0,0,0,.07); }.structure-panel > header div { display: flex; flex-direction: column; }.structure-panel > header strong { font-size: 10px; }.structure-panel > header span { color: var(--theme-text-secondary); font-size: 8px; }.structure-panel > header button { width: 25px; height: 25px; border: 0; color: var(--theme-text-secondary); background: transparent; cursor: pointer; font-size: 16px; }.structure-tabs { display: grid; grid-template-columns: 1fr 1fr; padding: 8px 9px 0; }.structure-tabs button { height: 28px; border: 0; border-bottom: 2px solid transparent; color: var(--theme-text-secondary); background: transparent; cursor: pointer; font-size: 9px; }.structure-tabs button.active { color: var(--theme-primary); border-color: var(--theme-primary); }.structure-list { max-height: 34%; min-height: 100px; padding: 7px 9px; overflow: auto; border-bottom: 1px solid rgba(0,0,0,.07); }.structure-list > button { width: 100%; min-height: 43px; display: flex; flex-direction: column; justify-content: center; padding: 5px 8px; border: 1px solid transparent; border-radius: 7px; color: var(--theme-text); background: transparent; text-align: left; cursor: pointer; }.structure-list > button:hover,.structure-list > button.active { border-color: rgba(var(--theme-primary-rgb),.18); background: rgba(var(--theme-primary-rgb),.06); }.structure-list strong { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 9px; }.structure-list span,.structure-list p { color: var(--theme-text-secondary); font-size: 8px; }.structure-warning { padding: 7px 10px; color: #936219; background: rgba(225,161,48,.1); font-size: 8px; line-height: 1.5; }.structure-empty,.property-placeholder { padding: 22px 14px; color: var(--theme-text-secondary); text-align: center; font-size: 9px; line-height: 1.6; }.structure-empty strong { color: var(--theme-text); font-size: 10px; }.property-form { display: flex; flex-direction: column; gap: 11px; padding: 14px; overflow: auto; }.property-form header { display: flex; align-items: center; justify-content: space-between; }.property-form header strong { font-size: 10px; }.property-form header span,.property-form p { color: var(--theme-text-secondary); font-size: 8px; line-height: 1.6; }.property-form label { display: grid; gap: 5px; color: var(--theme-text-secondary); font-size: 8px; }.property-form input,.property-form select { width: 100%; height: 31px; padding: 0 8px; box-sizing: border-box; border: 1px solid rgba(0,0,0,.1); border-radius: 6px; color: var(--theme-text); background: var(--theme-bg); outline: none; font-size: 9px; }.property-form input:focus,.property-form select:focus { border-color: var(--theme-primary); }.property-form > button { height: 31px; border: 0; border-radius: 6px; color: #fff; background: var(--theme-primary); cursor: pointer; font-size: 9px; }.property-form > button:disabled { opacity: .45; cursor: default; }
@media (max-width: 1100px) { .studio-workspace.with-inspector { grid-template-columns: minmax(300px,40%) minmax(0,1fr); }.structure-panel { position: absolute; top: 58px; right: 0; bottom: 0; z-index: 10; width: 290px; box-shadow: -12px 0 30px rgba(0,0,0,.12); } }
@media (max-width: 900px) { .studio-actions label:nth-child(2),.studio-actions > button:not(.save-button):not(.structure-toggle):not(.export-toggle) { display: none; }.studio-actions > button.active { display: block; }.export-panel { top: 54px; right: 10px; }.studio-workspace,.studio-workspace.with-inspector { grid-template-columns: 1fr; grid-template-rows: 46% minmax(0,1fr); }.source-panel { border-right: 0; border-bottom: 1px solid rgba(0,0,0,.09); } }
</style>
