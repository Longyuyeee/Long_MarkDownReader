# v1.0.6 受控更新生命周期审计

状态：**GitHub 托管 Windows 真实更新已通过并导入证据**

## 审计目标

验证正式发布链上的 `v1.0.5 -> v1.0.6`，而不是重新构建或直接调用新安装器冒充自动更新：

1. 下载并安装官方 v1.0.5 NSIS。
2. 由已安装 v1.0.5 访问固定 GitHub 最新 Release，发现 v1.0.6。
3. 更新弹窗必须展示用户确认、SHA-256 与覆盖安装说明；点击前不得下载或启动安装器。
4. 点击“下载并安装”后，核对缓存文件名、字节数与 SHA-256，再等待静默覆盖安装。
5. 验证安装目录不变、v1.0.6 二进制与发布清单一致、首次启动成功，手动检查显示已经是最新版。
6. 验证合成资料库标记和应用配置标记在覆盖安装及最终卸载后仍然存在。

## 安全边界

- 本机当前正在运行的 v1.0.5 不参与测试，不关闭、不覆盖，也不读取用户资料。
- 生命周期脚本只有同时收到 disposable 环境变量和两个显式破坏性开关才允许执行安装器。
- GitHub Actions 只下载官方 v1.0.5/v1.0.6 Release 资产，不从当前源码重新打包。
- 测试使用固定的 `C:\LongEditManagedUpdater*` 合成目录，证据声明 `sourceUserContentIncluded=false`。
- 社区包仍为 `NotSigned`；该观察不能提升企业签名或 Authenticode 结论。

## 当前实现

- `.github/workflows/v106-managed-updater-lifecycle.yml`：下载官方资产、核对 GitHub digest/Tag、执行隔离生命周期并上传小型证据。
- `scripts/run-v106-managed-updater-lifecycle.ps1`：安装、覆盖、首启、当前版本、资料保留和卸载编排。
- `scripts/capture-v106-managed-updater-lifecycle.mjs`：通过安装态 WebView2/CDP 验证更新弹窗、用户点击与更新后设置页。
- `scripts/check-v106-managed-updater-lifecycle.mjs`：锁定发布资产、测试安全门和导入证据哈希。

## 首次托管执行观察

运行 `31402919779` 已通过官方附件元数据、更新弹窗、显式用户确认和实际覆盖安装，但在安装后 EXE 哈希检查处停止。原因是初版合同错误地要求 NSIS 落盘 EXE 必须与未公开发布的独立构建 EXE 逐字节相同；公开信任锚点实际是 GitHub Release 上的 NSIS 附件及其 SHA-256。修订后的 runner 仍严格验证官方 NSIS 哈希、安装版本与路径，同时独立记录落盘 EXE 的大小、SHA-256 和它与裸构建参考值的比较结果，不再混淆两个不同载体。

运行 `31403819178` 已继续通过 v1.0.6 安装、安装目录保持、版本、签名状态和落盘 EXE 身份记录；落盘 EXE 为 87,552,512 字节，SHA-256 为 `f448de6093afa3a30f281e39bb646e52cd3124c1f4f983ca205461f49c3f14c2`，状态为 `NotSigned`。该次在探针过早直接修改 hash 后等待设置页超时，因此第三版改为等待资料库壳层稳定并点击用户实际使用的设置入口，同时记录导航前后 hash。

运行 `31404864372` 进一步确认点击设置入口后路由已进入 `#/settings`；更新设置行仍未出现的原因是设置页默认停留在“资料库”分类，而软件更新组件只在“系统与更新”分类挂载。探针现已补齐点击“系统与更新”的用户步骤，再执行手动检查。

运行 `31405898071` 已完成 11/11 生命周期检查且无失败，证明正式 `v1.0.5 -> v1.0.6` 更新链、首启、已是最新版、覆盖与卸载后的合成资料保留均通过。人工复核发现更新后截图拍摄时内容仍处于进场动画或视口外，只显示“系统与更新”分类标题；为提高视觉证据质量，最终验收运行会先把更新设置行滚动到视口中央并等待可见后再截图。

最终运行 `31406703253` 在提交 `e96111db79aacc1c802936f73ddd1d834fdaa359` 上成功完成。11/11 生命周期检查通过，三张截图均已人工复核；更新后截图清楚显示“当前已是最新版本 v1.0.6”。原始证据与 SHA-256 导入清单位于 `docs/evidence/v1.0.6-managed-updater/`，且全部声明 `sourceUserContentIncluded=false`。

## 下一步

受控更新发布后观察已经收口。后续持续监测社区更新可用性，并恢复有界的 Excel/Office 等价能力、新文件格式编辑器与主题预设开发；社区包仍为 `NotSigned`，不得据此提升企业签名结论。
