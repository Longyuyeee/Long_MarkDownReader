...
当前 `calamine-ooxml-v4` 组合由 Calamine 承担读取、独立 `workbook_ooxml` 内容补丁层和 `workbook_styles` 样式层承担写回。前端可通过 `get_workbook_capabilities` 获取真实能力，不能根据页面控件推断兼容等级。

第四阶段已支持文本、数字、布尔和公式单元格的创建、编辑或清空、连续区域 TSV 剪贴板，以及基础字体、填充、边框、水平对齐、换行和常用数字格式。保存携带读取时的内容签名，冲突时拒绝覆盖；未编辑 OOXML 部件保持原始字节。公式重算和完整高级样式仍未进入 `supported`。
