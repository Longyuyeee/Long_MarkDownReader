...
### 2.7 知识库拖拽功能兼容性瓶颈 (Unresolved)
- **现象**：在 Windows 平台上，长按文件树节点移动时始终显示“禁止符号”，且控制台无任何 HTML5 Drag 事件输出。
- **深度分析**：
    - **窗口策略冲突**：Tauri 2.0 的 `transparent: true` 与 `decorations: false` 配置在 Windows WebView2 下会创建一个复杂的窗口层级，导致系统原生文件拖放处理器 (`dragDropEnabled`) 抢先拦截了所有鼠标长按移动事件。
    - **内核限制**：WebView2 在透明无边框模式下，DOM 层的原生 `dragstart` 事件由于无法确定窗口锚点，往往在 OS 层级就被丢弃。
- **已尝试对策（均未彻底解决）**：
    - 禁用 `dragDropEnabled` 配置。
