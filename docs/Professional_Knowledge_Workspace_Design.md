...
Canvas 对齐吸附、参考线、键盘微调、图谱布局持久化和 SVG/PNG 导出均已完成。

### 3.3.1 G7-2 图谱关系创作闭环（已实施）

- 全局图谱节点详情新增语义关系编辑器，可从当前 Markdown 节点选择目标笔记并建立 `parent`、`child`、`depends-on`、`related`、`contains`、`cites`、`derived-from` 关系。
- 用户创建的关系直接写入源 Markdown 的 `relations` Frontmatter；已有 Frontmatter 和正文保持原顺序、换行风格与内容，不建立私有图数据库事实源。
- 关系依据卡片可删除由 Frontmatter 声明的语义关系；普通正文 Wikilink、PDF `annotates` 边和自动发现关系不会被伪装成可删除对象。
- 后端只接受知识库内 Markdown 源和目标，拒绝自关系、越界路径、未知关系类型、Wikilink 不安全文件名与非 UTF-8 写回。
- 每个 Markdown 图谱节点携带内容签名；增删前验证签名，删除时再次核对原始行号、关系类型和 Wikilink 语法，外部修改后整次拒绝。
- 写回继续使用同目录临时文件、恢复备份和原子提升；回归覆盖 CRLF/正文保留、重复关系拒绝、陈旧签名和越界目标。

这一批把图谱从“关系结果查看器”推进为“关系创作入口”。G7-3 已进一步把 OPML、Canvas、PDF 批注和结构化数据接入细粒度跨格式节点关系；统一契约见 `Knowledge_Object_Relation_Contract.md`。
...
当前落地状态（S6-15）：CSV/TSV 已提供原生二维编辑、类型推断、排序、筛选、冻结首列和虚拟滚动，并以原文件作为唯一事实源。Open Table 1.0 使用 `.table.json`，以稳定行列 ID 保存 `data`，以独立 `views` 保存表格、看板、图表和仪表盘配置；规范见 `Open_Table_Format_Spec.md`。仪表盘可组合最多 24 个既有图表，共享筛选后的行索引，并持久化卡片顺序与 12 栅格宽度；专业图表及 Markdown/Canvas 实时引用仍直接消费同一数据源。XLSX 已进入原生渐进编辑并建立 `xlsx-progressive-editing-v1` 发布门禁：复合写回执行全包差异白名单，10,000×12 业务负载执行 Debug CI 性能预算，公开能力以分维度矩阵为准。完整 Excel 等价仍是长期目标，当前总体 XLSX 往返能力保持“计划”。
