...
## G15D installed entry closure

U2 recapture run `30695157895` reused the verified installers and passed in 2m16s. Its evidence reports `visualSurfaceSettled=true`, installed smoke `10/10`, and lifecycle `18/18`. Manual review accepted both screenshots: the focused Knowledge Network Anonymous Observation row, readable baseline/follow-up labels, and existing Settings typography, controls, spacing, and border treatment are visible in the same application window. The sanitized receipt is `docs/evidence/g15d-guidance-observation-entry/acceptance-receipt.json`. G15D is now `hosted-installed-observation-entry-passed-real-user-execution-next`; the next product step requires explicit consent to run a baseline-remediation-follow-up comparison against a real user library. Release candidate, signed-client evidence, and real-user completion remain false.

## G15E consented real-library session guidance

The next product slice adds a four-step guide directly beneath the existing anonymous observation row: explicitly preview/save a baseline, open Workspace guidance, explicitly acknowledge one completed remediation, then select the original baseline and preview/save a comparison. The guide stores only `{ schemaVersion, phase }` in `sessionStorage`; it does not persist a library name, path, fingerprint, content, file name, object ID, baseline path, or receipt destination. Export, remediation, file selection, confirmation, and upload boundaries remain unchanged and user-controlled. Frontend production build and G9–G15E contracts pass. The installed runner now covers reset, existing-baseline handoff, Workspace round-trip, explicit remediation acknowledgement, phase-only persistence, and comparison unlock without clicking either export action. Current status is `consented-real-library-session-runner-integrated-hosted-execution-next`; hosted installed acceptance is next, while real-user execution, signed-client evidence, and `releaseCandidate` remain false.

The first hosted run, `30695963197`, built both installers successfully but hit a startup race before UI validation: the installed smoke requested the WebView2 CDP page target once before it was advertised. No G15E interaction ran, so this is neither accepted product evidence nor a product failure. CDP discovery now polls for up to 30 seconds and reports the final discovery error. The next hosted run will reuse the verified installers from `30695963197`.

Reuse run `30696588758` passed installed smoke `11/11` and lifecycle `18/18`, but manual visual review rejected the ready-state screenshot: phase 3 unlocked comparison while still highlighting the remediation-confirmation row and leaving its acknowledgement button enabled. The UI now marks step 3 complete, disables repeated acknowledgement, and highlights step 4 as the next action. A rebuilt installed artifact is required before G15E closure.

## G15E installed session guidance closure

Full U2 run `30696762556` rebuilt the corrected product and passed installed smoke `11/11` plus lifecycle `18/18`. The accepted session stores only `phase` and `schemaVersion`, resumes in the same Settings window, unlocks comparison at phase 3, and records no export or automatic remediation. Manual review accepted the corrected visuals: step 1 is initially highlighted; after acknowledgement, step 3 is complete with its button disabled and step 4 is highlighted. The sanitized receipt is `docs/evidence/g15e-consented-real-library-session/acceptance-receipt.json`. G15E is now `hosted-installed-session-guidance-passed-consented-real-user-execution-next`; actual real-library execution, signed-client evidence, and release promotion remain false.

## G15F local comparison receipt review

G15F adds an in-app local review surface for previously saved anonymous comparison receipts. The Rust loader accepts only user-selected JSON up to 1 MiB, rejects unknown fields, requires the G15B privacy contract, validates bounded outcomes/achievements, and recomputes time and all aggregate changes before returning display data. Settings renders only validated aggregate metrics and never stores or displays the selected path. Frontend production build, G9–G15F product contracts, and the focused Rust regression pass; the regression now also rejects reversed timestamps and overflow-shaped aggregate counts without panicking. Current status is `local-comparison-receipt-review-source-validated-installed-acceptance-next`; installed local-receipt review is next, while real-user receipt review, signed-client evidence, and release promotion remain false.

The installed acceptance runner now reopens the generated synthetic receipt through the formal packaged Tauri command, verifies the returned aggregates, rejects an injected unknown path field, and requires the visible Settings review entry to remain in the current window without rendering or persisting the receipt path. Expected evidence is a focused entry screenshot plus a sanitized JSON receipt. Current status is `installed-local-receipt-review-runner-integrated-hosted-execution-next`; hosted execution and manual screenshot review remain pending, so `installedReviewComplete`, real-user receipt review, signed-client evidence, and release promotion remain false.

Full U2 run `30699010067` built both installers and reached the installed G15F check. The valid receipt, Settings entry, same-window boundary, and non-rendered/non-persisted path checks passed; the injected unknown-field receipt also rejected. The runner nevertheless failed because CDP surfaced a generic rejected-Promise error instead of the Rust Chinese validation text, leaving its boolean false. This is an acceptance-runner assertion defect rather than a product failure. The runner now accepts any rejection for that single-purpose invalid fixture, while the exact rejection reason remains covered by the Rust regression and source contract. Reuse of run `30699010067` installers is next; all completion and release claims remain false until that run passes.

Reuse run `30700011023` passed installed smoke 12/12 and the full install/upgrade/rollback lifecycle 18/18. The accepted G15F evidence confirms the formal packaged command reopened the anonymous receipt, returned `improved` with +1 relation and +1 connected object, rejected the unknown-field fixture, kept the path out of the page and Web Storage, and performed no upload. Manual review accepted the readable “审阅回执” entry, existing Settings visual style, and right-side current-window layout. The sanitized receipt is `docs/evidence/g15f-local-comparison-receipt-review/acceptance-receipt.json`. G15F is now `hosted-installed-local-receipt-review-passed-real-user-review-next`; real-user receipt review, signed-client evidence, and release promotion remain false.

## G16 knowledge isolation action queue

The next product audit found a concrete gap between guidance and action: Workspace could report isolated-object totals and recommend remediation, but it did not identify which objects to connect. G16 adds a deterministic, bounded queue of up to six isolated objects directly inside the existing knowledge-network pulse. Each item opens the existing right-side graph centered on that object; it does not create a relation, modify a file, add a tag, or collect new content. Two focused Rust regressions, the production build, and G9–G16 product contracts pass. Current status is `knowledge-isolation-action-queue-source-validated-installed-acceptance-next`; installed current-window centered-navigation acceptance follows before any completion claim.
