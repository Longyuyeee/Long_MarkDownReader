...
更新日期：2026-07-28
交接基线：当前开发版本 `v0.7.0`；A4/A5、G8-1、G8-2A、G8-2B、PDF B0～B2B、DOCX C0～C2E、PPTX C3A～C5D 已完成；下一批为 B2C PDF 插入页面
...
> 当前阶段交付证据见 `docs/B2B_PDF_Merge_Audit_2026-07-28.md`；B2B 已完成 2～16 个 Library PDF 的显式排序、隔离合并、可靠新文件保存、应用内重开和全部源文件不变门禁。下一批进入 B2C PDF 插入页面。
...
- PDF B0～B2B 已完成页面草稿、隔离验证、原子无覆盖另存、PDF.js 重开、复杂兼容矩阵、按范围提取和多文件有序合并；对象流、页面继承和扫描页进入安全子集，高风险对象稳定阻断，仍禁止覆盖源文件或已有目标。
...
当前权威顺序：**B2C PDF 插入页面 → JSON/JSONC 软件内创建 → WPS/OpenDocument/旧版 Office 格式与转换审计 → 统一发布矩阵**。PDF B2B 已完成多文件有序合并，DOCX C2 和 PPTX C5 已收口；高风险交换格式继续只生成可靠新副本。
...
PDF B2A/B2B 已完成：可按显式页范围提取页面，也可将 2～16 个 Library PDF 显式排序后合并为可靠新文件；复用 B0～B1C 页面计划、兼容画像与安全另存内核，并验证跨输入页序、文本、页面几何、应用内重开、全部源文件不变及复杂/加密输入阻断。下一批推进 B2C 插入页面；仍只创建新文件，不扩张到正文排版编辑。
