...
> **2026-08-21 v1.0.15 交互治理已完成真实桌面验收：** 顶部文档标签已从原生 `title` 升级为软件内完整路径提示；普通壳层右键不再出现 WebView 默认菜单，真实文本编辑区和文件树专用菜单仍保留正确能力。扩展扫描中的原生 `confirm`、`alert`、`prompt` 已全部迁移到应用内对话框。干净提交 `27cc5ac` 的 Tauri 验收确认提示表面、三类右键策略、原工作区结构和零运行时错误；证据不含用户内容。当前事实版本仍为 1.0.14，本轮只是 v1.0.15 候选范围，下一步先跑完整门禁和主题/窄屏抽样，再决定版本冻结。详见 [`V1_0_15_Interaction_Polish_Audit_2026-08-21.md`](./V1_0_15_Interaction_Polish_Audit_2026-08-21.md)。
