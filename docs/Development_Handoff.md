...
最新阶段审计、设计对齐和后续收尾顺序见 `docs/Development_Stage_Audit_2026-07-22.md`。当前工作区已完成 S6-8、T7-1、F7-1、S6-9、F7-2、S6-10～S6-15、G7-2、G7-3、I7-1、I7-2、W7-1～W7-3、S8-1A、S8-1B1、S8-1B2A～S8-1C 与 S8-2A；Table 子阶段还剩 S8-2B～S8-2C，下一入口为 S8-2B / FR-DATA-009。
...
3. FR-DATA-009 的 S6-11 与 S8-2A 已完成冻结窗格读取/可靠写回、最多 50,000 行的非破坏性排序筛选、Excel Table 结构呈现、普通 Table 创建/范围扩缩/列定义同步，以及字面量列表和基础标量数据验证；Table 生命周期、持久化排序条件与公式/自定义验证仍未开放，边界见 `XLSX_Public_Compatibility_Matrix.md`。
...
25. 下一批 S8-2B / FR-DATA-009：实现 Table 改名、样式选项与转换为普通区域/删除生命周期事务。
26. 完善 FR-BASE-005：增加关键保存流程 E2E、格式 fixture 和视觉回归门禁。
