...
- Rust：S8-1B1 后共 132 项功能与性能测试；新增 5 项迁移回归，严格 Clippy 通过。
- 本机 XLSX 性能回归为 `inspect=624 ms / page=14,412 ms / patch=6,762 ms / total=21,799 ms`，分页读取超过 Debug CI 的标准 runner 预算；机器相关性能继续以 GitHub Quality Gate 为发布裁决。
...
完整阶段审计、设计对齐和后续交错排期见 `docs/Development_Stage_Audit_2026-07-20.md`。当前工作区已完成 S6-8、T7-1、F7-1、S6-9、F7-2、S6-10～S6-15、G7-2、G7-3、I7-1、I7-2、W7-1～W7-3、S8-1A 与 S8-1B1；下一入口为 S8-1B2 / FR-DATA-009。
...
19. 下一批 S8-1B2 / FR-DATA-009：把同一内核接入整行插入删除的包级原子写回，覆盖合并、验证、筛选、冻结、名称、Table、图表、drawing anchors 和维度。
20. 完善 FR-BASE-005：增加关键保存流程 E2E、格式 fixture 和视觉回归门禁。
