...
第二轮托管运行 `33267563652` 成功构建双安装包，却发现治理面板首次以 `open=true` 挂载时不执行扫描的真实产品缺陷。最终纠偏产品提交 `3d7807d7c1bc899b86b57a0cae4cd264746fdfb6` 已让面板初始打开即加载，并重新通过完整 `ci:patch-release`、R5F 11/11、R5G 6/6 检查与 11/11 路由。当前无 `v1.0.16` Tag/Release，`releaseCandidate=false`，九道门禁仍为 4/9。唯一接续点是从该精确候选重跑托管 MSI/NSIS、升级/卸载、安装态路由与 I/O、管理备份和索引回滚；全部通过前不得进入 M4F-4。
