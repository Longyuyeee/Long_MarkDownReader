...
更新日期：2026-07-27
交接基线：当前开发版本 `v0.7.0`；A4/A5、G8-1、G8-2A、G8-2B、PDF B0/B1A/B1B/B1C、DOCX C0～C2E、PPTX C3A、C3B1 及 C3B2 已完成；当前直接在 `main` 推进，下一阶段为 C3B3 对象分级渲染与真实缩略图

> 最新桌面门禁基线为 36 项真实 Tauri 检查和 28 张截图；PPTX C3B2 已完成组合子对象展开/坐标变换、图片裁剪/透明度、常用颜色变换和混合文本安全降级，见 `docs/C3B2_PPTX_Group_Image_and_Color_Audit_2026-07-27.md`。复杂旋转/渐变、连接符/表格、真实缩略图、WPS Presentation 与 C3 索引/桌面收口仍未完成。下文较早的逐批记录保留为历史证据，不应覆盖最新结论。
...
- C3B2 已展开组合子对象并恢复组内坐标，解析图片裁剪/透明度和常用颜色变换；混合文本不再错误套用单一运行样式，而是带警告安全降级。C3B 整体仍为部分完成。
...
开始新功能前先更新路线图中的需求状态与验收条件。下一批直接在 `main` 推进 C3B3：线条/连接符/常用自由形状/基础表格的分级渲染、图形框架只读卡面、真实画布缩略图和视觉回归。具备 WPS Presentation 的环境应补齐第三生产者 fixture，之后完成 C3C 全局索引/管理闭环和 C3D 真实桌面三生产者收口，再进入 C4 编辑。每个阶段至少执行相关契约检查，高风险或发布候选执行 `npm run ci:check`，涉及桌面端注册或 Rust 命令变更时再执行完整 Tauri 构建。
