...
- `npm run ci:check`：通过。
- Rust：89 项测试通过，0 失败。
- 100 MiB PDF 范围读取基准：约 96 ms，仅读取约 256 KiB（目标小于 2 秒）。
...
3. 推进 FR-DATA-009：单元格/区域内容和基础样式已支持 OOXML 局部写回；下一批增加整行整列、多区域选择、填充柄和公式相对引用迁移。
...
- XLSX 当前已具备基础单元格、连续区域和基础样式编辑能力；合并区域只允许编辑左上角锚点，高级样式、行列结构和公式重算尚未完成，不能提前宣传为 Excel 等价编辑器；完整等价编辑按兼容矩阵逐项验收。
