...
> **2026-09-23 v1.0.23 已公开发布：** Tag `403a5122`，Release `393964632`，三附件公开回下载核对通过。README 与清单已同步；版本页面和安装生命周期已完成，不重建。下一步是独立官方 1.0.22→1.0.23 更新观察；下一开发目标 v1.0.24 优先关闭/未保存保护一致性，图谱局部说明遮挡已有直接证据待修。以下为历史过程。

> **2026-09-23 同包视觉复验收口：** `35758597675` 成功；3 张版本原图逐张查看，更新设置正文完整可见，实际 v1.0.23/无 DEV。初次空白图保留，不覆盖。发布清单已生成，社区候选 ready-to-publish；下一步仅发布固定 `403a5122` 的既有 MSI/NSIS/SHA256SUMS 并回下载核验，之后独立官方更新观察。不要重建或重新开始已完成的视觉专项。

> **2026-09-23 原图复审：** 运行 `35754325807` 已成功，安装包摘要通过，原始报告与截图已入库；但设置截图正文空白，暂不发布。已增加祖先透明度/动画结束的只读检查，准备通过 `v123-installed-visual-review.yml` 复用同一安装包核实，不重新构建、不修改产品样式。详见 [原图复审与接续](./V1_0_23_Installed_Visual_Review_2026-09-23.md)。下方“当前正在执行”为历史。

> **当前正在执行：** [v1.0.23 安装候选运行 35754325807](https://github.com/Longyuyeee/Long_MarkDownReader/actions/runs/35754325807)。产品 `403a5122`、编排 `00a0470a` 固定；先查询此运行并下载其原始附件，审阅三张版本截图与保存截图，再核对版本、摘要和全部检查结果。不要只根据工作流绿灯发布，不要重新触发同一构建。下一版仅有 [痛点计划](./After_v1_0_23_User_Pain_Priorities_2026-09-23.md)，尚未实现。

> **2026-09-23 安装候选已冻结：** 产品固定为 `403a5122d169dfb238cd0a424d83e356f644fb08`；新增独立 `v123-candidate-lifecycle.yml`，从官方 v1.0.22 下载并核验旧 NSIS，重新构建候选 MSI/NSIS，在一次性 Windows 执行既有 22/18/11/7 生命周期及新增 4 项安装态版本检查。编排与产品提交分离，10 项编排/错误观察回归通过。尚无安装通过结果，不允许发布。运行号以候选政策后续记录为准；不要重复触发。

> **2026-09-23 已迁移到 v1.0.23 候选：** 44 项当前运行身份已同步，公开稳定版及其 Tag/附件仍为 v1.0.22。历史检查只补充对 1.0.23 候选身份的识别，没有改写历史证据，也没有继承安装通过。6 项迁移回归、当前开发审计、发布事实合同及完整 `ci:patch-release` 已通过（生产依赖 0 漏洞）；安装包尚未生成、Release 未发布。接续见 [候选审计](./V1_0_23_Unsigned_Community_Release_Audit_2026-09-23.md)：冻结迁移提交、创建 v1.0.23 专属候选构建/安装流程，不复用 v1.0.22 固定候选策略，不重跑五项已收口页面观察。下方“未升版”均为历史状态。

> **2026-09-23 故障页面观察已收口：** 隔离原生窗口五项故障/恢复场景全部完成，5 张未修改截图、5 份无障碍记录及哈希已入库。仅外部 API 返回值注入，产品状态逻辑未替换、没有执行安装；不能冒充真实断网或系统浏览器故障。测试窗口和专项服务已关闭，原已安装应用未动。1.0.23 范围冻结为版本身份与发布页错误隔离；下一步原子版本迁移、固定候选打包及安装验收，仍未升版/发布。下方等待恢复窗口是历史状态，见 [故障状态专项](./V1_0_23_Fault_State_Observation_2026-09-23.md)。

> **2026-09-23 外部故障专项准备：** 新增仅用于独立开发实例的外部 API 故障注入环境，禁止安装、拒绝生产构建；8 项隔离测试、13 项产品回归及当前开发审计通过。实际点击因测试窗口最小化与工具恢复循环暂停，用户已同意继续，仍待手动恢复测试窗口后观察五项场景。未升版、未发布，见 [故障状态专项](./V1_0_23_Fault_State_Observation_2026-09-23.md)。

> **2026-09-23 本机验证已恢复：** 无需换电脑，正式 computer-use API 已完成生产/开发前端三处版本一致性观察，以及打开官方发布页后不误报更新。5 张原图、7 份无障碍观察和哈希清单已归档。隔离标识下的 Rust debug 程序不是安装包；剩余失败/更新可用状态及安装验收继续在本机推进。见 [恢复方法与事实边界](./V1_0_23_Local_Validation_Recovery_2026-09-23.md)。下方“需要更换验证环境”为历史结论，不再适用。

> **最新补充：** 网页打开失败现与更新失败彻底分离，保留更新弹窗、检查状态和安装进度，两处入口提供独立错误提示；13 项回归通过。实际页面/安装仍未验收，1.0.23 未发布。接下来需确认可用的页面验证环境，不再把新增单元测试作为发布进度，见 [当前切片](./V1_0_23_Version_Identity_Slice_2026-09-22.md)。

> **2026-09-22 补充纠偏：** 继续修复 `openLatestRelease` 打开发布页后误报“有更新”、覆盖并行检查的问题，版本身份回归共 11 项通过。本机页面观察因浏览器启动被执行策略拒绝未完成，不能据此发布 1.0.23。下一步在允许的验证环境观察三个版本入口及“已是最新→打开发布页”场景；源码测试不是页面/安装证据。详见 [本轮审计](./V1_0_23_Version_Identity_Slice_2026-09-22.md)。

> **2026-09-22 当前开发接续：** 1.0.23 首个切片已修复版本身份取值，侧栏/能力页/设置共用实际运行版本，DEV 仅开发构建显示；8 项回归和完整补丁门通过。运行/公开仍为 1.0.22，没有替换其安装包。下一步仅实际页面及生产构建验证，随后冻结/迁移/打包 1.0.23。不要把代码测试写成新安装验收，见 [切片审计与接续](./V1_0_23_Version_Identity_Slice_2026-09-22.md)。

> **2026-09-22 更新观察收口：** 官方 1.0.21→1.0.22 运行 `35745952274` 成功，12/12、三张原图审阅完成，9 个原始证据文件入库。版本、包摘要、自动重启、资料保留均已核对；这是安装态自动化，不冒充物理输入回放。v1.0.22 发布和更新全部收口，下方“更新观察未完成”均为历史。见 [正式更新收口](./V1_0_22_Managed_Update_Closure_2026-09-22.md)。

> **2026-09-22 最新：v1.0.22 已正式发布并回下载验真。** Release `393850025`，Tag 指向固定产品 `cc58aa68`，三个公开附件大小/SHA-256 均与验收包一致；MSI/NSIS 为 NotSigned。README、发布说明、清单和原始报告已同步。最终提示观察 `35744511959` 显示图谱加载约 552ms 消失、旧消息约 805ms 清空，本环境未复现持续遮挡。已知静态 DEV/旧公开版本文案如实披露，下一目标 [v1.0.23](./V1_0_23_User_Task_Plan_2026-09-22.md)优先修复运行身份显示。**唯一发布后未完成项是官方 1.0.21→1.0.22 应用内更新观察**，不得用已通过的候选安装流程替代；无需重跑搜索或重建 1.0.22。下方“未发布”为历史。

> **2026-09-22 最新接续——安装态搜索已收口：** 固定候选 `cc58aa68` 的独立运行 `35743671363` 成功。搜索 4/4，安装生命周期 22/22、工作面 18/18、路由 11/11、管理回滚 7/7；8 张搜索原图逐张查看并原样入库。键盘重试、查询保留、准确打开及源文件不变均有独立证据，不再按下方历史记录重复测试。测试中的控件、Enter 默认动作、Markdown 容器及 Windows 扩展路径解析错误均已纠正，失败证据不改写。下一步是发布证据清单和最终审计、README/Tag/Release 与回下载核验，随后官方更新观察；**公开版仍为 1.0.21，本轮没有发布 1.0.22**。详见 [最终专项结果](./V1_0_22_Installed_Search_Execution_2026-09-22.md)。

> **2026-09-22 安装态搜索专项转入隔离自动执行：** 新专项复用已核验候选包，使用暗色正式配置和鼠标/键盘事件验证离线重试、近似长标题与正确打开，不再依赖本机桌面控制。产品候选和原测试门槛不改，结果待远端执行及截图审阅。详见 [搜索专项执行](./V1_0_22_Installed_Search_Execution_2026-09-22.md)。

> **2026-09-22 发布前原图审阅推进：** 修复候选 14 张截图现已全部实际查看并保存（含先前 TXT）。记录跨文件保存提示、图谱构建提示的时序待核对，不把静态图审阅等同交互通过。下一步仅补安装态搜索/键盘及上述时序观察；未通过前不发布。详见 [用户任务计划第三节](./Development_User_Task_Plan_2026-09-22.md)。

> **2026-09-22 当前审计与用户任务计划：** 修复候选 `cc58aa68` 的运行 `34679919233` 已成功；三包实际字节与无签名状态核对通过，原始报告及 TXT 截图已入库。已直接查看 TXT“文本已安全保存／已同步”，不再沿用旧候选报错。其余视觉审阅及搜索安装态专项仍待完成；本轮 debug 键盘操作受 computer-use 工具错误阻断，未判通过。唯一近期接续仍是 v1.0.22 收口，后续按近似文件辨认、图谱恢复、工作簿、迁移交付排序，见 [用户任务计划与证据](./Development_User_Task_Plan_2026-09-22.md)。以下“运行中”条目为历史。

> **2026-09-12 修复候选重新冻结：** 当前安装验收候选改为 `cc58aa68d1974ac2445ed4b884c7677f5a320e93`；旧候选 `ada0cd60` 与拒绝原因保留在政策历史，不继承安装通过。下一步只触发一次新候选托管双包与安装生命周期，随后直接检查原始保存截图并完成独立搜索专项；当前仍不具备发布资格。运行号以后续登记为准，见 [托管交接](./V1_0_22_Candidate_Lifecycle_Handoff_2026-09-12.md)。

> **2026-09-12 保存修复的本地真实桌面复验完成：** 在 `cc58aa68` 的独立 debug 资料库中，用鼠标、Ctrl+S 和普通工作语气输入验证保存切页返回、未保存草稿保留、显式写盘及预算文件不变；两张实际截图已入库。完整补丁门通过（6,280 模块、Rust check、生产依赖 0 漏洞）。这不是安装态验收，也未在原生操作中触发慢回执竞争；旧候选仍拒绝。下一步固定修复后候选并重做托管双包、保存和搜索安装态验收，详见 [桌面复验与边界](./V1_0_22_Text_Save_Desktop_Review_2026-09-12.md)。

> **2026-09-12 保存后切页问题已作代码修复，仍待真实操作验收：** 从旧候选实际函数复现 null-state 报错；新实现隔离旧回执、同步原标签并在立即返回时等待保存，保留继续编辑的草稿。8 项回归、构建和当前总审计通过；安装脚本保留立即切页场景并增加保存反馈检查，不加等待凑通过。旧候选仍拒绝发布，详见 [保存生命周期修复](./V1_0_22_Text_Save_Lifecycle_Correction_2026-09-12.md)。

> **2026-09-12 重要：v1.0.22 当前候选不予发布。** 远端安装流程虽成功且三包摘要通过，人工查看 TXT 原始截图发现“保存失败：Cannot read properties of null (reading 'state')”及未保存标识，自动化未捕获。保留原图，不改结果凑通过。下一步复现“保存后立即切走再返回”的用户操作，修复保存生命周期后生成新候选；搜索实际场景仍未验收。详见 [用户场景审查与阻断](./V1_0_22_User_Scenario_Review_2026-09-12.md)。

> **2026-09-12 候选制品身份核对工具已完成：** 远端 `34676269824` 仍待完成；新增只读回执/三包字节校验，10 项回归及总审计通过，不把校验成功提升为安装或搜索通过。下一次仍先查同一运行，下载完整 artifact 后按 [托管验收交接](./V1_0_22_Candidate_Lifecycle_Handoff_2026-09-12.md)执行校验；不要重复启动打包。

> **2026-09-12 v1.0.22 托管安装流程已建立：** 固定产品候选 `ada0cd60…`；新工作流构建 MSI/NSIS，并以下载且校验摘要的官方 v1.0.21 NSIS 为升级基线，不再重建旧版。4 项工作流/PowerShell 语法测试通过；安装结果、搜索专项及发布后官方更新仍待验收。接续与证据合同见 [固定候选托管验收](./V1_0_22_Candidate_Lifecycle_Handoff_2026-09-12.md)。

> **2026-09-12 v1.0.22 运行身份迁移完成：** 44 个活动身份文件已同步到 1.0.22，公开版仍为 v1.0.21；新身份完整补丁门通过（6,279 模块、Rust locked check、生产依赖 0 漏洞），迁移 6 项回归与总审计通过。历史 Tag/证据不变，新版安装/发布门禁不继承旧版通过状态。下一步核对远端 CI，固定候选并建立/执行 v1.0.21 → v1.0.22 托管安装生命周期及搜索安装态验收；不得重复运行迁移脚本。详见 [v1.0.22 候选迁移审计](./V1_0_22_Unsigned_Community_Release_Audit_2026-09-12.md)。

> **2026-09-12 v1.0.22 迁移前门禁纠偏完成：** 修复旧版观察检查锁死当前运行版本、未发布候选却要求官方更新 passed 的矛盾；保留历史证据哈希检查，旧观察与新版路径分离。29 项状态回归、两个专用检查与当前总审计通过。运行版本仍为 1.0.21；下一步继续原子身份迁移，不重做产品功能。详见 [发布门禁分离审计](./Post_v1.0.21_v1.0.22_Release_Gate_Separation_2026-09-12.md)。

> **2026-09-12 v1.0.22 范围冻结与质量审计完成：** 本版仅包含搜索故障恢复与紧凑结果可读性。产品基线完整补丁门禁通过，Rust 功能 560 通过 / 5 忽略、独立性能 1/1、生产依赖 0 漏洞；当前阶段与检查器已纠偏，纠偏后总审计通过。下一步为原子版本迁移及固定候选安装包验收，不继续追加搜索功能。当前公开/运行时仍为 v1.0.21，尚无 v1.0.22 安装包、Tag 或 Release。具体步骤和证据边界见 [范围冻结与发布准备](./Post_v1.0.21_v1.0.22_Scope_Freeze_and_Readiness_2026-09-12.md)。

> **2026-09-12 紧凑搜索结果布局已收口：** 标题、命中信息、关系按钮改为分行，长标题两行且保留完整名称提示；真实约 200px Tauri 侧栏显示与点击打开已验证，3 项布局契约、生产构建和总审计通过。下一步进入 v1.0.22 有限范围冻结/发布就绪审计，不继续无限扩展功能。详见 [紧凑搜索结果审计](./Post_v1.0.21_Compact_Search_Result_Layout_Audit_2026-09-12.md)。版本未提升。

> **2026-09-12 搜索恢复有界验收已收口：** 真实 Tauri 普通搜索与标签搜索均完成“目录离线报错 → 恢复 → 点击重试命中”；标签后端根目录失败返回空数组的问题已纠偏。Vue 14/14、Rust 搜索 4/4、当前总审计通过，脱敏截图与回执已入库。下一步审计紧凑侧栏搜索结果标题/徽标拥挤问题，详见 [桌面验收与接续](./Post_v1.0.21_Search_Recovery_Desktop_Audit_2026-09-12.md)。版本未提升。

> **2026-09-12 v1.0.22 首个搜索恢复切片已实现：** 搜索异常不再冒充空结果，新增重试及请求隔离；9 项回归、类型检查、前端构建与当前开发总审计通过，组件明暗预览已实际查看。完整 Tauri 工作面集成验收仍待执行，不能标为发布就绪。接续见 [搜索恢复切片](./Post_v1.0.21_Search_Recovery_Slice_2026-09-12.md)。运行时/公开版仍为 1.0.21，不重复更新链验收。

> **2026-09-12 M8 正式收口：** 完整 Quality Gate `34674531897` 成功，官方 v1.0.20 → v1.0.21 更新观察 `34674860989` 一次通过 12/12；三张截图已直接查看，九份脱敏证据及导入清单已保存。专用政策、社区政策和发布回执现有真实证据支持 passed。版本仍为 1.0.21；下一步为 v1.0.22 跨格式检索切片范围选择，不再重复 M8 更新观察。详见 [M8-12E 收口与下一接续点](./Post_v1.0.21_M8_12E_Managed_Update_Closure_2026-09-12.md)。以下 pending 条目均为此前历史。

> **2026-09-12 M8-12D 导入器补强：** 证据先验证后写入，新增必须提供的三张截图摘要绑定审阅记录；9 项导入回归通过并接入总审计。正式更新仍待执行，导入命令须使用新增第七参数，详见 [M8-12D 审计](./Post_v1.0.21_M8_12D_Evidence_Import_Safety_2026-09-12.md)。

> **2026-09-12 M8-12C 状态纠偏：** 社区政策、仓库发布回执已与专用更新观察政策统一为 pending；新增 9 项状态组合回归并接入总审计。既有 Release、Tag、安装包不变。详见 [M8-12C 审计](./Post_v1.0.21_M8_12C_Updater_Status_Correction_2026-09-12.md)。下一步核对远端 CI 后执行正式托管观察，尚未关闭 M8。

> **2026-09-12 最新开发审计与后续路线：** 已拉取并核对 `4fa36ea7…`，公开/运行时仍为 v1.0.21，HEAD Quality Gate 成功，当前开发总审计通过；M8-12B 专用更新观察仍无运行，不能按部分发布记录中的 passed 宣布关闭。后续建议依次评估跨格式检索定位、图谱恢复与可读性、工作簿性能兼容、配置诊断和项目导出；均为候选而非已实现或冻结排期。详见 [当前开发审计与后续版本升级方向（2026-09-12）](./Development_Status_and_Upgrade_Directions_2026-09-12.md)。旧条目保留作历史，不覆盖此处当前状态。

> **2026-09-03 M8-12B 暂停交接：** 当前 `main` 已推进至 `bbe4c035…`，M8-12A Quality Gate 历史标签/Node 22 检查器纠偏及 M8-12B 官方更新观察合同均已推送；远端 Quality Gate `33760803962`、`33762171375` 均成功。`V1.0.21 Managed Updater Lifecycle` 尚未触发、运行记录为空、证据尚未导入，政策仍为 `hosted-execution-pending`；v1.0.22 产品开发尚未开始。下一台电脑唯一接续点是只触发一次公开 v1.0.20 → v1.0.21 托管应用内更新观察，12/12 后人工复核并导入证据。详见 [`Development_Handoff_v1.0.21_M8-12B_2026-09-03.md`](./Development_Handoff_v1.0.21_M8-12B_2026-09-03.md)。
...
> **2026-08-31 M5-4 v1.0.17 质量债务与发布就绪审计已通过：** Workbook 对有值区之外 `D6:E6` 合并区和 `D9 #FFF2CC` 空白样式的产品复读缺陷、PPTX 过期 `Copy` 断言以及知识索引监视器时序竞争均已修正。逐项 4/4 通过，全仓 Rust 为 548 通过、0 失败、5 忽略；完整 `ci:patch-release`、前端 6,275 模块、43 格式/91 扩展名、Rust 编译和生产依赖 0 漏洞全部通过。唯一接续点为 **M5-5 v1.0.17 原子版本迁移与候选安装包构建**；先更新版本事实源并重新过门，再构建真实 MSI/NSIS、记录 SHA-256 与 `NotSigned`，不得跳过安装生命周期直接发布。详见 [`Post_v1.0.16_M5_4_v1.0.17_Quality_Debt_and_Release_Readiness_Audit_2026-08-31.md`](./Post_v1.0.16_M5_4_v1.0.17_Quality_Debt_and_Release_Readiness_Audit_2026-08-31.md)。

> **2026-08-31 M5-3 ODP 简单页正文副本工作区已通过：** 资料库内 ODP 现按安全库存选择简单正文，支持内存草稿、撤销/重做、路由离开与重新读取保护、可靠新副本和自动打开；自定义形状页整页阻断并提供清晰说明，外部 ODP 仍只读。LibreOffice/PowerPoint 两份真实源各 2 个候选，桌面副本为 12,108/43,938 bytes，均通过 PowerPoint 语义复开和 LibreOffice 15,900/14,109 bytes PDF 渲染，源摘要不变、运行时错误 0。格式事实源提升为 `basic-edit / copy`，但版本仍为 1.0.16；全仓 Rust 最新结果为 544 通过、4 失败、5 忽略，其中 3 项独立复跑稳定失败、知识索引项单独复跑通过但存在间歇性。唯一接续点是 **M5-4 v1.0.17 质量债务与发布就绪审计**，清零这些问题后再决定是否打包与发布。详见 [`Post_v1.0.16_M5_3_ODP_Simple_Slide_Body_Copy_Workspace_Audit_2026-08-31.md`](./Post_v1.0.16_M5_3_ODP_Simple_Slide_Body_Copy_Workspace_Audit_2026-08-31.md)。

> **2026-08-31 M5-2 ODP 简单页正文可靠副本基础已通过：** 后端已实现库存、复杂对象整页阻断、摘要锁、`content.xml` 单部件补丁、可靠新文件和拒绝覆盖。LibreOffice/PowerPoint 真实简单源各 2 个候选，PowerPoint 复杂形状源 0 候选、1 阻断页；两份副本均通过 PowerPoint 语义重开与 LibreOffice 渲染，备注/稳定正文保持、原正文消失、源摘要不变。实际测试修正了 XML 实体误判和 PowerPoint 单一样式 span 包装。唯一接续点为 **M5-3 ODP 简单幻灯片正文副本工作区与真实桌面审计**；外部 ODP 保持只读，M5-3 前不提升注册表或打包 1.0.17。详见 [`Post_v1.0.16_M5_2_ODP_Simple_Slide_Body_Reliable_Copy_Foundation_Audit_2026-08-31.md`](./Post_v1.0.16_M5_2_ODP_Simple_Slide_Body_Reliable_Copy_Foundation_Audit_2026-08-31.md)。
...
## 2026-08-29 M4C-3 图谱派生输出选择交接

M4C-3 已完成，未修改产品运行代码。真实 Tauri 通过实际按钮创建并自动打开首个 Canvas 与项目笔记，并用命令生成编号碰撞目标；Canvas 复读为 2 个相对文件节点/2 条保留类型与方向的关系，项目笔记含生成元数据、实际 3 层深度、固定任务模板和 1 个关联对象。两份 Markdown 来源摘要不变，1280/480 与 0 错误通过，四张截图已接受。

代码审计确认两条输出都缺写盘前披露，但不能合并处理。图谱→Canvas 的前端对所有顶层对象开放，命令入口也先接受 CSV/TSV/Table，实际 `build_local_graph` 却只允许 Markdown/PDF，存在资格漂移；图谱→项目笔记的前后端均明确限定 Markdown/PDF，边界一致。

当前唯一接续点为 **M4C-4 图谱→项目笔记披露闭环**：披露相对中心、实际深度、同目录候选目标、不覆盖/编号/自动打开，以及追溯元数据、固定任务模板、排序、100 个关联对象上限、省略数和独立快照语义；真实覆盖首个/编号目标与超过 100 个关联对象。图谱→Canvas 资格纠偏与快照披露、全局框架、清理和发布冻结继续延期。详见 [`Post_v1.0.15_M4C3_Graph_Derived_Output_Disclosure_Selection_Audit_2026-08-29.md`](./Post_v1.0.15_M4C3_Graph_Derived_Output_Disclosure_Selection_Audit_2026-08-29.md)。

## 2026-08-29 M4C-4 图谱→项目笔记披露交接

M4C-4 已完成。顶层 Markdown/PDF 图谱对象生成项目笔记前，现在会披露资料库相对中心、当前 1～4 层范围、同目录候选目标、不覆盖/编号和自动打开，并明确追溯元数据、固定任务模板、不复制中心正文、标题/路径排序、100 个关联对象上限、省略数及独立时间点派生语义。创建成功后通知资料库并打开后端真实路径。
