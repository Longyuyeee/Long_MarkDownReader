...
- Rust：S8-1B2A 后共 135 项功能与性能测试；134 项功能回归全部通过，新增包级行插删、复杂载体拒绝和命令签名保护证据。
- 本机 XLSX 性能回归为 `inspect=273 ms / page=8,311 ms / patch=6,519 ms / total=15,105 ms`，分页读取略超 Debug CI 的 8 秒标准 runner 预算；机器相关性能继续以 GitHub Quality Gate 为发布裁决。
- 100 MiB PDF 范围读取基准：522 ms，仅读取约 256 KiB（目标小于 2 秒；不同机器会有波动）。
...
完整阶段审计、设计对齐和后续交错排期见 `docs/Development_Stage_Audit_2026-07-20.md`。当前工作区已完成 S6-8、T7-1、F7-1、S6-9、F7-2、S6-10～S6-15、G7-2、G7-3、I7-1、I7-2、W7-1～W7-3、S8-1A、S8-1B1 与 S8-1B2A；下一入口为 S8-1B2B / FR-DATA-009。
...
20. 下一批 S8-1B2B / FR-DATA-009：迁移合并、验证、筛选、冻结、条件格式、超链接、Table、图表、drawing anchors 和计算链，再接入前端行操作。
21. 完善 FR-BASE-005：增加关键保存流程 E2E、格式 fixture 和视觉回归门禁。
