...
- XLSX 多工作表预览、空白/已有基础单元格编辑、连续区域选择和 TSV 剪贴板，支持冲突检测及 OOXML 局部可靠写回；可将工作表转换为开放表格。
...
- `npm run ci:check`：通过。
- Rust：88 项测试通过，0 失败。
- 100 MiB PDF 范围读取基准：约 86 ms，仅读取约 256 KiB（目标小于 2 秒）。
...
3. 推进 FR-DATA-009：文本、数字、布尔、清空和公式支持单元格/区域编辑及 OOXML 局部写回；下一批读取样式 ID 并增加数字格式、字体、填充、边框和对齐编辑。
...
- XLSX 当前已具备基础单元格与连续区域编辑能力；合并区域只允许编辑左上角锚点，样式和公式重算尚未完成，不能提前宣传为 Excel 等价编辑器；完整等价编辑按兼容矩阵逐项验收。
