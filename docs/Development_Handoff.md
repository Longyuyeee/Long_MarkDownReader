...
- Rust：S8-1B2B1 后共 135 项功能与性能测试；工作表内复杂载体回归已扩展到冻结、选择、合并、筛选、验证、条件格式、超链接及真实包语义重读。
- 本机 XLSX 性能回归为 `inspect=325 ms / page=6,442 ms / patch=4,568 ms / total=11,336 ms`，全部处于 Debug CI 预算内；机器相关性能仍以 GitHub Quality Gate 为最终发布裁决。
- 100 MiB PDF 范围读取基准：107 ms，仅读取约 256 KiB（目标小于 2 秒；不同机器会有波动）。
...
完整阶段审计、设计对齐和后续交错排期见 `docs/Development_Stage_Audit_2026-07-20.md`。当前工作区已完成 S6-8、T7-1、F7-1、S6-9、F7-2、S6-10～S6-15、G7-2、G7-3、I7-1、I7-2、W7-1～W7-3、S8-1A、S8-1B1、S8-1B2A 与 S8-1B2B1；下一入口为 S8-1B2B2 / FR-DATA-009。
...
20. S8-1B2B1 已完成：工作表 XML 内的合并、验证、筛选、冻结、选择状态、条件格式和超链接随整行插删迁移；完整对象删除仍安全拒绝。
21. 下一批 S8-1B2B2 / FR-DATA-009：迁移 Table、图表、drawing anchors 和计算链，补齐复杂对象删除，再接入前端行操作。
22. 完善 FR-BASE-005：增加关键保存流程 E2E、格式 fixture 和视觉回归门禁。
