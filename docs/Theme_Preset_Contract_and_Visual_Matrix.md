...
前三套为核心预设，后四套为 T8-1 首批场景预设。注册表继续保留其余 12 套已有组合以兼容历史配置和命令面板；设置页不再暴露发布层级，而是统一为主题库，按浅色、深色、护眼、创意和高对比筛选，并隐藏视觉配置完全重复的组合。历史配置继续使用原有 `theme + visualStyle + codeTheme` 字段，不需要迁移；未知旧主题会安全回退到 `system`。命令面板应用预设时走 `store.applyThemePreset`，一次性更新并持久化色调、视觉风格、代码主题、编辑器背景与动效节奏。
