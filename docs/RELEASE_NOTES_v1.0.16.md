...
v1.0.16 聚焦专业格式工作流、行动工作台、知识图谱 2.0 与跨格式可靠输出。本文是候选发布说明；Quality Gate、安装包、Windows 生命周期和远端发布完成前，不代表已有可下载的 v1.0.16。
...
- v1.0.16 的完整补丁 Quality Gate、当前候选运行烟测、无签名 MSI/NSIS、托管 Windows 生命周期和远端附件复核仍必须依次执行。
