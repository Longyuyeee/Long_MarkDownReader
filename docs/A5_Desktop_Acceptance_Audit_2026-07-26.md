...
阶段 A 收口后，G8-1/G8-2A/G8-2B 和 PDF B0/B1A/B1B 继续复用同一真实桌面回归框架。当前门禁结果为 **31/31 项通过，23 张真实 Tauri 截图通过机器校验**。
...
- Rust 功能测试 `283/283` 通过，性能测试 `1/1` 通过；
- Rust 复杂工作簿性能测试 `1/1` 通过；
- 100 MiB PDF 范围读取只请求约 `255.9 KiB`，本轮本机约 `55 ms`；
- 生产依赖审计 `0` 个漏洞。
...
4. **PDF B1C（下一步）**：复杂生产者、页面继承、扫描页和高风险阻断兼容矩阵，不增加新操作；
5. DOCX/PPTX 先读后编，按公开 OOXML 兼容矩阵开放基础子集；
6. WPS `.wps/.et/.dps` 先做规范、许可与转换保真审计，无法证明可靠时只提供外部打开或受控副本转换；
7. 统一批处理、元数据、工作区健康和发布矩阵。

权威执行顺序和能力事实见 `docs/B1B_PDF_Reliable_Save_Audit_2026-07-27.md`。
