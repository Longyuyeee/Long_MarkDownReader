...
阶段 A 收口后，G8-1/G8-2A/G8-2B 和 PDF B0/B1A/B1B/B1C 继续复用同一真实桌面回归框架。当前门禁结果为 **32/32 项通过，24 张真实 Tauri 截图通过机器校验**。
...
- Rust 功能测试 `289/289` 通过，性能测试 `1/1` 通过；
...
5. **DOCX C0/C1（下一步）**：建立三类生产者 fixture 和只读工作面，再按公开 OOXML 兼容矩阵开放基础子集；
