        ...
        match reader.read_event() {
            Ok(Event::Start(event)) => {
                ...
                if current.is_none() && matches!(name, b"sp" | b"pic" | b"grpSp" | b"graphicFrame")
                {
                    ...
                    match name {
                        b"cNvPr" => {
                            state.id = attribute_value(&event, b"id", reader.decoder())?
                                .unwrap_or_default();
                            state.name = attribute_value(&event, b"name", reader.decoder())?
                                .unwrap_or_default();
                            state.alt_text = attribute_value(&event, b"descr", reader.decoder())?;
                        }
                        b"ph" => {
                        ...
                        b"prstGeom" => {
                            state.shape_type = attribute_value(&event, b"prst", reader.decoder())?;
                        }
                        b"off" => {
                            state.x = parse_i64(attribute_value(&event, b"x", reader.decoder())?);
                            state.y = parse_i64(attribute_value(&event, b"y", reader.decoder())?);
                        }
                        b"ext" => {
                            state.width =
                                parse_i64(attribute_value(&event, b"cx", reader.decoder())?);
                            state.height =
                                parse_i64(attribute_value(&event, b"cy", reader.decoder())?);
                        }
                        b"blip" => {
                            state.relationship_id =
                                attribute_value(&event, b"embed", reader.decoder())?;
                        }
                        b"t" => in_text = true,
                        _ => {}
                    }
...
pub fn parse_pptx(bytes: &[u8]) -> Result<PptxPresentationModel, String> {
    ...
    for (index, (slide_id, relationship_id, hidden)) in ordered_slide_ids.into_iter().enumerate() {
        ...
        let (objects, has_background, has_animation, mut warnings) =
            parse_slide(&slide_xml, &slide_relationships)?;
        if has_animation {
            animation_count += 1;
        }
        ...
        let mut output = Cursor::new(Vec::new());
        {
            ...
            let entries = [
                ...
                (
                    ...
                    r#"<p:sld xmlns:p="p" xmlns:a="a" xmlns:r="r"><p:cSld><p:spTree><p:sp><p:nvSpPr><p:cNvPr id="2" name="Title 1"/></p:nvSpPr><p:spPr><a:xfrm><a:off x="10" y="20"/><a:ext cx="300" cy="100"/></a:xfrm><a:prstGeom prst="rect"/></p:spPr><p:txBody><a:p><a:r><a:t>Audit title</a:t></a:r></a:p></p:txBody></p:sp><p:pic><p:nvPicPr><p:cNvPr id="3" name="Picture" descr="Audit image"/></p:nvPicPr><p:blipFill><a:blip r:embed="rId2"/></p:blipFill></p:pic></p:spTree></p:cSld><p:timing/></p:sld>"#,
                ),
                (
                    "ppt/slides/_rels/slide1.xml.rels",
                    r#"<Relationships xmlns="r"><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="../media/image1.png"/><Relationship Id="rId3" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/notesSlide" Target="../notesSlides/notesSlide1.xml"/></Relationships>"#,
                ),
                (
