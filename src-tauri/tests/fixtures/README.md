...
- `workbook/chart-visual-matrix.xlsx`：包含同页可编辑柱状图、折线图、饼图和散点图的桌面视觉与往返基线。
- `workbook/chart-visual-matrix.json`：记录四类图表的标题、图例位置和序列数量契约。
...
- 运行 `cargo run --manifest-path src-tauri/Cargo.toml --example generate_chart_visual_fixture` 可重新生成图表视觉 fixture。
